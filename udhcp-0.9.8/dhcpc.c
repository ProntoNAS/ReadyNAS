...
printf("enter dhcpd 2\n");
	pid_fd = pidfile_acquire(client_config.pidfile);
	pidfile_write_release(pid_fd);

printf("enter dhcpd 3\n");
	if (read_interface(client_config.interface, &client_config.ifindex, 
			   NULL, client_config.arp) < 0)
		exit_client(1);
printf("enter dhcpd 4\n");
...
printf("enter dhcpd 5\n");
	/* setup signal handlers */
	socketpair(AF_UNIX, SOCK_STREAM, 0, signal_pipe);
	signal(SIGUSR1, signal_handler);
	signal(SIGUSR2, signal_handler);
	signal(SIGTERM, signal_handler);
printf("enter dhcpd 6\n");
	
	state = INIT_SELECTING;
	run_script(NULL, "deconfig");
	change_mode(LISTEN_RAW);
printf("enter dhcpd 7\n");

	for (;;) {

printf("enter dhcpd for loop\n");
...
printf("enter dhcpd listen mode\n");
			if (listen_mode == LISTEN_KERNEL) {
printf("enter dhcpd listen mode 1\n");

				fd = listen_socket(INADDR_ANY, CLIENT_PORT, client_config.interface);
			}
			else {
printf("enter dhcpd listen mode 2\n");
