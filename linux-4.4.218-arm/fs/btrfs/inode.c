 ...
 */
int btrfs_inode_by_name(struct inode *dir, struct dentry *dentry,
			       struct btrfs_key *location)
{
...
#ifdef CONFIG_BTRFS_FNAME_CASEINSENSITIVE
	if (ctx->actor == caselookup_actor)
		case_flag = 1;
	if (case_flag)
		btrfs_get_delayed_items_case(inode, &ins_list, &del_list);
	else
#endif
...
#ifdef CONFIG_BTRFS_FNAME_CASEINSENSITIVE
		if (case_flag == 1) {
			if (btrfs_should_delete_dir_index_case(&del_list, found_key.offset))
				goto next;
		}
		else
#endif
...
#ifdef CONFIG_BTRFS_FNAME_CASEINSENSITIVE
	if (case_flag)
		ret = btrfs_readdir_delayed_dir_index_case(ctx, &ins_list);
	else
#endif
...
#ifdef CONFIG_BTRFS_FNAME_CASEINSENSITIVE
	if (case_flag)
		btrfs_put_delayed_items_case(&ins_list, &del_list);
	else
#endif
						...
						struct qstr *qstr)
{
	...
	if (CASEINSENSITIVE_CHECK(root)) {
		qstr->hash = btrfs_name_hash(qstr->name, qstr->len);
		return 0;
	}
	...
	iov.iov_base = (void *)qstr->name;
	iov.iov_len = qstr->len;

	while (iov.iov_len > 0) {

		u32 cp = CASEINSENSITIVE_OPS(root)->toupper_vec(&iov, 1);
		u16 c;
		...
		if (c == 0xffff)
			break;
		hash = partial_name_hash((unsigned char)(c >> 8), hash);
	}
		...
		unsigned int len, const char *str, const struct qstr *name)
{
	...
	if (!CASEINSENSITIVE_R(root))
		return strncmp(str, name->name, name->len);
	...
	return CASEINSENSITIVE_OPS(root)->strcasecmp_vec(&iov1, 1, &iov2, 1);
}
#endif
