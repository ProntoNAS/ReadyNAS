...
#define find_name_in_backref(path, name, name_len, ref_ret)	\
	__find_name_in_backref((root), (path), (name), (name_len), (ref_ret))

static int __find_name_in_backref(struct btrfs_root *root,
...
int __btrfs_find_name_in_ext_backref(struct btrfs_root *root,
				   struct btrfs_path *path, u64 ref_objectid,
			...
			u64 inode_objectid, u64 ref_objectid, u64 *index)
{
	...
	ret = btrfs_search_slot(trans, root, &key, path, -1, 1);
#ifdef CONFIG_BTRFS_FNAME_CASEINSENSITIVE
	if (ret > 0 && CASEINSENSITIVE_R(root) && inode_objectid >= 256ULL) {
		struct btrfs_dir_item *p;
