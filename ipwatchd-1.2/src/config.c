...
int ipwd_read_config (const char *filename)
{
	...
	while (fgets (line, 499, fr) != NULL)
	{
		...
		if (strcmp (variable, "defend_interval") == 0)
		{
			...
			if ((config.defend_interval < 0) || (config.defend_interval > 1800))
			{
				ipwd_message (IPWD_MSG_ERROR, "Configuration parse error : defend_interval value must be between 0 and 1800");
				return (IPWD_RV_ERROR);
			}
