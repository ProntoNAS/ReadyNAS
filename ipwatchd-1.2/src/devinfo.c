...
int ipwd_devinfo (const char *p_dev, char *p_ip, char *p_mac)
{
	...
	if (ioctl (sock, SIOCGIFADDR, &ifr) < 0)
	{
		...
		ipwd_message (IPWD_MSG_ERROR, "Could not retrieve IP address of the device \"%s\": %s", p_dev, strerror(errno));
		close (sock);
		return (IPWD_RV_ERROR);
	}
...
trymac:
	...
	{
		ipwd_message (IPWD_MSG_ERROR, "Could not retrieve MAC address of the device \"%s\": %s", p_dev, strerror(errno));
		close (sock);
		return (IPWD_RV_ERROR);
	}
	...
	{
		ipwd_message (IPWD_MSG_ERROR, "Could not convert MAC address of the device \"%s\"", p_dev);
		close (sock);
		return (IPWD_RV_ERROR);
	}
...
int ipwd_fill_devices (void)
{
	...
	for (i = 0; i < ifaces_num; i++)
	{
		...
		{
			ipwd_message (IPWD_MSG_ERROR, "Could not get interface flags for %s", iface->ifr_name);
			continue;
		}
		if(iface->ifr_ifru.ifru_flags & IFF_LOOPBACK)
		{
			ipwd_message (IPWD_MSG_DEBUG, "Skipping loopback device \"%s\"", iface->ifr_name);
			continue;
		}
