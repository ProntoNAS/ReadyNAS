...
void ipwd_analyse (u_char * args, const struct pcap_pkthdr *header, const u_char * packet)
{
	...
	for (i = 0; i < devices.devnum; i++)
	{
		...
		if (testing_flag == 0)
		{
			...
			{
				int ii;
				for (ii = 0; ii < devices.devnum; ii++)
				{
					if (ipwd_devinfo (devices.dev[ii].device, devices.dev[ii].ip, devices.dev[ii].mac) == IPWD_RV_ERROR)
					{
						ipwd_message (IPWD_MSG_ERROR, "Unable to get IP and MAC address of %s", devices.dev[ii].device);
						continue;
					}
					if (strcmp (rcv_smac, devices.dev[ii].mac) == 0)
					{
						ii = -1;
						break;
					}
		...
		{
			ipwd_message (IPWD_MSG_DEBUG, "MAC address %s causes IP conflict with address %s set on interface %s - no action taken because this happened within the defend interval", rcv_smac, devices.dev[i].ip, devices.dev[i].device);
			break;
		}
		...
		if (config.script != NULL)
		{
			...
			struct ether_addr *addr = ether_aton(rcv_smac);
			if (asprintf (&command, "%s \"%s\" \"%s\" \"%.02X:%.02X:%.02X:%.02X:%.02X:%.02X\"",
			              config.script, devices.dev[i].device, devices.dev[i].ip,
