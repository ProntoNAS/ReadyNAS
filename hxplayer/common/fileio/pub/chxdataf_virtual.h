/* ***** BEGIN LICENSE BLOCK *****
 * Source last modified: $Id: chxdataf_virtual.h,v 1.1.1.1 2006/03/29 16:45:37 hagi Exp $
 * 
...
/* 	Create creates a file using the specified mode */
	virtual HX_RESULT	Create			(const char *filename, UINT16 mode, BOOL textflag = 0)= 0;

	/* 	Open will open a file with the specified permissions */
	virtual HX_RESULT	Open			(const char *filename, UINT16 mode, BOOL textflag = 0) = 0;

/* 	Open will open a file for sharing with the specified permissions */
	virtual HX_RESULT	OpenShared    		(const char *filename, UINT16 mode, UINT16 sharedmode, BOOL textflag = 0) {return Open(filename, mode, textflag);}
