/* ***** BEGIN LICENSE BLOCK *****
 * Source last modified: $Id: chxdataf_stdio.h,v 1.1.1.1 2006/03/29 16:45:37 hagi Exp $
 * 
...
{
public:
	static	CHXDataFile*	Construct		(UINT32 ulFlags = 0, IUnknown** ppCommonObj = NULL);
	
	virtual			~CHXDataFile		(void);

	BOOL	 	GetTemporaryFileName	(const char *tag, char* name, UINT32 ulBufLen);
...
/* 	Create creates a file using the specified mode */
	HX_RESULT	Create			(const char *filename, UINT16 mode, BOOL textflag = 0);

	/* 	Open will open a file with the specified permissions */
	HX_RESULT	Open			(const char *filename, UINT16 mode, BOOL textflag = 0);

/* 	Open will open a file for sharing with the specified permissions */
	HX_RESULT	OpenShared    		(const char *filename, UINT16 mode, UINT16 sharedmode, BOOL textflag = 0) {return Open(filename, mode, textflag);}
...
protected:
        ...
        FILE *                  m_FP;
	CHXDataFile		(void) : mLastError(HXR_OK),m_FP(NULL){};
};
