/* ***** BEGIN LICENSE BLOCK *****
 * Source last modified: $Id: fio.h,v 1.1.1.1 2006/03/29 16:45:38 hagi Exp $
 * 
...
#include "hlxclib/windows.h" // for GetFileSize
...
{
    err = 0;
    fd = ::_open(file, HX_SAFEINT(flags), mode);
    _flags = flags;
    if (fd < 0)
	err = errno;
}
...
{
    int ret = ::_read(fd, buf, HX_SAFEINT(len));
    if (ret < 0)
	err = errno;
    return ret;
}
...
{
    LONG32 ret = ::_write(fd, buf, HX_SAFEINT(len));
    if (ret < 0)
	err = errno;
    return ret;
}
...
{
#ifdef _WINCE
    DWORD res = GetFileAttributes(Unicode(path));
    return (res == 0xffffffff) ? 0 : res & FILE_ATTRIBUTE_DIRECTORY;
#else
    struct stat st;
