/* ***** BEGIN LICENSE BLOCK *****
 * Source last modified: $Id: cunixf.cpp,v 1.1.1.1 2006/03/29 16:45:37 hagi Exp $
 * 
...
HX_RESULT CUnixFile::Create(const char *filename, UINT16 mode, BOOL textflag)
{
...
HX_RESULT CUnixFile::Open(const char *filename, UINT16 mode, BOOL textflag)
{
        // close previous file if necessary
        if ( mFD > 0 )
            close( mFD );
...
BOOL CUnixFile::GetTemporaryFileName(const char *tag, char* name, UINT32 ulBufLen)
{
#ifdef _MAC_UNIX

	// GR 7/15/03 other Unix implementations should probably use this (or tempnam)
	// instead of tmpnam
