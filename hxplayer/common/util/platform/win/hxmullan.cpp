/* ***** BEGIN LICENSE BLOCK *****
 * Source last modified: $Id: hxmullan.cpp,v 1.1.1.1 2006/03/29 16:45:32 hagi Exp $
 * 
...
BOOL		g_bWindows31 = FALSE;	// We need to know about Win3.1 
...
public:
	...
	~CDynamicLoadLanguageService();
	BOOL IsValidCodePage(UINT CodePage) const;
	int GetLocaleInfoA(LCID LCID, LCTYPE LCType, LPSTR lpLCData, 
		int cchData) const;
	BOOL IsValid() const { return (m_hInstance > (HINSTANCE)32 && fpGetLocaleInfoA != NULL);};
};
...
BOOL CDynamicLoadLanguageService::IsValidCodePage(UINT CodePage) const
{
...
void HXSetupMulLang(HINSTANCE hInstBase, BOOL bWin31)
{
...
BOOL AddLocaleListbox(HWND hwndListBox, LCID locale)
{
