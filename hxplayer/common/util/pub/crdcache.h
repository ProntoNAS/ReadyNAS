/* ***** BEGIN LICENSE BLOCK *****
 * Source last modified: $Id: crdcache.h,v 1.1.1.1 2006/03/29 16:45:32 hagi Exp $
 * 
...
{
    CredentialEntry(char* pUserName, char* pPassword)
    {
	m_pUserName = NULL;
	m_pPassword = NULL;
	...
	if (pUserName)
	{
	    ...
	    m_pUserName->AddRef();
	    m_pUserName->Set((const unsigned char*)pUserName, strlen(pUserName)+1);
	}

	if (pPassword)
	{	    
	    m_pPassword = new CHXBuffer();
	    m_pPassword->AddRef();
	    m_pPassword->Set((const unsigned char*)pPassword, strlen(pPassword)+1);
	}
    }
...
public:
    ...
    STDMETHOD_(BOOL, IsEmpty) (THIS_ IHXBuffer* pBuffer);
