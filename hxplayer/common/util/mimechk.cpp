/* ***** BEGIN LICENSE BLOCK *****
 * Source last modified: $Id: mimechk.cpp,v 1.1.1.1 2006/03/29 16:45:31 hagi Exp $
 * 
...
BOOL isProhibitedMimeType(const char* pMimeType, ULONG32 ulMimeLen)
{
    BOOL bProhibited = TRUE;
...
BOOL decryptMimeType(CHXString &strMimeType)
{
    BOOL bRetVal = FALSE;	// assume no decryption
...
BOOL encryptMimeType(CHXString &strMimeType)
{
    BOOL bRetVal = FALSE;	// assume no encryption
    ULONG32 ulMimeLength = strMimeType.GetLength();
    const char* pMimeChars = (const char*) (strMimeType);
