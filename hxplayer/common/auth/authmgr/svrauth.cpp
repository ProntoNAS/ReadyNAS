/* ***** BEGIN LICENSE BLOCK *****
 * Source last modified: $Id: svrauth.cpp,v 1.1.1.1 2006/03/29 16:45:27 hagi Exp $
 * 
...
)
{
    ...
    if(!pIHXServerAuthResponseRequester)
    {
    ...
    {
	pIHXServerAuthResponseRequester->ChallengeReady
	(
	    HXR_UNEXPECTED, 
	    NULL
	);
    ...
    m_spIHXValuesChallengeHeaders = new CHXHeader;
...
)
{
	...
	m_spIHXServerAuthResponseRequester->ChallengeReady
	(
	    m_HX_RESULTStatus, 
	    m_spIHXRequestResponse
	);
...
)
{
    ...
    (*ppIHXValuesValues) = new CHXHeader();
    (*ppIHXValuesValues)->AddRef();
	...
	switch(type)
	{
		...
		{
		    (*ppIHXValuesValues)->SetPropertyULONG32
		    (
			propSubName, 
			val
		    );
	    ...
	    case PT_STRING:
	    {
		if(HXR_OK == spregRegistry->GetStrById(prop_id,
						   pBuffer))
		{
		    (*ppIHXValuesValues)->SetPropertyCString
		    (
			propSubName,
			pBuffer
		    );
	    ...
	    case PT_BUFFER:
	    {
		if(HXR_OK == spregRegistry->GetBufById(prop_id,
						   pBuffer))
		{
		    (*ppIHXValuesValues)->SetPropertyBuffer
		    (
			propSubName,
			pBuffer
		    );
