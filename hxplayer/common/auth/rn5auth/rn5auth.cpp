...
//  $Id: rn5auth.cpp,v 1.1.1.1 2006/03/29 16:45:27 hagi Exp $
...
STDMETHODIMP
CRN5Authenticator::Authenticated(BOOL bAuthenticated)
{
    return HXR_OK;
}
...
{
    pOptions = new CHXHeader();

    if (pOptions)
    {
	pOptions->AddRef();

	_SetPropertyFromCharArray(pOptions, "PluginType", "Authenticator");
