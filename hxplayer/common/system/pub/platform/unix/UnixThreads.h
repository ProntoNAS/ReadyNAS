/* ***** BEGIN LICENSE BLOCK *****
 * Source last modified: $Id: UnixThreads.h,v 1.1.1.1 2006/03/29 16:45:36 hagi Exp $
 * 
...
public:
    ...
    HXUnixEvent(const char* pEventName = NULL, BOOL bManualReset=TRUE);
    virtual ~HXUnixEvent();
...
class HXUnixThread : public HXThread
{
  public:
    ...
    //This call returns the *CALLING THREAD's ID* not the one that was created.
    virtual ULONG32 GetCurrentThreadID();
    ...
    //
    // New methods for Unix thread support.
    //

    //Cancels the child thtread.
    HX_RESULT CancelThread();

    //Waits for the child thread to exit and return the thread result code.
...
protected:
    ...
    //Just returns the calling threads ID. Hope we never have a platform
    //that doesn't use ints or longs for thread IDs.
    virtual ULONG32 _thread_self() = 0;
