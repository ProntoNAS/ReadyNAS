/* ***** BEGIN LICENSE BLOCK *****
 * Source last modified: $Id: unix_dll.cpp,v 1.1.1.1 2006/03/29 16:45:36 hagi Exp $
 * 
...
class UnixDLLAccess : public DLLAccessImp
{
public:
    UnixDLLAccess();
    virtual ~UnixDLLAccess();
...
int UnixDLLAccess::Open(const char* dllName)
{
...
void* UnixDLLAccess::GetSymbol(const char* symbolName)
{
    return dlsym(m_handle, symbolName);
}

const char* UnixDLLAccess::GetErrorStr() const
{
    return dlerror();
}

char* UnixDLLAccess::CreateVersionStr(const char* dllName) const
{
