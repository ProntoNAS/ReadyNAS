/* ***** BEGIN LICENSE BLOCK *****
 * Source last modified: $Id: unix_conn.cpp,v 1.1.1.1 2006/03/29 16:45:28 hagi Exp $
 * 
...
{
    return unix_net::new_socket(type);
}
...
{
    return unix_net::init_drivers(params);
}

HX_RESULT
conn::close_drivers (void *params)
{
    return unix_net::close_drivers(params);
}

HX_RESULT
conn::host_to_ip_str(char *host, char *ip, UINT32 ulIPBufLen)
{
    return unix_net::host_to_ip_str(host,ip, ulIPBufLen);
}
...
conn::get_host_name(char *name, int namelen)
{
    ...
    return unix_net::get_host_name(name, namelen);
}
...
conn::get_host_by_name(char *name, REF(struct hostent*) hostent)
{
    ...
    return unix_net::get_host_by_name(name, hostent);
}
