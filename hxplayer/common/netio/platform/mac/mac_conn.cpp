/* ***** BEGIN LICENSE BLOCK *****
 * Source last modified: $Id: mac_conn.cpp,v 1.1.1.1 2006/03/29 16:45:28 hagi Exp $
 * 
...
#include "platform/mac/mac_net.h"

conn*
conn::actual_new_socket(UINT16 type)
{
    return Mac_net::new_socket(type);
}
...
{
    return Mac_net::init_drivers(params);
}

HX_RESULT
conn::close_drivers (void *params)
{
    return Mac_net::close_drivers(params);  
}
...
{
    return Mac_net::host_to_ip_str(host,ip, ulIPBufLen);
}
...
conn::get_host_name(char *name, int namelen)
{
    ...
    return Mac_net::get_host_name(name, namelen);
}
...
{
    return Mac_net::get_host_by_name(name, hostent);
}
