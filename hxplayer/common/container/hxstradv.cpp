/* ***** BEGIN LICENSE BLOCK *****
 * Source last modified: $Id: hxstradv.cpp,v 1.1.1.1 2006/03/29 16:45:28 hagi Exp $
 * 
...
void CHXString::AppendULONG(ULONG32 value)
{
    ...
    int tmp=0;	// TO FIX UNUSED VARIABLE WARNING
...
void CHXString::TrimRight()
{
	...
	if (strSize)
	{
	    ...
	    for(;(pCur >= m_pRep->GetBuffer()) && (isspace(*pCur)); pCur--, strSize--);
...
void CHXString::TrimLeft()
{
	...
	for(; (*pStart) && (isspace(*pStart)); pStart++);
...
BOOL CHXString::FindAndReplace(const char* pSearch , const char* pReplace,
				BOOL bReplaceAll)
{
    BOOL ret = FALSE;
