/* ***** BEGIN LICENSE BLOCK *****
 * Source last modified: $Id: chxmaplongtoobj.cpp,v 1.1.1.1 2006/03/29 16:45:28 hagi Exp $
 * 
...
#define MAP_TYPE CHXMapLongToObj
#include "chxmapcommon_inl.h"

DECLARE_ITEMVEC_IMP(CHXMapLongToObj,ItemVec_t,Item,Item(),0,0);

// ==================== Code specific to the type of key and/or value goes below here.

void CHXMapLongToObj::ConstructTypeSpecifics()
{
}

ULONG32 CHXMapLongToObj::DefaultHashFunc (key_type key)
{
    // XXXSAB: Need something to reasonably distribute longs...anyone have
    //         an idea?
    ULONG32 ret = (ULONG32)key;
    ret = ret ^ ((ret << 8) | (ret >> (sizeof(ULONG32)*8 - 8)));
    return ret;
}

void CHXMapLongToObj::Dump() const
{
#ifdef _DEBUG
    int i;
    printf("(CHXMapLongToObj*)%p:\n", this);
    printf("   items[sz=%d/%d]", m_items.size(), m_items.capacity());
    for (i = 0; i < m_items.size(); ++i)
    {
        printf("%s{%ld,%p%s}",
               (i % 3) ? " " : "\n      ",
               m_items[i].key, m_items[i].val,
