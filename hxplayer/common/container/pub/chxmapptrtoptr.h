/* ***** BEGIN LICENSE BLOCK *****
 * Source last modified: $Id: chxmapptrtoptr.h,v 1.1.1.1 2006/03/29 16:45:28 hagi Exp $
 * 
...
#ifndef _CHXMAPPTRTOPTR_H_
#define _CHXMAPPTRTOPTR_H_
...
public:
    ...
    typedef void* key_arg_type;
    typedef void* key_ref_type;
    inline static key_type key_nil() { return NULL; }
...
public:
    ...
    CHXMapPtrToPtr(int chunkSize = z_defaultChunkSize);
    ~CHXMapPtrToPtr();
    ...
    BOOL Lookup(key_arg_type key, value_arg_type& value) const;
...
private:
    ...
    {
        return ((CHXMapPtrToPtr*)this)->LookupItem(bucket, key);
    }
...
int CHXMapPtrToPtr::GetCount() const
{
    return m_items.size() - m_free.size();
}

BOOL CHXMapPtrToPtr::IsEmpty() const
{
    return GetCount() == 0;
}

ULONG32 CHXMapPtrToPtr::GetHashTableSize() const
{
    return m_numBuckets;
}

CHXMapPtrToPtr::HashFunc_t CHXMapPtrToPtr::SetHashFunc (
    CHXMapPtrToPtr::HashFunc_t hf)
{
    HashFunc_t old = m_hf;
    m_hf = hf;
    return old;
...
ULONG32 CHXMapPtrToPtr::HashKey (key_arg_type key) const
{
    if (m_hf) return m_hf(key);
    return DefaultHashFunc(key);
}

void CHXMapPtrToPtr::SetDefaultNumBuckets (ULONG32 numBuckets)
{
#if !defined(HELIX_CONFIG_NOSTATICS)
...
void CHXMapPtrToPtr::SetDefaultChunkSize (ULONG32 chunkSize)
{
#if !defined(HELIX_CONFIG_NOSTATICS)
...
void CHXMapPtrToPtr::SetDefaultBucketChunkSize (ULONG32 chunkSize)
{
#if !defined(HELIX_CONFIG_NOSTATICS)
...
void CHXMapPtrToPtr::SetBucketChunkSize (ULONG32 chunkSize)
{
    m_bucketChunkSize = chunkSize;
}

#endif // _CHXMAPPTRTOPTR_H_
