/* ***** BEGIN LICENSE BLOCK *****
 * Source last modified: $Id: unkimp.h,v 1.1.1.1 2006/03/29 16:45:28 hagi Exp $
 * 
...
inline STDMETHODIMP
CUnknownIMP::SetupAggregation( IUnknown* pvOuterObj, IUnknown** pvResult )
{
    if( !pvResult )
	return HXR_POINTER;

    // Initial result code, initialize out parameter
    HX_RESULT result = HXR_FAIL;
    *pvResult = NULL;

    // This can only happen before any calls to AddRef()
    HX_ASSERT( 0 == m_lCount );

    if( !m_lCount )
    {
	if(pvOuterObj)
	{
	    m_punkOuter = pvOuterObj;
	    m_punkControlling = new CAggregateImpl( this );
	    m_punkControlling->AddRef();

	    result = HXR_OK;
	}
	else
	{
	    result = QueryInterface( IID_IUnknown, (void**) &m_punkControlling );
	    HX_ASSERT( SUCCEEDED( result ) );
	}
    }

    if( SUCCEEDED( result ) )
	*pvResult = m_punkControlling;
