/* ***** BEGIN LICENSE BLOCK *****
 * Source last modified: $Id: chxmapcommon_inl.h,v 1.1.1.1 2006/03/29 16:45:28 hagi Exp $
 * 
...
BOOL MAP_TYPE::Iterator::operator==(const Iterator& other) const
{
...
BOOL MAP_TYPE::Lookup(key_arg_type key, int& retItem) const
{
...
BOOL MAP_TYPE::Lookup(key_arg_type key, value_ref_type value) const
{
    if (m_buckets.empty()) return FALSE;
...
BOOL MAP_TYPE::RemoveKey(key_arg_type key)
{
...
HX_RESULT MAP_TYPE::InitHashTable(ULONG32 numBuckets, BOOL bAlloc)
{
    RemoveAll();
