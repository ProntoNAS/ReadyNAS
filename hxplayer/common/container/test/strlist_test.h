/* ***** BEGIN LICENSE BLOCK *****
 * Source last modified: $Id: strlist_test.h,v 1.1.1.1 2006/03/29 16:45:28 hagi Exp $
 * 
...
class HLXStrListTest : public HLXCmdBasedTest
{
public:
    HLXStrListTest();
    ~HLXStrListTest();
...
protected:
    CHXString* CreateValue(const char* pStr);
    void DestroyValue(void* pStr);
    bool GetValue(void* pStr, CHXString& val);
    bool FindValue(const CHXString& value, LISTPOSITION pos, void*& pValue);
    ...
    bool HandleAddHeadStringCmd(const UTVector<UTString>& info);
    bool HandleAddTailStringCmd(const UTVector<UTString>& info);
    bool HandleAddStringAlphabeticCmd(const UTVector<UTString>& info);
    bool HandleFindStringCmd(const UTVector<UTString>& info);
    bool HandleFindPrefixSubstringCmd(const UTVector<UTString>& info);
    bool HandleRemoveHeadStringCmd(const UTVector<UTString>& info);
    bool HandleRemoveTailStringCmd(const UTVector<UTString>& info);

    bool HandleDumpCmd(const UTVector<UTString>& info);

private:
    CHXStringList m_list;
    LISTPOSITION m_pos;
};
