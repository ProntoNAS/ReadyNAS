/* ***** BEGIN LICENSE BLOCK *****
 * Source last modified: $Id: xmlvalid.cpp,v 1.1.1.1 2006/03/29 16:45:27 hagi Exp $
 * 
...
BOOL UTF16Valid::IsEntityValueValid(const UINT16* p, UINT32 len)
{
    // copy UTF8 after it has been tested.
    return FALSE;
}

BOOL UTF16Valid::IsAttValueValid(const UINT16* p, UINT32 len)
{
    // copy UTF8 after it has been tested.
    return FALSE;
}

BOOL UTF16Valid::IsSystemLiteralValid(const UINT16* p, UINT32 len)
{
    return TRUE;
}

BOOL UTF16Valid::IsPubidLiteralValid(const UINT16* p, UINT32 len)
{
    BOOL bValid = TRUE;
    for (const UINT16* end = p + len; bValid && *p && p < end; ++p )
    {
...
inline BOOL UTF16Valid::IsLetter(const UINT16 c)
{
    return IsBaseChar(c) || IsIdeographic(c);
}

inline BOOL UTF16Valid::IsBaseChar(const UINT16 c)
{
    return (bsearch(&c, pBaseChar, sizeof(pBaseChar)/sizeof(Node), sizeof(Node), compare) ? TRUE : FALSE );
}


inline BOOL UTF16Valid::IsIdeographic(const UINT16 c)
{
...
inline BOOL UTF16Valid::IsCombiningChar(const UINT16 c)
{
...
inline BOOL UTF16Valid::IsDigit(const UINT16 c)
{
    return (bsearch(&c, pDigitChar, sizeof(pDigitChar)/sizeof(Node), sizeof(Node), compare) ? TRUE : FALSE );
}

inline BOOL UTF16Valid::IsExtender(const UINT16 c)
{
...
void main()
{
    ...
    {
	BOOL b = UTF16Valid::IsBaseChar(i);
    ...
    for ( i = 0; i < 0xFFFF; ++i )
    {
	BOOL b = UTF16Valid::IsCombiningChar(i);
	if ( ( b && !inRange) || ( !b && inRange ) )
	{
	    printf("0x%X, ", i);
	    if ( count == 8 )
