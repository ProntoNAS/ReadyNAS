/* ***** BEGIN LICENSE BLOCK *****
 * Source last modified: $Id: hxmemprb.h,v 1.1.1.1 2006/03/29 16:45:37 hagi Exp $
 * 
...
public:
     ...
     */
    CHXMemProbe(BOOL bIsFrozen = TRUE);
    ~CHXMemProbe();

    LONG32	GetMemUsed(BOOL bWithCrt = TRUE, BOOL bWithBase = FALSE, CHXMemProbe *pReferenceProbe = NULL);
    LONG32	GetBaseMemUsed(BOOL bWithCrt = TRUE);
    HX_RESULT	DumpMemUsedSinceReset(FILE *pFile = NULL, BOOL bWithCrt = TRUE);
    ULONG32	GetMaxMemUsed(BOOL bWithCrt = TRUE, BOOL bWithBase = FALSE, CHXMemProbe *pReferenceProbe = NULL);
    BOOL	IsRestored(BOOL bWithCrt = TRUE);

    HX_RESULT	Freeze(BOOL bGenerateDetailInfo = FALSE, BOOL bWithCrt = TRUE);
    ...
    static LONG32   GetGlobalMemUsed(BOOL WithCrt = TRUE);
