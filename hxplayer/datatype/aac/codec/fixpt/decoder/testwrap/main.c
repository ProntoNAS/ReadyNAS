/* ***** BEGIN LICENSE BLOCK *****  
 * Source last modified: $Id: main.c,v 1.4 2006/04/06 15:20:26 hagi Exp $ 
 *   
...
void Usage(void)
{
	printf("\nUsage:\n");
	printf("   %s (build: %s)\n", progname, build);
	printf(">> %s [-wav] infile outfile\n", progname);
        printf("     -wav:    if specified, then outfile is a .wav file.\n");
        printf("              otherwise, outfile is a raw PCM file\n");
	printf("     infile:  input file. can be -stdin\n");
	printf("     outfile: output file. can be -stdout\n");
}
...
void WriteWAVHeader(FILE* fp, AACFrameInfo* pFrameInfo)
{
    ...
    if (fp && pFrameInfo && pFrameInfo->nChans <= 2)
    {
        ...
        pack32LE(27500580, &pTmp, &ulTmp);	// HAGI:: just non zero (datasize+36)
        ...
        pack32LE(27500544, &pTmp, &ulTmp);	// HAGI:: just nonzero
...
//	float totalDecTimeTNSOn, totalDecTimeTNSOff, audioSecs, audioFrameSecs, currMHz, peakMHz;	/* for calculating percent RT */
	...
	signal(SIGPIPE,  (void (*)(int))aac2wav_abort);
