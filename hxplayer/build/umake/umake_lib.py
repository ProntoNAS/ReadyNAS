# 
# ***** BEGIN LICENSE BLOCK *****
# Source last modified: $Id: umake_lib.py,v 1.1.1.1 2006/03/29 16:45:39 hagi Exp $
# 
...
def SetupProject(platform, project):
    ...
    ## create the objects list by checking all the source files
    ## to see if they exist; if they do, then change the suffix and
    ## add them to project.objects; if the source file does not exist,
    ## then we need to check for object files that look like they could
    ## have been generated by the source files if they actually existed
    ## and add them to project.srcobjs

    ## must use the list copy syntax for project.sources because we're
    ## going to remove the source files which do not exist from the source
    ## list from within the loop
    if sysinfo.host_type != "mac":
        for path in project.sources[:]:
            sourcefile = SourceFile(platform, path, project.object_dir)

            ## link_output is a boolean flag which tells us if the output
            ## from the build rule should be put on the final link line;
            ## in this case, we're not interested in the source if it
            ## doesn't belong in the link line
            if not sourcefile.build_rule.link_output:
                continue

            ## if the source file is found
            if os.path.isfile(sourcefile.path):
                project.objects.append(sourcefile.obj_path)
                continue

            ## deal with missing source files
            warning('missing source file="%s"' % (sourcefile.path))

            if os.path.isfile(sourcefile.obj_path):
                project.RemoveSources(sourcefile.path)
                project.AddSourceObjects(sourcefile.obj_path)
            else:
                fatal('could not find object file for missing source="%s"' % (
                    sourcefile.path))
