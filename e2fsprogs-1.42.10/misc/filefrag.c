...
static void print_extent_header(void)
{
	printf(" ext: %*s %*s length: %*s flags:\n",
	       ...
	       physical ? 0 : physical_width + 1,
	       physical ? "device:" : "expected:");
}
			      ...
			      ext2fs_struct_stat *st)
{
	...
	unsigned long long lext_blks;
	unsigned long long logical_len;
	char flags[256] = "";
	...
	logical_len = (fm_extent->fe_flags & FIEMAP_EXTENT_LOGLENGTH) ?
			fm_extent->fe_loglength : fm_extent->fe_length;
	lext_blks = (logical_len - 1) >> blk_shift;
	logical_blk = fm_extent->fe_logical >> blk_shift;
	physical_blk = fm_extent->fe_physical >> blk_shift;

	if (physical) {
		if (fm_extent->fe_flags & FIEMAP_EXTENT_PHYSICAL)
			sprintf(flags, "%3d:%-3d ",
				major(fm_extent->fe_pdev),
				minor(fm_extent->fe_pdev));
		else
			strcat(flags, "        ");
	} else if (expected)
	...
	if (fm_extent->fe_logical + logical_len >= (__u64) st->st_size)
		strcat(flags, "eof,");
	...
	printf(ext_fmt, cur_ex, logical_width, logical_blk,
	       logical_width, logical_blk + lext_blks,
			   ...
			   ext2fs_struct_stat *st)
{
	...
	do {
		...
		for (i = 0; i < fiemap->fm_mapped_extents; i++) {
			...
			next_start = fm_ext[i].fe_logical +
			    ((fm_ext[i].fe_flags & FIEMAP_EXTENT_LOGLENGTH) ?
					fm_ext[i].fe_loglength :
					fm_ext[i].fe_length);
			expected = fm_ext[i].fe_physical + fm_ext[i].fe_length;
			if (fm_ext[i].fe_flags & FIEMAP_EXTENT_LAST)
				last = 1;
			if (!physical ||
			    !(fm_ext[i].fe_flags & FIEMAP_EXTENT_MORESTRIPE)) {
				n++;
			}
			if (physical &&
			    !(fm_ext[i].fe_flags & FIEMAP_EXTENT_MORESTRIPE))
		...
		fiemap->fm_start = next_start;
	} while (last == 0);

	*num_extents = physical ? n : tot_extents;
...
int main(int argc, char**argv)
{
	...
	while ((c = getopt(argc, argv, "Bb::ekPsvxX")) != EOF)
		switch (c) {
		case 'B':
			force_bmap++;
			break;
