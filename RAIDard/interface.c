	      ...
	      switch(iface[i].sa.sa_family) {
		     ...
		     if (bind(iface_fd[i], &(iface[i].sa), sizeof(struct sockaddr_in)) ==-1){
			     perror("bind");
                     ...
                     if(raidard_mode&MODE_BROADCAST){
                        ...
                        if(setsockopt(fd_out, IPPROTO_IP, 
                                                IP_ADD_MEMBERSHIP,
                                                (void *) &mreq, 
                                                sizeof(mreq))<0){
                        ...
                        if(setsockopt(iface_fd[i], IPPROTO_IP, 
                                                IP_ADD_MEMBERSHIP,
                                                (void *) &mreq, 
                                                sizeof(mreq))<0){
		     ...
		     if (bind(iface_fd[i], &(iface[i].sa), sizeof(struct sockaddr_in6)) ==-1){
			    perror("bind6:");
                     ...
                     if ( setsockopt(iface_fd[i], IPPROTO_IPV6, IPV6_ADD_MEMBERSHIP, &mreq6, sizeof(mreq6)) != 0)
                     {
...
int interfaces_get()
{
	...
	extern unsigned short raidard_mode;
        ...
        if(raidard_verbose) {
	            ...
	            for (i=0;i< iface_count;i++) {
		           printf("hostname=%s ", iface[i].host_name);
