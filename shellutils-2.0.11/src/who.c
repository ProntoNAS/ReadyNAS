...
/* return the strlen of the largest decimal string for the given
   integer sizeof */
int
decstrlen(int size)
{
  /* This is really (int)(ceil(log10(pow(2,x*8)))) but do we really
     want to add -lm just for this? Let's cop out and assume some
     standard int sizes. We'll also add 1 for a potential -. */
  ...
  switch (size)
    ...
    case 8:
      return 20;
    default:
      error (0, 0, "unknown integer length %d\n", size);
      exit (-1);
    }
}
...
#define DEV_DIR_LEN (sizeof (DEV_DIR_WITH_TRAILING_SLASH) - 1)
  ...
  if (!pidstr)
    pidstr = (char*) malloc (decstrlen(sizeof(utmp_ent->ut_pid))+1);
  sprintf (pidstr, "%d", utmp_ent->ut_pid);
...
print_boottime (const STRUCT_UTMP *utmp_ent)
{
  ...
  print_line ("", ' ', "system boot", time_string(UT_TIME_MEMBER(utmp_ent)),
	      "", "", "", "");
}
...
{
  static char *comment, *exitstr, *pidstr;

  if (!comment)
    comment = (char*)malloc(sizeof(_("id="))+sizeof(utmp_ent->ut_id)+1);
  sprintf (comment, "%s%.*s", _("id="),
	   sizeof utmp_ent->ut_id, utmp_ent->ut_id);

  if (!exitstr)
    exitstr = (char*)malloc(sizeof(_("term="))+
			    decstrlen(sizeof(utmp_ent->ut_exit.e_termination))+
			    1+
			    sizeof(_("exit="))+
			    decstrlen(sizeof(utmp_ent->ut_exit.e_exit))+
			    1);
  sprintf (exitstr, "%s%d %s%d",
	   _("term="),
...
print_runlevel (const STRUCT_UTMP *utmp_ent)
{
  ...
  /* FIXME: The following is correct for linux, may need help on other platforms */
  ...
  print_line ("", ' ', runlevline, time_string(UT_TIME_MEMBER(utmp_ent)),
	      "", "", comment, "");
...
list_entries_who (int n, const STRUCT_UTMP *utmp_buf)
{
  ...
  while (n--)
    {
      if (UT_USER (utmp_buf)[0] && utmp_buf->ut_type == USER_PROCESS)
	{
	  char *trimmed_name;
  ...
  while (n--)
	  ...
	  strncmp(ttyname_b, utmp_buf->ut_line, sizeof(utmp_buf->ut_line)) == 0)
	{
	  if (need_users && UT_USER (utmp_buf)[0]
	    ...
	    print_user (utmp_buf);
	  else if (need_runlevel && utmp_buf->ut_type == RUN_LVL)
	    print_runlevel (utmp_buf);
	  else if (need_boottime && utmp_buf->ut_type == BOOT_TIME)
	  ...
	  else if (need_clockchange && utmp_buf->ut_type == NEW_TIME)
	    print_clockchange (utmp_buf);
	  else if (need_initspawn && utmp_buf->ut_type == INIT_PROCESS)
	    print_initspawn (utmp_buf);
	  else if (need_login && utmp_buf->ut_type == LOGIN_PROCESS)
	    print_login (utmp_buf);
	  else if (need_deadprocs && utmp_buf->ut_type == DEAD_PROCESS)
...
main (int argc, char **argv)
{
  ...
  while ((optc = getopt_long (argc, argv, "abdilmpqrstuwHT", longopts, &longind))
	 != -1)
    {
      switch (optc)
	{
	...
	case 'i':
	  error (0, 0, _("Warning: -i will be removed in a future release; please use -u\n"));
	  /* Fall through.  */
	case 'u':
	  need_users = 1;
	  include_idle = 1;
	...
	case 'l':
	  error (0, 0, _("Warning: the meaning of '-l' will change in a future release to conform to POSIX\n"));
	case LOOKUP_OPTION:
	  do_lookup = 1;
	  break;
