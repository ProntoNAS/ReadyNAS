...
account_t *account_copy(account_t *acc)
{
	...
	a->port = acc->port;
	a->connect_timeout = acc->connect_timeout;
...
int get_pos_int(const char *s)
{
    ...
    x = strtol(s, &p, 0);
    if (p == s || x <= 0 || x == LONG_MAX || x > INT_MAX)
    {
   	x = -1;
    }
    else if (*p != '\0')
...
void override_account(account_t *acc1, account_t *acc2)
{
    ...
    if (acc2->mask & ACC_CONNECT_TIMEOUT)
    {
	acc1->connect_timeout = acc2->connect_timeout;
    }
...
int check_account(account_t *acc, char *errstr)
{
    if (!acc->host)
    {
	snprintf(errstr, errstr_bufsize, "no host information");
	return CONF_ESYNTAX;
    }
    if (acc->tls_key_file && !acc->tls_cert_file) 
    {
	snprintf(errstr, errstr_bufsize, 
		"TLS key file requires TLS cert file");
	return CONF_ESYNTAX;
    }
    if (!acc->tls_key_file && acc->tls_cert_file)
    {
	snprintf(errstr, errstr_bufsize, 
		"TLS cert file requires TLS key file");
	return CONF_ESYNTAX;
    }
    if (acc->tls_nocertcheck && acc->tls_trust_file)
    {
	snprintf(errstr, errstr_bufsize, 
		"cannot use TLS trust file with TLS cert check turned off");
	return CONF_ESYNTAX;
    }
	...
	char *errstr)
{
    ...
    *arg = NULL;
	...
	if (ferror(f))
	{
	    snprintf(errstr, errstr_bufsize, "input error");
	    return CONF_EIO;
	}
	else /* EOF */
	{
	...
	char *errstr)
{
	...
	if (*acc_id == '\0')
	{
	    free(acc_id);
	    snprintf(errstr, errstr_bufsize,
		    "line %d: missing account name", line);
	    return CONF_ESYNTAX;
	}
	if (!(acc = find_account(acc_list, acc_id)))
	{
	    snprintf(errstr, errstr_bufsize,
		    "line %d: account %s not (yet) defined", line, acc_id);
	    free(acc_id);
	    return CONF_ESYNTAX;
	}
	free(acc_id);
	...
	char *errstr)
{
    ...
    *acc_list = list_new();
	...
	if (strcmp(cmd, "defaults") == 0)
	{
	    ...
	    {
		snprintf(errstr, errstr_bufsize, 
			"line %d: command %s does not take an argument", 
			line, cmd);
		e = CONF_ESYNTAX;
		break;
	    }
	...
	else if (strcmp(cmd, "account") == 0)
	{
		...
		*t = '\0';
		...
		list_free(copy_from);
		snprintf(errstr, errstr_bufsize,
			"line %d: an account name must not contain "
 			"colons or commas", line);
		e = CONF_ESYNTAX;
		break;
	    }
		...
		list_free(copy_from);
		snprintf(errstr, errstr_bufsize,
			"line %d: account %s was already defined in this file",
			line, arg);
		e = CONF_ESYNTAX;
		break;
	    }
	...
	else if (strcmp(cmd, "port") == 0)
	{
		...
		{
		    snprintf(errstr, errstr_bufsize,
 			    "line %d: invalid argument %s for command %s",
			    line, arg, cmd);
		    e = CONF_ESYNTAX;
		    break;
		}
	...
	else if (strcmp(cmd, "connect_timeout") == 0)
 	{
	    acc->mask |= ACC_CONNECT_TIMEOUT;
	    if (*arg == '\0')
	    {
		snprintf(errstr, errstr_bufsize,
			"line %d: command %s needs an argument", line, cmd);
		...
		if (is_off(arg))
		{
		    acc->connect_timeout = 0;
		}
		else
		{
		    acc->connect_timeout = get_pos_int(arg);
		    if (acc->connect_timeout < 1)
		    {
			snprintf(errstr, errstr_bufsize,
				"line %d: invalid argument %s for command %s",
				line, arg, cmd);
	...
	else if (strcmp(cmd, "from") == 0)
	{
	    acc->mask |= ACC_FROM;
	    free(acc->from);
	    acc->from = (*arg == '\0') ? NULL : xstrdup(arg);
	}
	else if (strcmp(cmd, "maildomain") == 0)
	{
	    acc->mask |= ACC_MAILDOMAIN;
	    free(acc->maildomain);
	    acc->maildomain = (*arg == '\0') ? NULL : xstrdup(arg);
	}
	...
	else if (strcmp(cmd, "tls_key_file") == 0)
	{
	    ...
	    {
		if (expand_tilde(arg, &(acc->tls_key_file), errstr) 
			!= PATHS_EOK)
		{
		    e = CONF_ESYNTAX;
		    break;
	...
	else if (strcmp(cmd, "tls_cert_file") == 0)
	{
	    ...
	    {
		if (expand_tilde(arg, &(acc->tls_cert_file), errstr) 
			!= PATHS_EOK)
		{
		    e = CONF_ESYNTAX;
		    break;
	...
	else if (strcmp(cmd, "tls_trust_file") == 0)
	{
	    ...
	    {
		if (expand_tilde(arg, &(acc->tls_trust_file), errstr) 
			!= PATHS_EOK)
		{
		    e = CONF_ESYNTAX;
		    break;
	...
	else if (strcmp(cmd, "logfile") == 0)
	{
	    ...
	    {
		if (expand_tilde(arg, &(acc->logfile), errstr) != PATHS_EOK)
		{
		    e = CONF_ESYNTAX;
		    break;
		}
	...
	char *errstr)
{
    ...
    if (securitycheck)
    {
	/* because we do the check after we opened the file, we know that it
	 * exists, so we can treat all error codes returned by check_secure() as
	 * PATHS_EINSECURE. */
	if (check_secure(conffile, errstr) != PATHS_EOK)
	{
	    fclose(f);
	    return CONF_EINSECURE;
	}
    }
