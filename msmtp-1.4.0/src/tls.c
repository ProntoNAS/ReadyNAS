...
int seed_prng(char *errstr)
{
	...
	{
	    snprintf(errstr, errstr_bufsize, "error reading %s", randfile);
	    return TLS_ESEED;
	}
	...
	if ((t = time(NULL)) < 0)
	{
	    snprintf(errstr, errstr_bufsize, "cannot get system time: %s",
	    	    strerror(errno));
	    return TLS_ESEED;
	}
	...
	if (!RAND_status())
	{
	    snprintf(errstr, errstr_bufsize, "random file + time + pseudo "
		    "randomness is not enough, giving up");
	    return TLS_ESEED;
	}
...
int tls_lib_init(char *errstr)
{
    ...
    {
	snprintf(errstr, errstr_bufsize, "%s", 
...
int tls_cert_info_get(tls_t *tls, tls_cert_info_t *tci, char *errstr)
{
    ...
    int e;
    char *p;
    const char *errmsg = "cannot get TLS certificate info";
    ...
    {
	snprintf(errstr, errstr_bufsize, 
		"%s: no certificate was found", errmsg);
	return TLS_ECERT;
    }
    if (gnutls_x509_crt_init(&cert) != 0)
    {
	snprintf(errstr, errstr_bufsize, 
		"%s: cannot initialize certificate structure", errmsg);
	return TLS_ECERT;
    }
    if (gnutls_x509_crt_import(cert, &cert_list[0], GNUTLS_X509_FMT_DER) != 0)
    {
	snprintf(errstr, errstr_bufsize, 
		"%s: error parsing certificate", errmsg);
	return TLS_ECERT;
    }
    ...
    {
	snprintf(errstr, errstr_bufsize,
		"%s: error getting SHA1 fingerprint", errmsg);
	return TLS_ECERT;
    }
    size = 16;
	...
	X509_free(x509cert);
	snprintf(errstr, errstr_bufsize, 
		"%s: cannot get certificate subject", errmsg);
	return TLS_ECERT;
    }
	...
	X509_free(x509cert);
	snprintf(errstr, errstr_bufsize, 
		"%s: cannot get certificate issuer", errmsg);
	return TLS_ECERT;
    }
...
int tls_check_cert(tls_t *tls, const char *hostname, int verify, char *errstr)
{
    ...
    {
	snprintf(errstr, errstr_bufsize,
		"%s: %s", error_msg, gnutls_strerror(error_code));
	return TLS_ECERT;
    }
    if (verify)
    {
	...
	{
	    snprintf(errstr, errstr_bufsize, 
	    	    "%s: the certificate hasn't got a known issuer", error_msg);
	    return TLS_ECERT;
	}
    ...
    {
	snprintf(errstr, errstr_bufsize, 
		"%s: no certificate was found", error_msg);
	return TLS_ECERT;
    }
	...
	{
    	    snprintf(errstr, errstr_bufsize, 
		    "%s: the certificate owner does not match hostname %s", 
		    error_msg, hostname);
    	    return TLS_ECERT; 
	}
	...
	{
	    snprintf(errstr, errstr_bufsize,
		    "%s: certificate %u of %u has expired", 
		    error_msg, i + 1, cert_list_size);
	    return TLS_ECERT;
	}
    ...
    {
	snprintf(errstr, errstr_bufsize, 
		"%s: no certificate was sent", error_msg);
	return TLS_ECERT;
    }
	...
	const char *trust_file, char *errstr)
{
	...
	snprintf(errstr, errstr_bufsize, 
		"cannot set priorities on TLS Session: %s",
		gnutls_strerror(error_code));
	return TLS_ELIBFAILED;
    }
	...
	gnutls_deinit(tls->session);
	snprintf(errstr, errstr_bufsize, 
		"cannot allocate certificate for TLS Session: %s",
		gnutls_strerror(error_code));
	return TLS_ELIBFAILED;
    }
		       	...
		       	cert_file, key_file, GNUTLS_X509_FMT_PEM)) < 0)
	{
	    ...
	    snprintf(errstr, errstr_bufsize, 
		    "cannot set X509 key file %s and/or "
    ...
    {
	snprintf(errstr, errstr_bufsize, "cannot set TLS method");
	return TLS_ELIBFAILED;
    }
    tls->ssl_ctx = SSL_CTX_new(ssl_method);
    if (!tls->ssl_ctx)
    {
	snprintf(errstr, errstr_bufsize, "cannot create TLS context: %s",
	...
	if (SSL_CTX_use_certificate_chain_file(tls->ssl_ctx, cert_file) != 1)
	{
	    ...
	    tls->ssl_ctx = NULL;
	    snprintf(errstr, errstr_bufsize,
		    "cannot load certificate file %s: %s", 
	...
	tls->ssl_ctx = NULL;
	snprintf(errstr, errstr_bufsize, 
		"cannot create a TLS structure: %s", 
		ERR_error_string(ERR_get_error(), NULL));
	return TLS_ELIBFAILED;
    }
    return TLS_EOK;
 ...
 * The resulting string will be returned in 'errstr'.
 * OpenSSL error strings are max 120 characters long according to
 * ERR_error_string(3). 
 */

#ifdef HAVE_OPENSSL
void openssl_io_error(int error_code, int error_code2, 
	const char *error_string, char *errstr)
{
	...
	tls_cert_info_t *tci, char *errstr)
{
	...
	SSL_CTX_free(tls->ssl_ctx);
	snprintf(errstr, errstr_bufsize,
		"cannot set the file descriptor for TLS: %s", 
		ERR_error_string(ERR_get_error(), NULL));
	return TLS_ELIBFAILED;
    }
	...
	SSL_free(tls->ssl);
	SSL_CTX_free(tls->ssl_ctx);
	openssl_io_error(error_code, SSL_get_error(tls->ssl, error_code),
		"TLS handshake failed", errstr);
	return TLS_EIO;
    }
    if (tci)
    {
...
int tls_getchar(tls_t *tls, char *c, int *eof, char *errstr)
{
	...
	*eof = 1;
    ...
    {
	snprintf(errstr, errstr_bufsize,
		"cannot read from TLS connection: %s", 
		gnutls_strerror(ret));
	return TLS_EIO;
    }
	...
	{
	    openssl_io_error(error_code, error_code2, 
		    "cannot read from TLS connection", errstr);
	    return TLS_EIO;
	}
    }
    else
...
int tls_puts(tls_t *tls, const char *s, size_t len, char *errstr)
{
    ...
    {
	snprintf(errstr, errstr_bufsize,
		"cannot write to TLS connection: %s", 
		gnutls_strerror(ret));
	return TLS_EIO;
    }
    ...
    {
	snprintf(errstr, errstr_bufsize,
		"cannot write to TLS connection: unknown error");
	return TLS_EIO;
    }    
    ...
    {
	openssl_io_error(error_code, SSL_get_error(tls->ssl, error_code),
		    "cannot write to TLS connection", errstr);
	return TLS_EIO;
    }
