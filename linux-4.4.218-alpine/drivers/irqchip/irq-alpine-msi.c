...
static int al_msix_allocate_sgi(struct alpine_msix_data *mdata, int num_req)
{
	...
	pos = find_first_zero_bit(mdata->al_msix_use_map, mdata->al_msix_num_irqs);
	if (pos >= mdata->al_msix_num_irqs)
		return -ENOSPC;
	...
	if (avail != num_req)
		return -ENOSPC;
	return mdata->al_msix_sgi_first + pos;
}
...
int alpine_msix_init(struct device_node *node, struct device_node *parent)
{
	...
	if (of_irq_parse_one(node, 0, &first) || of_irq_parse_one(node, 1, &last)){
		pr_err("alpine-msix: couldn't parse irq!");
		return -EINVAL;
	}
	mdata->al_msix_num_irqs = last.args[1] - first.args[1];
	mdata->al_msix_sgi_first = first.args[1];
	...
	mdata->msi_domain = pci_msi_create_irq_domain(of_node_to_fwnode(node),
						      &alpine_msix_domain_info,
						      mdata->middle_domain);
	BUG_ON(!mdata->msi_domain);
