...
#define RN20X_PHY_POWER_GPIO 469
...
al_mdio_read(struct mii_bus *bp, int mii_id, int reg)
{
	...
	if (rc)
	{
		...
		if (!(adapter->flags & AL_ETH_FLAG_PHY_RESET_TRIED))
		{
			gpio_direction_output(RN20X_PHY_POWER_GPIO, 1);
			msleep(3000);
			gpio_direction_output(RN20X_PHY_POWER_GPIO, 0);
			adapter->flags |= AL_ETH_FLAG_PHY_RESET_TRIED;
			return al_mdio_read(bp, mii_id, reg);
		}
	}
...
al_eth_rx_poll(struct napi_struct *napi, int budget)
{
	...
	do {
		...
		if (likely(adapter->netdev->features & NETIF_F_RXHASH)) {
			enum pkt_hash_types type = PKT_HASH_TYPE_L3; //TODO: is it right?
			if (likely((hal_pkt->l4_proto_idx == AL_ETH_PROTO_ID_TCP) ||
				   (hal_pkt->l4_proto_idx == AL_ETH_PROTO_ID_UDP)))
				type = PKT_HASH_TYPE_L4;
			skb_set_hash(skb, hal_pkt->rxhash, type);
		}
...
{
	struct al_eth_adapter *adapter =
			container_of(work, struct al_eth_adapter, reset_task);
