					  ...
					  struct btrfs_bio *bbio)
{
	...
	ret = radix_tree_insert(&dev->reada_zones,
			(unsigned long long)(zone->end >> PAGE_CACHE_SHIFT),
			zone);
					      ...
					      struct btrfs_key *top)
{
	...
	int real_stripes;
	int nzones = 0;
	unsigned long long index = logical >> PAGE_CACHE_SHIFT;
...
{
	int i;
	unsigned long long index = re->logical >> PAGE_CACHE_SHIFT;
...
{
	int i;
	unsigned long long index = zone->end >> PAGE_CACHE_SHIFT;
