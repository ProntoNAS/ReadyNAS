...
static int btrfs_check_super_csum(struct btrfs_fs_info *fs_info,
				  char *raw_disk_sb, unsigned long long start,
				      ...
				      struct btrfs_key *location)
{
	...
	if (root_flags & BTRFS_SUBVOL_CASEINSENSITIVE) {
...
static void btrfs_init_balance(struct btrfs_fs_info *fs_info)
{
	...
	mutex_init(&fs_info->sweep_mutex);
	       ...
	       char *options)
{
	...
	fs_info->sweep_ctl = kmalloc(sizeof(struct btrfs_sweep_ctl),
					GFP_KERNEL);
	if (!fs_info->sweep_ctl) {
		err = -ENOMEM;
		goto fail_iput;
	}
	memset(fs_info->sweep_ctl, 0, sizeof(*(fs_info->sweep_ctl)));
	 ...
	 */
	if (btrfs_check_super_csum(fs_info, bh->b_data, super_bytenr,
