...
void btrfs_get_delayed_items_case(struct inode *inode, struct list_head *ins_list,
			     struct list_head *del_list)
{
	struct btrfs_delayed_node *delayed_node;
	struct btrfs_delayed_item *item;
	...
	while (item) {
		atomic_inc(&item->refs);
		list_add_tail(&item->case_list, ins_list);
		item = __btrfs_next_delayed_item(item);
	}
	...
	while (item) {
		atomic_inc(&item->refs);
		list_add_tail(&item->case_list, del_list);
		item = __btrfs_next_delayed_item(item);
	}
	mutex_unlock(&delayed_node->mutex);
	/*
...
void btrfs_put_delayed_items_case(struct list_head *ins_list,
			     struct list_head *del_list)
{
	struct btrfs_delayed_item *curr, *next;

	list_for_each_entry_safe(curr, next, ins_list, case_list) {
		list_del(&curr->case_list);
		if (atomic_dec_and_test(&curr->refs))
			kfree(curr);
	}

	list_for_each_entry_safe(curr, next, del_list, case_list) {
		list_del(&curr->case_list);
		if (atomic_dec_and_test(&curr->refs))
			kfree(curr);
	}
...
int btrfs_should_delete_dir_index_case(struct list_head *del_list,
				  u64 index)
{
	struct btrfs_delayed_item *curr, *next;
	int ret;
...
int btrfs_readdir_delayed_dir_index_case(struct dir_context *ctx,
				    struct list_head *ins_list)
{
	struct btrfs_dir_item *di;
	struct btrfs_delayed_item *curr, *next;
