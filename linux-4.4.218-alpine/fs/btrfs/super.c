...
enum {
	...
	Opt_datasum, Opt_treelog, Opt_noinode_cache, Opt_noquota,
...
int btrfs_parse_options(struct btrfs_root *root, char *options)
{
	...
	while ((p = strsep(&options, ",")) != NULL) {
		...
		case Opt_clear_cache:
			...
			break;
		case Opt_noquota:
			btrfs_set_and_info(root, NOQUOTA,
					   "disabling quotas");
			break;
...
static int btrfs_statfs(struct dentry *dentry, struct kstatfs *buf)
{
	...
	int mixed = 0;
	u64 max_rfer, rfer;
	bool use_qgroup = false;

	if ((dentry->d_flags & DCACHE_NFSFS_STATFS) &&
	    test_bit(BTRFS_FS_QUOTA_ENABLED, &fs_info->flags) &&
	    strcmp(dentry->d_name.name, "/")) {
		ret = btrfs_inode_by_name(d_inode(dentry->d_parent),
					  dentry, &location);
		if (!ret && location.type == BTRFS_ROOT_ITEM_KEY) {
			ret = btrfs_get_qgroup_info(fs_info,
						    location.objectid,
						    &max_rfer, &rfer);
			if (!ret)
				use_qgroup = true;
	...
	if (use_qgroup) {
		buf->f_blocks = (max_rfer >>= bits);
		buf->f_bfree = buf->f_blocks - (rfer >> bits);
