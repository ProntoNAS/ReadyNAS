...
struct btrfs_fs_info {
	...
	struct mutex sweep_mutex;
	struct btrfs_sweep_ctl *sweep_ctl;
	void *bdev_holder;
...
#define CASEINSENSITIVE_CHECK(root)  \
        ((root == NULL) || ((root)->fs_info == NULL) || \
	((root)->fs_info->strcase_ops == NULL))
#define CASEINSENSITIVE_OPS(root)	((root)->fs_info->strcase_ops)
#define CASEINSENSITIVE_R(root)	\
	(!!(btrfs_root_flags(&(root)->root_item) &	\
		BTRFS_SUBVOL_CASEINSENSITIVE))
#else
#define CASEINSENSITIVE_CHECK(root)     (0)
#define CASEINSENSITIVE_R(root)		(0)
#endif
};
...
#define BTRFS_MOUNT_NOQUOTA		(1 << 26)
...
#ifdef CONFIG_BTRFS_FNAME_CASEINSENSITIVE
int __btrfs_find_name_in_ext_backref(struct btrfs_root *root,
				   struct btrfs_path *path,
				   u64 ref_objectid, const char *name,
				   int name_len,
				   struct btrfs_inode_extref **extref_ret);
#define btrfs_find_name_in_ext_backref(	\
			path, ref_objectid, name, name_len, extref_ret)	\
	__btrfs_find_name_in_ext_backref(root,	\
		(path), (ref_objectid), (name), (name_len), (extref_ret))
#else
