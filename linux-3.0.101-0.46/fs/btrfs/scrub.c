...
#define __BTRFS_CSLINK(cond, exp, comp, size, sb, offset, blklen)	\
	__btrfs_cslink(cond, (exp), (comp), (size),	\
			btrfs_csum_final,	\
					 ...
					 u16 csum_size)
{
	...
	__BTRFS_CSLINK(sblock->checksum_error,
			csum, calculated_csum, csum_size,
...
static int scrub_checksum_data(struct scrub_block *sblock)
{
	...
	__MDCS(on_disk_csum, csum, sctx->csum_size, sctx->sectorsize,
		sblock->pagev[0]->physical,
		sblock->pagev[0]->dev->bdev->bd_dev,
		scrub_get_inum(sblock));
	if (memcmp(csum, on_disk_csum, sctx->csum_size))
		fail = 1;
	__BTRFS_CSLINK(fail, on_disk_csum, csum, sctx->csum_size,
				sblock, 0, sctx->sectorsize);
...
static int scrub_checksum_tree_block(struct scrub_block *sblock)
{
	...
	__MDCS(on_disk_csum, calculated_csum, sctx->csum_size,
		sctx->nodesize - BTRFS_CSUM_SIZE,
		sblock->pagev[0]->physical + BTRFS_CSUM_SIZE,
		sblock->pagev[0]->dev->bdev->bd_dev,
		scrub_get_inum(sblock));
	if (memcmp(calculated_csum, on_disk_csum, sctx->csum_size))
		++crc_fail;
	__BTRFS_CSLINK(crc_fail, on_disk_csum, calculated_csum, sctx->csum_size,
			sblock, BTRFS_CSUM_SIZE, sctx->nodesize);
