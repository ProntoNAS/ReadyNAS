...
u64 btrfs_log2phys(struct inode *inode, u64 logical, u64 len, dev_t *dev)
{
	...
	if (!em) {
		printk(KERN_ERR "%s: cannot lookup extent mapping for %llu\n",
				__func__, logical);
		goto err;
	}
	...
	if (map->type & (BTRFS_BLOCK_GROUP_RAID1|BTRFS_BLOCK_GROUP_RAID10)) {
		printk(KERN_ERR "%s: not supporting RAID1/10", __func__);
		goto err;
	}
...
static u64 fpos2phys(struct inode *inode, u64 fpos, u64 len, dev_t *dev)
{
	...
	if (IS_ERR(em) || !em) {
		printk(KERN_ERR "%s: cannot get extent mpping for %llu\n",
				__func__, fpos);
		return (u64)~0;
	}
...
/* Keep this as a macro as __btrfs_cslink() uses the __func__ macro. */
...
{	\
	...
	\
	if (physical != (u64)~0)	\
		__btrfs_cslink((cond), (expected), (computed), (csumlen), \
			(csfunc), (physical), (length),	\
			dev, BTRFS_I(inode)->root->anon_super.s_dev, \
			btrfs_ino((inode)), (fpos), 0);	\
}
#else
#define __btrfs_cslink_inode(cond, ...)		do {} while (0)
#endif
			       ...
			       struct extent_state *state, int mirror)
{
	...
	__MDCS((void *)&private, (void *)&csum, 4, end - start + 1,
		start, root->anon_super.s_dev, btrfs_ino(inode));
	__btrfs_cslink_inode(csum != private,
			(u8 *)&private, (u8 *)&csum, sizeof csum,
			btrfs_csum_final,
			start, end - start + 1,
			inode);
	if (csum != private)
	...
	while (1) {
		...
		while (di_cur < di_total) {
			 ...
			 * skip it
                         ...
                         * compatibility.
			 */
			/* Also skip the snapper '.snapshots' subvolume */
			if (location.type == BTRFS_ROOT_ITEM_KEY &&
			    (location.objectid == root->root_key.objectid ||
			     strncmp(name_ptr, ".snapshots", name_len) == 0)) {
				over = 0;
				goto skip;
			}
...
static void btrfs_endio_direct_read(struct bio *bio, int err)
{
	...
	do {
		if (!(BTRFS_I(inode)->flags & BTRFS_INODE_NODATASUM)) {
			...
			__MDCS((void *)&private, (void *)&csum, 4,
				bvec->bv_len, start, root->anon_super.s_dev,
				btrfs_ino(inode));
			__btrfs_cslink_inode(csum != private,
				(u8 *)&private, (u8 *)&csum, sizeof csum,
				btrfs_csum_final,
...
{
	struct btrfs_root *root = BTRFS_I((struct inode *)inode)->root;
...
{
	struct btrfs_root *root = BTRFS_I((struct inode *)pinode)->root;
