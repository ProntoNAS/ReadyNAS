...
#define COMMON_MPOOL_CHECK_ALIGNED4(ptr) (!(((uint32_t)(ptr)) & 0x00000003))
					   ...
					   common_mpool_ftr_t * ftr)
{
	...
	if (hdr->magic != COMMON_MPOOL_HDR_MAGIC) {
		printk(KERN_ERR "illegal hdr magic %x for ptr %p",
			hdr->magic,ptr);
		return false;
	}

	if (ftr->magic != COMMON_MPOOL_FTR_MAGIC) {
		printk(KERN_ERR "illegal ftr magic %x for ptr %p",
			ftr->magic,ptr);
		return false;
	}

	if (hdr->pool != pool || ftr->pool != pool) {
		printk(KERN_ERR "inconsistent size hdr->pool: "
			"%p ftr->pool: %p for ptr %p", hdr->pool,ftr->pool,ptr);
		return false;
	}
						...
						uint32_t entry_size)
{
	...
	uint32_t pool_entry_size =
			COMMON_MPOOL_ALIGN4(sizeof(common_mpool_hdr_t) +
			aligned_entry_size + sizeof(common_mpool_ftr_t));
	common_mpool_hdr_t* hdr;
...
int32_t common_mempool_get_number_of_free_entries(common_mempool_t* pool)
{
	return pool ? (int32_t)pool->number_of_free_entries : -1;
}

int32_t common_mempool_get_number_of_entries(common_mempool_t* pool)
{
	return pool ? (int32_t)pool->number_of_entries : -1;
}

int32_t common_mempool_get_entry_size(common_mempool_t* pool)
{
	return pool ? (int32_t)pool->entry_size : -1;
}
...
common_mempool_t* common_mempool_get(void* ptr)
{
	...
	if (!ptr || !COMMON_MPOOL_CHECK_ALIGNED4(ptr))
		return NULL;
	...
	if (hdr->pool != ftr->pool || !hdr->pool) {
		printk(KERN_ERR
		"inconsistent size hdr->pool: %p ftr->pool: %p for ptr %p",
			hdr->pool,ftr->pool,ptr);
		return false;
	}
	return hdr->pool;
}
...
/*
 * This is a hacky and imcomplete implementation.
 * The caller (i.e. user process calling splice) must know how many bytes
 * will be transferred from socket. If fewer bytes, it is considered an
 * error and the destination file will not be updated.
 */
				     ...
				     loff_t __user *ppos, size_t count)
{
	...
	BUG_ON(!mapping->a_ops->write_begin || !mapping->a_ops->write_end);
	...
	if (count > MAX_PAGES_PER_RECVFILE * PAGE_SIZE) {
		printk("%s: count(%lu) exceeds maxinum\n",
	...
	if (!rv_cb || !iov) {
		printk(KERN_ERR "%s: Failed to get pool mem for %d pages "
		"(rv_cb %p iov %p)\n", __func__, page_cnt_est, rv_cb, iov);
		err = -ENOMEM;
		goto done_pool;
	}
	...
	count_tmp = count;
	pos_tmp = pos;
	for (nr_pages = 0; count_tmp > 0; nr_pages++) {
		unsigned long bytes;	/* Bytes to write to page */
		unsigned long offset;	/* Offset into pagecache page */
		struct page *pageP;
		void *fsdata;

		offset = pos_tmp & (PAGE_CACHE_SIZE - 1);
		bytes = PAGE_CACHE_SIZE - offset;
		if (bytes > count_tmp)
			bytes = count_tmp;
		ret = mapping->a_ops->write_begin(file, mapping, pos_tmp, bytes,
	...
	for (inx = 0; inx < nr_pages; inx++) {
		size_t copied = count_tmp;
		struct page *saver_page = NULL;
		char *saver_buf;

		if (copied > rv_cb[inx].rv_count)
			copied = rv_cb[inx].rv_count;
		count_tmp -= copied;

		/* For a short page write, block_write_end() called in generic
		 * ->write_end() will zero the entire page. Save the valid
		 * transferred content for a later recovery.
		 */
		if (copied && copied < rv_cb[inx].rv_count &&
			(saver_page = alloc_page(GFP_KERNEL|GFP_NOFS))) {
			saver_buf = kmap(saver_page);
			memcpy(saver_buf,
				kmap(rv_cb[inx].rv_page) +
				(rv_cb[inx].rv_pos & (PAGE_CACHE_SIZE-1)),
				copied);
			kunmap(rv_cb[inx].rv_page);
		}
		kunmap(rv_cb[inx].rv_page);
		...
		if (unlikely(ret < 0)) {
			...
			if (saver_page) {
				__free_page(saver_page);
				saver_page = NULL;
			}
		}

		/* If we had a short write for the last valid page, redo it
		 * from saver.
		 */
		if (saver_page) {
			struct page *page;
			...
			ret = mapping->a_ops->write_begin(file, mapping,
						rv_cb[inx].rv_pos, copied,
						AOP_FLAG_UNINTERRUPTIBLE,
						&page, &fsdata);
			if (unlikely(ret)) {
				__free_page(saver_page);
				saver_page = NULL;
				err = ret;
				/* This page failed to begin. Start
				 * cleaning from next page.
				 */
				inx++;
				goto cleanup;
			}
	...
	balance_dirty_pages_ratelimited_nr(mapping, nr_pages);
	file->f_pos = pos;
	if (ppos && copy_to_user(ppos, &pos, sizeof(loff_t)))
		err = -EFAULT;
done_trunc:
	/* Adjusting file size to the last known good value is a
	 * Quick-n-Dirty(TM) work-around for fixing block_write_end() called
	 * in generic ->write_end() from extending file size beyond valid EOF
	 * by writing a zero-length countent for a short write (i.e. received
	 * data size is less than write_begin() reservation).
	 */
	truncate_setsize(inode, i_size);
done_pool:
...
cleanup:
	for (; inx < nr_pages; inx++) {
		kunmap(rv_cb[inx].rv_page);
		mapping->a_ops->write_end(file, mapping,
						rv_cb[inx].rv_pos,
						rv_cb[inx].rv_count,
						rv_cb[inx].rv_count,
						rv_cb[inx].rv_page,
...
retry:
	if ((ret = ipipe_prep(ipipe, flags)))
		return ret;

	if ((ret = opipe_prep(opipe, flags)))
		return ret;
