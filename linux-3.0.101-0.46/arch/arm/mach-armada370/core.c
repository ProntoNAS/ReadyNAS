...
#include "../include/asm/mach/marvell.h"
...
/* Netgear */
unsigned int netgear_bdtype;
unsigned int get_board_type()
{
	return netgear_bdtype;
}
int bdtype_cmdline_config(char *s);
__setup("bdtype=", bdtype_cmdline_config);

int bdtype_cmdline_config(char *s)
{
	if (0 == strcmp("rn102", s))
		netgear_bdtype = NETGEAR_BD_TYPE_RN102;
	else if (0 == strcmp("rn104", s))
		netgear_bdtype = NETGEAR_BD_TYPE_RN104;
	else if (0 == strcmp("s2000", s))
		netgear_bdtype = NETGEAR_BD_TYPE_S2000;
	else
		netgear_bdtype = NETGEAR_BD_TYPE_UNKNOWN;
	return 1;
}
/* for debug putstr */
static char arr[256];
...
#else
       .freq_m         = 9, /* assumes 200 MHz TCLK */
       .freq_n         = 4,
       .timeout        = 5000, /* Default timeout of 5 second */
#endif
};
...
static struct i2c_board_info __initdata i2c_isl12057 = {
	I2C_BOARD_INFO("isl12057", 0x68),
...
static void __init rtc_init(void)
{
	...
	i2c_register_board_info(0, &i2c_isl12057, 1);
}
...
static struct i2c_board_info __initdata i2c_g760a = {
...
void __init armadaxp_hwmon_init(void)
{
	...
	i2c_register_board_info(0, &i2c_g760a, 1);
}

/*****************************************************************************
 * PCA9554(Rn104 SATA LED CONTROL)
 *****************************************************************************/
static struct pca953x_platform_data pca9554_i2c_data ={
	.gpio_base = 69,
};

static struct i2c_board_info __initdata i2c_pca9554 = {
	I2C_BOARD_INFO("pca9554", 0x23),
	.platform_data = &pca9554_i2c_data,
};

static void __init pca9554_init(void)
{
	i2c_register_board_info(0, &i2c_pca9554, 1);
}
#if 0
/*************
 * 7-Segment *
...
void print_board_info(void)
{
	...
	printk(" BE");
#endif
	printk("\n  LSP version: %s", LSP_VERSION);
	printk("\n  Netgear kernel version: %s", "1.12");
	printk("\n\n");
...
static void __init armadaxp_xor0_init(void)
{
	...
	dma_cap_set(DMA_CRC32C, armadaxp_xor00_data.cap_mask);
...
#define ARMADA_CESA0_BASE       (0x00090000 + 0xd0000000)
#define ARMADA_CESA0_IRQ        48
#define ARMADA_SRAM_PHYS_BASE   0xc8010000
#define ARMADA_SRAM_SIZE        SZ_2K
static void __init armadaxp_crypto_init(void)
{
	mv_crypto_res_init(ARMADA_CESA0_BASE, SZ_64K,
			ARMADA_SRAM_PHYS_BASE, ARMADA_SRAM_SIZE,
			ARMADA_CESA0_IRQ);
}
