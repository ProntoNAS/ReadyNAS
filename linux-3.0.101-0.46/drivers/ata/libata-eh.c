...
static int ata_eh_handle_dev_fail(struct ata_device *dev, int err)
{
	...
	if (ata_dev_enabled(dev) && !ehc->tries[dev->devno]) {
		...
		/* Force SATA Controller wait on GEN2. */
		u32 ctrl;
		if (sata_scr_read(ata_dev_phys_link(dev), SCR_CONTROL, &ctrl) == 0)
		{
			ctrl &= (~0xff);
			if (sata_scr_write(ata_dev_phys_link(dev), SCR_CONTROL, ctrl) == 0)
				mdelay(100);
			else
				printk("@ %s, reset sata port error!(ctrl = 0x%08x)\n", __func__, ctrl);
		}
		else
			printk("@ %s, reset sata port error!(ctrl = 0x%08x)\n", __func__, ctrl);
