...
static ssize_t
show_ata_dev_gpio(struct device *dev,
		  struct device_attribute *attr, char *buf)
{
	struct ata_device *ata_dev = transport_class_to_dev(dev);
	u32 val = 0;
	...
	if (ata_dev->class != ATA_DEV_PMP)
		return 0;
	if ( sata_pmp_read(ata_dev->link, 130, &val)){
		printk(KERN_INFO "read fail from GPIO\n");
		return 0;
	} 
	return sprintf(buf, "0x%08x\n", val);
}
static ssize_t set_ata_dev_gpio(struct device *dev, struct device_attribute *attr,	
			const char *buf, size_t count)
{
	...
	if (ata_dev->class != ATA_DEV_PMP)
		return 0;
	if (sata_pmp_write(ata_dev->link, 130,val)) {
		printk(KERN_INFO "write fail to GPIO\n");
	}
	return count;
}

static DEVICE_ATTR(gpio, S_IWUSR | S_IRUGO, show_ata_dev_gpio, set_ata_dev_gpio);//Dale
...
#define SETUP_GPIO_ATTRIBUTE(field)					\
	SETUP_TEMPLATE(dev_attrs, field, S_IWUSR | S_IRUGO, 1)
