...
static int mv_init_host(struct ata_host *host)
{
	...
	for (hc = 0; hc < n_hc; hc++) {
		...
		/* Tune eSATA PHY signal according to HW's requirement */
		if (get_board_type() & NETGEAR_BD_TYPE_RN104)
		{
			//MVT
			writelfl(0x8a2d, hc_mmio + 0x283c);//Gen2
			writelfl(0xC924, hc_mmio + 0x2834);//Gen1
		}
		if (get_board_type() & NETGEAR_BD_TYPE_RN102)
		{
			//MVT
			writelfl(0x8a2d, hc_mmio + 0x283c);//Gen2
			writelfl(0xC926, hc_mmio + 0x2834);//Gen1
		}
	}
	if (!IS_SOC(hpriv)) {
