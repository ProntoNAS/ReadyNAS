...
/*	Vcore VIN0  AVCC  3VCC  VIN1  VIN3  VIN3  VSB3  VBAT  NULL */
/*  *8    *8    *16    *16   *8    *8    *8    *16   *16       */
	0x20, 0x21, 0x22, 0x23, 0x24, 0x26, 0x26, 0x550, 0x551, 0x552
};
...
#define format_reg(val,Ra,Rb) ((val * 8)* ((Ra) + (Rb))/(Ra))
	...
	switch(nr)
		{
			case 0: //vcore
				...
				break;				
			case 1: //vin0,12V
				return sprintf(buf, "%ld\n",(long)format_reg(val,10000,110000));				
				break;				
			case 4: //vin1 5V
				return sprintf(buf, "%ld\n",(long)format_reg(val,5100,20000));						
				break;				
			case 5: //vin3 P1V5
				return sprintf(buf, "%ld\n",(long)format_reg(val,20000,10000));
				break;
			case 7:
			case 8:
				return sprintf(buf, "%ld\n",(val* 16));
					...
					const char *buf, size_t count)
{
	...
	superio_enter(0x4e);
	superio_select(0x4e, 0x16);
	printk("before set_eup:0x%02x\n",val);
	superio_outb(0x4e, 0x30,val);
	val = superio_inb(0x4e, 0x30);
	printk("after set_eup:0x%02x\n",val);
	superio_exit(0x4e);
	mutex_unlock(&data->update_lock); 
					...
					const char *buf, size_t count)
{
	...
	superio_enter(0x4e);
	superio_select(0x4e, 0x0a);
	printk("before set_wol:0x%02x\n",val);
	superio_outb(0x4e, 0xfe,val);
	val = superio_inb(0x4e, 0xfe);
	printk("after set_wol:0x%02x\n",val);
	superio_exit(0x4e);
	mutex_unlock(&data->update_lock); 
...
static void nct6775_device_remove_files(struct device *dev)
{
	...
	device_remove_file(dev, &dev_attr_eup);
	device_remove_file(dev, &dev_attr_wol);
}
...
static int nct6775_probe(struct platform_device *pdev)
{
	...
	err = device_create_file(dev, &dev_attr_eup);
	err |= device_create_file(dev, &dev_attr_wol);
	if (err)
		goto exit_remove;
