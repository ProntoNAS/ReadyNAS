...
/*======================= change logs ========================*/
/*
 * 10/10/2008 - Weihan Qian
 * 		add manual fan speed contrl support
 *
 *============================================================*/
...
#define W83L786NG_REG_FAN_PRESCALE	0x8F
...
#define DEFAULT_FAN_VAULE	1080000 //1350000
...
FAN_TO_REG(long rpm, int div)
{
	...
	return SENSORS_LIMIT((DEFAULT_FAN_VAULE + rpm * div / 2) / (rpm * div), 1, 254);
}

#define FAN_FROM_REG(val,div)	((val) == 0   ? -1 : \
				((val) == 255 ? 0 : \
				DEFAULT_FAN_VAULE / ((val) * (div))))
				//1350000 / ((val) * (div))))
		 ...
		 const char *buf, size_t count)
{
	...
	if (val > 3)  /* only modes 1 and 2 are supported */
		return -EINVAL;
	...
	reg |= val << W83L786NG_PWM_ENABLE_SHIFT[nr];
	w83l786ng_write_value(client, W83L786NG_REG_FAN_CFG, reg);
	w83l786ng_write_value(client, W83L786NG_REG_FAN_PRESCALE, 0xDD);
	mutex_unlock(&data->update_lock);
	return count;
}
...
w83l786ng_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	w83l786ng_write_value(client, W83L786NG_REG_FAN_DIV, 0x33);
	reg_tmp = w83l786ng_read_value(client, W83L786NG_REG_FAN_DIV);
	data->fan_div[0] = reg_tmp & 0x07;
	data->fan_div[1] = (reg_tmp >> 4) & 0x07;

	/* init fan configure register */
	reg_tmp = w83l786ng_read_value(client, W83L786NG_REG_FAN_CFG);
	reg_tmp &= 0xC3; reg_tmp |= 0xC0;
	w83l786ng_write_value(client, W83L786NG_REG_FAN_CFG, reg_tmp);
	w83l786ng_write_value(client, W83L786NG_REG_PWM[0], 5);
	w83l786ng_write_value(client, W83L786NG_REG_PWM[1], 5);
	...
	if (time_after(jiffies, data->last_updated + HZ + HZ / 2)
		...
		for (i = 0; i < 2; i++) {
			data->pwm_mode[i] =
			    ...
			    ? 1 : 0;
			data->pwm_enable[i] =
			    ((pwmcfg >> W83L786NG_PWM_ENABLE_SHIFT[i]) & 0x3);
			data->pwm[i] = (w83l786ng_read_value(client,
			    W83L786NG_REG_PWM[i])) & 0xF;
		}
//printk(KERN_ERR "JLOG1: %d,%d\n", data->fan[0],  (1 << data->fan_div[0]));
...
//printk(KERN_ERR "JLOG1: 0x%x,0x%x\n", w83l786ng_read_value(client, 0x25), w83l786ng_read_value(client, 0x26));
//printk(KERN_ERR "JLOG2: 0x%x,0x%x\n", w83l786ng_read_value(client, 0x93), w83l786ng_read_value(client, 0x94));
