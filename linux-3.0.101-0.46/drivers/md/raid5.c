...
static void handle_stripe5(struct stripe_head *sh)
{
	...
	for (i=disks; i--; ) {
		...
		//fix bug: the reshape process will be hang up,
		//when we hot-remove the disk from raid5 during reshape;
		//as the disk can't be hot removed and handle_stripe5 will retry the absence disk in dead loop;
		//so we set the absence disk fail and let the reshape move on;
		if (rdev && test_bit(Faulty, &rdev->flags))
			rdev = NULL;
...
static void handle_stripe6(struct stripe_head *sh)
{
	...
	for (i=disks; i--; ) {
		...
		// don't let system hung when disk fail in reshape process
		if (rdev && test_bit(Faulty, &rdev->flags))
			rdev = NULL;
...
static ssize_t
stripe_cache_invalidate(mddev_t *mddev, const char *page, size_t len)
{
	...
	if (strict_strtoull(page, 10, &pos))
		return -EINVAL;
	sector = pos >> 9;
