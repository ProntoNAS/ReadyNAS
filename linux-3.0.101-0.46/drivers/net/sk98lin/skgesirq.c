...
SK_U16		IStatus)	/* Interrupt Status */
{
	...
	if ((IStatus & PHY_M_IS_AN_ERROR) != 0) {
		...
		if (pAC->GIni.GICopperType) {
			 ...
			 *
			 * The issue happens with 100Mbps and autoegotiaiton only.
			 * The specificity of the issue is that a 100Mbit link cannot be
			 * established althrough speed and duplex mode can be exchanged during
			 * autonegotiation process.
			 * The workaround forces a good link. It is only executed if an
			 * autonegotiation error occurs and next pages have already been
