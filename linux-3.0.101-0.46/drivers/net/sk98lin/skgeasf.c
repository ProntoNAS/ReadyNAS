	...
	case SK_INIT_RUN:
		...
		if (fw_read(pAC,FwFileNameS1,&pAsfFwS1,&FileLengthS1) && 
			...
			if( FwCs == 0  )  {  //  CS == 0 => O.K.
				...
				printk("%s: File FW Checksum not OK\n", SK_DRV_NAME);
			...
			if( (pAC->AsfData.FileFwVersion[1] == pAC->AsfData.DriverVersion[1]) &&
				...
				SK_DBG_MSG(pAC, SK_DBGMOD_ASF, SK_DBGCAT_CTRL,("    FW suits the driver version\n"));
				...
				/* calculate CS of the FW flash */
					...
					if( FwCs == 0  )  {  //  CS == 0 => O.K.
						...
						FwFlashCsOk = 0;
#ifdef USE_ASF_DASH_FW
						printk("%s: Chip FW Checksum not OK\n", SK_DRV_NAME);
#endif						
					...
					printk("%s: Flash FW Version %c%c%c%c Rev.%c, File FW Version: %c%c%c%c Rev.%c\n",
						   SK_DRV_NAME,
					       pAC->AsfData.FlashFwVersion[1], pAC->AsfData.FlashFwVersion[2],
					       pAC->AsfData.FlashFwVersion[3], pAC->AsfData.FlashFwVersion[4],
					       pAC->AsfData.FlashFwRev,
					...
					if( ( (pAC->AsfData.FlashFwVersion[1] != pAC->AsfData.DriverVersion[1]) ||
						...
						(pAC->AsfData.FileFwRev == 'x')                                   ||   // Rev == 'x' means: do always a flash update ! (for test purposes)
#ifdef USE_ASF_DASH_FW
						(FwFlashCsOk != 1) ) && (!pAC->ReturningFromSuspend) )           // Checksum error in flash
#else
...
#ifdef USE_ASF_DASH_FW
								printk("%s: Flash Size (%lu) not supported\n", SK_DRV_NAME, FlashSize);
#endif
								break;
						}
						if (!RetCode)
...
#ifdef USE_ASF_DASH_FW
					        printk("%s: Starting the flash process\n", SK_DRV_NAME);
#endif
...
SK_IOC IoC ) /* IO context handle */  {
    ...
    SK_EVPARA   EventParam;
    SK_U8       i, NewAddr, NewPattern;
    static SK_U8       ManageNewAddr, ManageNewPattern;
    SK_U8       lRetCode;
    SK_U8       TmpBuffer[128];
    SK_U8       *MemBuffer;
    SK_U8       Ind;
    ...
    // We use the setfwip utility which uses another trigger mechanism
#ifndef USE_ASF_DASH_FW
    for( i=0; i<4; i++ )  {
    ...
    for( i=0; i<16*7; i++ )  {
        if( pAC->IpV6Addr[i] != pAC->AsfData.Mib.IpV6Source[i] )  {
            NewAddr = 1;
            SK_DBG_MSG(pAC, SK_DBGMOD_ASF, SK_DBGCAT_CTRL,("SkAsfTimer: New IP Addr detected\n"));
        }
    }
    ...
    // The same for the patterns...
    if (pAC->RecvNewPattern == 1) {
      NewPattern = 1;
      pAC->RecvNewPattern = 0;
    }
#endif
	  ...
	  case SK_GEASF_MODE_IPMI:
	    ...
	    if ( (NewAddr == 1) ||
		 ...
		 (pAC->AsfData.Mib.RlmtMode != (SK_U8)(pAC->Rlmt.NumNets & 0xff)) ) {
	      ...
	      SK_DBG_MSG(pAC, SK_DBGMOD_ASF, SK_DBGCAT_CTRL,
			 ("SkAsfTimer: IPMI update config with %d 0x%08x 0x%08x 0x%08x\n",
	  ...
	  case SK_GEASF_MODE_DASH:
	    if ( ( NewAddr == 1 ) || 
		 (NewPattern == 1 ) ) {
	      ...
	      ManageNewAddr = NewAddr;
	      ManageNewPattern = NewPattern;
	...
	SK_DBG_MSG(pAC, SK_DBGMOD_ASF, SK_DBGCAT_CTRL,
		("SkAsfTimer: sending  YASF_HOSTCMD_CFG_SET_IP_SOURCE %03d.%03d.%03d.%03d -> %03d.%03d.%03d.%03d\n",
		 pAC->AsfData.Mib.IpSource[0], pAC->AsfData.Mib.IpSource[1], pAC->AsfData.Mib.IpSource[2], 
		 pAC->AsfData.Mib.IpSource[3], pAC->IpAddr[0], pAC->IpAddr[1], pAC->IpAddr[2], pAC->IpAddr[3]));

#ifdef USE_ASF_DASH_FW
		printk("%s: Firmware IP: %u.%u.%u.%u\n",
		   SK_DRV_NAME,
		   (unsigned int)pAC->IpAddr[0],
		   (unsigned int)pAC->IpAddr[1],
		   (unsigned int)pAC->IpAddr[2],
		   (unsigned int)pAC->IpAddr[3]);
      
		   for( i=0; i<7; i++ ) {
			printk("%s: Firmware IPv6: %02x%02x:%02x%02x:%02x%02x:%02x%02x:%02x%02x:%02x%02x:%02x%02x:%02x%02x\n",
			   SK_DRV_NAME,
			   (unsigned int)pAC->IpV6Addr[0+i*16], (unsigned int)pAC->IpV6Addr[1+i*16], 
			   (unsigned int)pAC->IpV6Addr[2+i*16], (unsigned int)pAC->IpV6Addr[3+i*16], 
			   (unsigned int)pAC->IpV6Addr[4+i*16], (unsigned int)pAC->IpV6Addr[5+i*16], 
			   (unsigned int)pAC->IpV6Addr[6+i*16], (unsigned int)pAC->IpV6Addr[7+i*16], 
			   (unsigned int)pAC->IpV6Addr[8+i*16], (unsigned int)pAC->IpV6Addr[9+i*16], 
			   (unsigned int)pAC->IpV6Addr[10+i*16], (unsigned int)pAC->IpV6Addr[11+i*16], 
			   (unsigned int)pAC->IpV6Addr[12+i*16], (unsigned int)pAC->IpV6Addr[13+i*16], 
			   (unsigned int)pAC->IpV6Addr[14+i*16], (unsigned int)pAC->IpV6Addr[15+i*16]);
		   }
#endif
		   Ind = 0;
		   TmpBuffer[Ind++] = YASF_HOSTCMD_CFG_SET_IP_SOURCE;
		   TmpBuffer[Ind++] = 2 + 4 + 16*7;  // Length
...
#ifdef USE_ASF_DASH_FW
	if (ManageNewPattern == 1) {
	  if (sizeof(DRIVER_INTERFACE) <= 512) {
	     MemBuffer = (SK_U8*) &pAC->NewPatternDef;

#if 0
		for (i=0; i<sizeof(DRIVER_INTERFACE); i++) {
			printk("%02x ", MemBuffer[i]);
			if ((i+1) % 16 == 0)
				printk("\n");
		}	
		printk("\n");
	  ...
	  case 0:
#if 0
		  printk("%s: Pattern block %i downloaded to firmware\n", SK_DRV_NAME, PatternCounter+1);
#endif
		  // Pattern Memory Part 1
		  Ind = 0;
		  TmpBuffer[Ind++] = YASF_HOSTCMD_CFG_SET_SW_PATTERN;
		  TmpBuffer[Ind++] = 4 + 124;  
		  TmpBuffer[Ind++] = 0;  
		  TmpBuffer[Ind++] = 1;  // Pattern Part 1 of 4
	  
		  for (i=0; i<124; i++)
		    TmpBuffer[Ind++] = MemBuffer[Counter++];

#if 0
		  for (i=4; i<128; i++) {
		    printk("%02x ", TmpBuffer[i]);
		    if ((i-4) % 16 == 0)
		      printk("\n");
		  }
		  printk("\n");
	  ...
	  case 1:
		  ...
		  printk("%s: Pattern block %i downloaded to firmware\n", SK_DRV_NAME, PatternCounter+1);
#endif
		  // Pattern Memory Part 2
		  Ind = 0;
		  TmpBuffer[Ind++] = YASF_HOSTCMD_CFG_SET_SW_PATTERN;
		  TmpBuffer[Ind++] = 4 + 124;  
		  TmpBuffer[Ind++] = 0;  
		  TmpBuffer[Ind++] = 2;  // Pattern Part 2 of 4
	  ...
	  case 2:
		  ...
		  printk("%s: Pattern block %i downloaded to firmware\n", SK_DRV_NAME, PatternCounter+1);
#endif
		  // Pattern Memory Part 3
		  Ind = 0;
		  TmpBuffer[Ind++] = YASF_HOSTCMD_CFG_SET_SW_PATTERN;
		  TmpBuffer[Ind++] = 4 + 124;  
		  TmpBuffer[Ind++] = 0;  
		  TmpBuffer[Ind++] = 3;  // Pattern Part 3 of 4
	  ...
	  case 3:
		  ...
		  printk("%s: Pattern block %i downloaded to firmware\n", SK_DRV_NAME, PatternCounter+1);
#endif
  		  // Pattern Memory Part 4
		  Ind = 0;
		  TmpBuffer[Ind++] = YASF_HOSTCMD_CFG_SET_SW_PATTERN;
		  TmpBuffer[Ind++] = 4 + sizeof(DRIVER_INTERFACE) - 124*3;  // Write the rest...
		  TmpBuffer[Ind++] = 0;  
		  TmpBuffer[Ind++] = 4;  // Pattern Part 4 of 4
	  
		  for (i=0; i<sizeof(DRIVER_INTERFACE) - 124*3; i++)
		    TmpBuffer[Ind++] = MemBuffer[Counter++];
	  
#if 0
		  for (i=4; i<4 + sizeof(DRIVER_INTERFACE) - 124*3; i++) {
		    printk("%02x ", TmpBuffer[i]);
		    if ((i-4) % 16 == 0)
		      printk("\n");
		  }	
		  ...
		  break;
	default:
		  printk("%s: Error writing patterns to firmware\n", SK_DRV_NAME);
 		  break;
	}
	} else {
	  ManageNewPattern = 0;
	  printk("%s: Could not write patterns to firmware: Pattern memory to large\n", SK_DRV_NAME);
	}
      }
#endif
	...
	if (ManageNewPattern == 1) {
	  ...
	  if (PatternCounter == 4) {
		  PatternCounter = 0;
		  ManageNewPattern = 0;
		  // Double increase state!!!
	  	  pAC->AsfData.GlHciState++;
	  	  pAC->AsfData.GlHciState++;          
		  printk("%s: Firmware patterns updated\n", SK_DRV_NAME);
	  }
	}
	if (ManageNewAddr == 1) {
	  ManageNewAddr = 0;
	  ...
	  for( i=0; i<16*7; i++ ) {
	    pAC->AsfData.Mib.IpV6Source[i] = pAC->IpV6Addr[i];
	  }
  	  pAC->AsfData.GlHciState++;          
	}
#endif
	...
	if (ManageNewPattern == 1) {
	  ManageNewPattern = 0;
	  printk("%s: Error while updating pattern memory", SK_DRV_NAME);
	}
	if (ManageNewAddr == 1) {
	  ManageNewAddr = 0;
	  printk("%s: Error while updating IP addresses", SK_DRV_NAME);
	}
	pAC->AsfData.GlHciState = 255;      // error
      }
      break;
...
#endif
#ifdef USE_ASF_DASH_FW
	lRetCode = AsfHciSendCommand(pAC, IoC, YASF_HOSTCMD_CFG_STORE_CONFIG, 0, 0, 0, ASF_HCI_WAIT, 2 );
	if( lRetCode == HCI_EN_CMD_READY )  {
	  pAC->AsfData.GlHciState = 254;      // go to finish
	}
#endif	
...
void AsfRunCpu( SK_AC *pAC, SK_IOC IoC )  {
  ...
  if( pAC->AsfData.ChipMode == SK_GEASF_CHIP_EX )  {
    ...
    /* These three entries are new */

    /* Clear link reset */
    // SK_OUT16( IoC, LINK_CTRL, LINK_CTRL_RST_CLR );
    /* Clear ASF FIFO reset */
    SK_OUT32( IoC, ASF_CTRL, ASF_CTRL_ASF_ENA );
    /* Enable access above 128K, A17 gate */
    // SK_OUT32( IoC, SPI_CFG, SPI_CFG_A17_GATE );
...
SK_U8   port )  {
    ...
    SK_U8 MacFramePattern[128]; /* Pattern for MAC Address */
    SK_U8 MacPatternMask[2]; /* PatternMask  for MAC Address */
    SK_U8 MulFramePattern[128]; /* Pattern for Multicast Packets */
    SK_U8 MulPatternMask[2]; /* PatternMask  for Multicast Address */
    SK_U8 BdcFramePattern[128]; /* Pattern for Broadcast Packets */
    SK_U8 BdcPatternMask[2]; /* PatternMask  for Broadcast Address */
    SK_U8 Iv6FramePattern[128]; /* Pattern for ICMPv6 Packets */
    SK_U8 Iv6PatternMask[2]; /* PatternMask  for ICMPv6 Address */
    ...
    switch (pAC->AsfData.OpMode) {
	    ...
	    if( pAC->AsfData.ChipMode != SK_GEASF_CHIP_EX )
	      break;
            ...
            // disable Wake Up Frame Unit before write to pattern ram
#ifdef USE_ASF_DASH_FW
	    TmpVal16 = WOL_CSR_WF_PME_DIS | WOL_CSR_WF_DIS;
            SK_OUT16(IoC, WOL_CSR, TmpVal16);	    
#else
            SK_IN16(IoC, ASF_YEC_PATTERN_CTRL1+(port*0x80), &TmpVal16);
            TmpVal16 &= ~0x03;   // clear bit 0, bit1
            TmpVal16 |=  0x01;   // set bit 0
	    ...
	    /* Setup MAC Pattern */
	    MacFramePattern[0] = pAC->AsfData.Mib.MacSource[0];
	    MacFramePattern[1] = pAC->AsfData.Mib.MacSource[1];
	    MacFramePattern[2] = pAC->AsfData.Mib.MacSource[2];
	    MacFramePattern[3] = pAC->AsfData.Mib.MacSource[3];
	    MacFramePattern[4] = pAC->AsfData.Mib.MacSource[4];
	    MacFramePattern[5] = pAC->AsfData.Mib.MacSource[5];
	    MacFramePattern[6] = 0x0;
	    MacFramePattern[7] = 0x0;

	    MacPatternMask[0] = 0x3F;
	    MacPatternMask[1] = 0x0;

            // write MAC pattern 
	    AsfWritePatternRamEx(pAC, IoC, port, ASF_DASH_PATTERN_NUM_MAC,  6, MacPatternMask, MacFramePattern );

#if 0
	    printk("%s: Mac Addresse: %02x::%02x::%02x::%02x::%02x::%02x\n",
			   SK_DRV_NAME,
		       pAC->AsfData.Mib.MacSource[0],
		       pAC->AsfData.Mib.MacSource[1],
		       pAC->AsfData.Mib.MacSource[2],
		       pAC->AsfData.Mib.MacSource[3],
		       pAC->AsfData.Mib.MacSource[4],
		       pAC->AsfData.Mib.MacSource[5]);
#endif

	    /* Setup MUL Pattern */
	    MulFramePattern[0] = 0x1; /* Bit 0 of Byte 1 indicates a Multicast/Broadcast packet on Layer 2 */
    	    MulFramePattern[1] = 0x0;
    	    MulFramePattern[2] = 0x0;
    	    MulFramePattern[3] = 0x0;
    	    MulFramePattern[4] = 0x0;
    	    MulFramePattern[5] = 0x0;
    	    MulFramePattern[6] = 0x0;
    	    MulFramePattern[7] = 0x0;
	
	    MulPatternMask[0] = 0x1;
	    MulPatternMask[1] = 0x0;

            // write MULticast pattern 
	    AsfWritePatternRamEx(pAC, IoC, port, ASF_DASH_PATTERN_NUM_MUL,  6, MulPatternMask, MulFramePattern );

	    /* Setup BroaDCast Pattern */
	    BdcFramePattern[0] = 0xFF;
    	    BdcFramePattern[1] = 0xFF;
    	    BdcFramePattern[2] = 0xFF;
    	    BdcFramePattern[3] = 0xFF;
    	    BdcFramePattern[4] = 0xFF;
    	    BdcFramePattern[5] = 0xFF;
    	    BdcFramePattern[6] = 0x0;
    	    BdcFramePattern[7] = 0x0;
	
	    BdcPatternMask[0] = 0x1;
	    BdcPatternMask[1] = 0x0;

            // write BroaDCast pattern 
	    AsfWritePatternRamEx(pAC, IoC, port, ASF_DASH_PATTERN_NUM_BDC,  6, BdcPatternMask, BdcFramePattern );

	    /* Setup IcmpV6 Pattern */
	    Iv6FramePattern[0] = 0x33; /* Bit 0 of Byte 1 indicates a Multicast/Broadcast packet on Layer 2 */
    	    Iv6FramePattern[1] = 0x0;
    	    Iv6FramePattern[2] = 0x0;
    	    Iv6FramePattern[3] = 0x0;
    	    Iv6FramePattern[4] = 0x0;
    	    Iv6FramePattern[5] = 0x0;
    	    Iv6FramePattern[6] = 0x0;
    	    Iv6FramePattern[7] = 0x0;
	
	    Iv6PatternMask[0] = 0x1;
	    Iv6PatternMask[1] = 0x0;

            // write IcmpV6 pattern 
	    AsfWritePatternRamEx(pAC, IoC, port, ASF_DASH_PATTERN_NUM_ICMPV6,  6, Iv6PatternMask, Iv6FramePattern );

	    // Enable Pattern Match Unit
	    TmpVal16 = WOL_CSR_WF_ENA;
            SK_OUT16(IoC, WOL_CSR, TmpVal16);	    
...
SK_U8   pattno )
{
	...
	pattno += 6;
      ...
      SK_IN16(IoC, WOL_ASF_MEN, &val16);
      val16 |= (0x01 << pattno);
      SK_OUT16(IoC, WOL_ASF_MEN, val16);
      
      // enable pattern pattno
      //SK_IN16(IoC, ASF_YEX_PATTERN_ENA1+(port*0x80), &val16);
      // val16 |= (0x01 << pattno);
      //SK_OUT16(IoC, ASF_YEX_PATTERN_ENA1+(port*0x80), val16);
      SK_IN16(IoC, WOL_MCTRL, &val16);
      val16 |= (0x01 << pattno);
      SK_OUT16(IoC, WOL_MCTRL, val16);
    }
    else  {
      SK_U8   val8;
      pattno += 4;
