...
#endif
#ifdef USE_ASF_DASH_FW
static void SkSetFWLinkParams(SK_AC *pAC, SK_IOC IoC, int Port);
static void SkGeHandleLLCTimer(unsigned long ptr);
static void SkGeHandleWakeupTimer(unsigned long ptr);
#endif
#ifdef Y2_RECOVERY
...
#ifdef USE_ASF_DASH_FW
extern void SkSendLLCPacket(SK_AC *pAC, SK_IOC IoC, SK_U32 PortNumber);
#endif
#if LINUX_VERSION_CODE > KERNEL_VERSION(2,6,19)
...
extern int SkGeGetStatsLen(struct net_device *dev, int stringset);
...
#ifdef USE_ASF_DASH_FW
static int	RamReadAddr(SK_AC *, SK_U32 , SK_U32 *, SK_U32 *, int);
static int	RamWriteAddr(SK_AC *, SK_U32 , SK_U32 , SK_U32 , int);
#endif
...
/*	.get_perm_addr		= ethtool_op_get_perm_addr, */ 
	...
	.get_sset_count		= SkGeGetStatsLen,
...
#define RXPORT pAC->RxPort[pNet->PortNr]
	...
	if (	(CHIP_ID_YUKON_2(pAC)) &&
		(!netif_queue_stopped(pAC->dev[pNet->PortNr]))) {
...
struct pci_dev *pdev)   /* the device that is to resume */
{
	...
	/* NETGEAR */
        {
        SK_U32          our2;
        SkPciReadCfgDWord(pAC, PCI_OUR_REG_1, &our2);
        our2 |= PCI_PME_STATUS;
        SkPciWriteCfgDWord(pAC, PCI_OUR_REG_1, our2);
        }
	/* NETGEAR */
...
#endif
{
	...
	pci_enable_wake(pdev, PCI_D3hot, 1);	/* D3 hot */
	pci_enable_wake(pdev, PCI_D3cold, 1);	/* D3 cold */
...
struct pci_dev	*pdev)   /* pointer to the device that is to suspend */
{
	...
	if (pAC->WolInfo.ConfiguredWolOptions == 0) {
		//return; /* WOL possible, but disabled via ethtool */
		goto exit_no_wol;
	}
	...
	pci_disable_device(pdev);
	pci_set_power_state(pdev, PCI_D3hot); /* set the state */
...
#ifdef USE_ASF_DASH_FW
	printk("sk98lin: shutdown complete\n");
#endif
	return;
}
        ...
        *PhyCtrl = 0;   /* PHY_MARV_CTRL */
        ...
        if (pAC->GIni.GICopperType) {
                ...
                case SK_LSPEED_AUTO:
                        ...
                        *PhyCtrl |= PHY_CT_SP100; /* Default: 100MBPS */
                        break;
        ...
        /* Set the duplex mode */
        if (pAC->WolDuplexType) {
                *PhyCtrl = PHY_CT_RESET | PHY_CT_SPS_LSB | PHY_CT_ANE |
                                PHY_CT_DUP_MD |  PHY_CT_COL_TST | PHY_CT_SPS_MSB |
                                PHY_CT_RE_CFG;
        } else {
                *PhyCtrl = PHY_CT_RESET | PHY_CT_SPS_LSB | PHY_CT_ANE |
                                PHY_CT_COL_TST | PHY_CT_SPS_MSB | PHY_CT_RE_CFG;
        }
}

static void SkEnableWOMagicPacket1(
...
SK_MAC_ADDR    MacAddr)  /* MacAddr expected in magic packet */
{
	 ...
	 *                                     */
        if ((pAC->GIni.GIChipId == CHIP_ID_YUKON_EC)   ||
                ...
                SkCheckWOParam(pAC, &AutoNegAdv, &PhyCtrl);
                SkGmPhyWrite(pAC, IoC, Port, PHY_MARV_AUNE_ADV, AutoNegAdv); /* Advertise 10/100 HD/FD */
                SkGmPhyWrite(pAC, IoC, Port, PHY_MARV_1000T_CTRL, 0); /* Do not advertise 1000 HD/FD */
                SkGmPhyWrite(pAC, IoC, Port, PHY_MARV_CTRL, PhyCtrl); /* 100 MBps, Autoneg, FD */
                ...
                SK_OUT8(IoC, GMAC_CTRL, 0x02);                  /* clear MAC reset */
                SkCheckWOParam(pAC, &AutoNegAdv, &PhyCtrl);
                SkGmPhyWrite(pAC, IoC, Port, PHY_MARV_CTRL, PhyCtrl);     /* 100 MBit, disable Autoneg */
                ...
                SkGmPhyWrite(pAC, IoC, Port, PHY_MARV_PHY_CTRL, Word);     /* Write Parameters */
...
SK_IOC         IoC)      /* I/O control context              */
{
        ...
        TmpVal32 |= PCIE_OUR4_DYN_CLK_GATE_SET;
        TmpVal32 &= ~P_TIMER_VALUE_MSK; /* Set timer values to 0 (dr) */
#if 0
        SK_OUT32(pAC->IoBase, Y2_CFG_SPC + PCI_OUR_REG_4, TmpVal32); /* 0x68003015 */
#else
        SK_OUT32(pAC->IoBase, Y2_CFG_SPC + PCI_OUR_REG_4, 0x68003015);
#endif
#if 0
...
#endif
{
        ...
        SkDrvEnterDiagMode1(pAC);
        SkEnableWOMagicPacket1(pAC, pAC->IoBase, MacAddr);
        SetDynamicClockGating(pAC, pAC->IoBase);
...
SK_U32		IntSrc;		/* interrupts source register contents */	
	...
	if (netif_rx_schedule_prep(&pNet->napi)) {
...
struct SK_NET_DEVICE *dev)  /* the device that is to be opened */
{
	...
	spin_lock_irqsave(&pAC->SlowPathLock, Flags);
#ifdef USE_ASF_DASH_FW
	if (pAC->LinkSpeedSet == SK_TRUE) {
		pAC->GIni.GP[0].PLinkSpeed = pAC->LinkSpeed;
		pAC->LinkSpeedSet = SK_FALSE;
	}

	/* Initialize the LLC kernel timer */
	init_timer(&pNet->LLCTimer);
	pNet->LLCTimer.function	= SkGeHandleLLCTimer;
	pNet->LLCTimer.data	= (unsigned long) pNet;
	pNet->LLCTimer.expires	= jiffies + (HZ/10); // 0.1s later...

	/* Initialize the Wakeup kernel timer */
	init_timer(&pNet->WakeupTimer);
	pNet->WakeupTimer.function	= SkGeHandleWakeupTimer;
	pNet->WakeupTimer.data		= (unsigned long) pNet;
	pNet->WakeupTimer.expires	= jiffies + (HZ/10); // 0.1s later...
#endif
	...
	if ((pAC->GIni.GIChipId == CHIP_ID_YUKON_EC) ||
		...
		printk("%s: Alternative timing system globally activated (TSC)\n", SK_DRV_NAME);
#endif
	SK_DBG_MSG(NULL, SK_DBGMOD_DRV, SK_DBGCAT_DRV_ENTRY,
		("SkGeOpen suceeded\n"));
...
#ifdef USE_ASF_DASH_FW
	del_timer(&pNet->LLCTimer);
	del_timer(&pNet->WakeupTimer);
#endif
...
{
	/* Support for Yukon Extreme only. */
...
int		SelectedRam)	/* Selected RAM buffer 0, 1 or 3 */
{
	...
	if (pAC->GIni.GIChipId == CHIP_ID_YUKON_EX) {
		if (Addr == BIT_29) {
			...
			Addr = (Addr & 0x7ffL) | BIT_29 | (SelectedRam << 30);
		}
		SelectedRam = 0;
	}
	else {
...
SK_AC	*pAC,			/* Adapter context */
SK_U32	Addr,			/* Address to be read at */
SK_U32	LowDword,		/* Lower Dword to be read */
SK_U32	HighDword,		/* Upper Dword to be read */
int		SelectedRam)	/* Selected RAM buffer 0, 1 or 3 */
{
	/* Support for Yukon Extreme only. */
	if (pAC->GIni.GIChipId == CHIP_ID_YUKON_EX) {
...
/******************************************************************************
 *
 *	AccessRamBuf() - Dump a number of RAM Quad Words
 *
 * Description:
 *	Dump the given number of QWords from the RAM. Use the current address.
 *
 * Returns:
 *	nothing
 */
...
SK_BOOL	ReadAccess)
{
	...
	SK_U32	QWordLength = 0x0;
	SK_U32	FrameLength = 0x0;
	SK_U32	CurrRamAddr;
	SK_U16	*pQWordLength = (SK_U16 *) &LowDword;
	SK_U16	*pFrameLength = (SK_U16 *) &HighDword;
	SK_U32	*pWriteDword = NULL;
	struct	sk_buff *pMsg = NULL;

	CurrRamAddr = pAC->RamAddr;

	if (ReadAccess == SK_TRUE) {

		/* Read values. */
		(void) RamReadAddr(pAC, CurrRamAddr, &LowDword, &HighDword, pAC->RamSelect);

		QWordLength = pQWordLength[1];
		FrameLength = SWITCH_16(pFrameLength[0]);

		if (FrameLength == 0) {
			return 2;
		}

		pMsg = alloc_skb(((QWordLength + 1) * 8), GFP_ATOMIC);
		if (pMsg == NULL) {
			printk("AccessRamBuf: Allocation of RX buffer failed!\n");
			return 1;
		}
		...
		pWriteDword = (SK_U32 *) pMsg->data;
		NumOfRamDwords--;
		CurrRamAddr++;
		NumOfRamDwords = QWordLength;
	}

	while (NumOfRamDwords > 0) {
		if (ReadAccess == SK_TRUE) {

			if (pMsg == NULL) {
				printk("AccessRamBuf: No RX buffer!!\n");
				return 1;
			}
			...
			*pWriteDword = HighDword;
			...
			/* Write values. */
			(void) RamWriteAddr(pAC, CurrRamAddr, LowDword, HighDword, pAC->RamSelect);
		}
		NumOfRamDwords--;
		CurrRamAddr++;
	}
...
int                   cmd)  /* requested IOCTL command number               */
{
	...
	case SK_IOCTL_IPTOFW:
		...
		if(netif_running(dev)) {
			if (dev->ip_ptr != NULL) {
				 ...
				 * But if it happens it is only bad in case we are 
				 * running DASH
				 */
				pAC->IpAddr[3] = 0;
				pAC->IpAddr[2] = 0;
			...
			if (iface != NULL) {
				...
				while ( (address_list != NULL) && (savedaddr < 7) ) {
			...
			/* So possibly there is no IpV6 support in this kernel. */
				for (i=0; i<16; i++) {
					pAC->IpV6Addr[i] = 0;
				}
			}
	...
	case SK_IOCTL_READASFFIFO:
		/* Read pattern from ASF Fifo. */
		SetRamAddr(pAC, SK_ST_FIFOTYPE, SK_ST_BUFADDR_LOW, SK_ST_BUFADDR_HIGH);
		AccessRamBuf(pAC, SK_ST_BUFSIZE, SK_TRUE);
		break;
	...
	case SK_IOCTL_LINKTOFW:
		if(copy_from_user(&LinkData[0], rq->ifr_data, sizeof(LinkData))) {
			return -EFAULT;
		}

		pAC->SuspendLinkSpeed  = LinkData[0];
		pAC->SuspendDuplexMode = LinkData[1];
	...
	case SK_IOCTL_GETLINKCAP:
		if(netif_running(dev)) {
			...
			SkGmPhyRead(pAC, pAC->IoBase, pAC->ActivePort, PHY_MARV_AUNE_LP, &LinkCap);
			memcpy((char *) &LinkData[0], (char *) &LinkCap, sizeof(LinkCap));

			if ((pAC->dev[pAC->ActivePort]->flags & IFF_RUNNING) == 0) {

				LinkData[2] = SK_AUTONEG_NOLINK; /* No link. */
			}
			else if ((pAC->GIni.GP[pAC->ActivePort].PLinkModeConf == SK_LMODE_HALF) ||
				(pAC->GIni.GP[pAC->ActivePort].PLinkModeConf == SK_LMODE_FULL)) {
				...
				LinkData[2] = pAC->GIni.GP[pAC->ActivePort].PAutoNegFail;
			}
...
#else //JM: enable by default, accept an off parameter instead
	pAC->LowLatency = SK_TRUE;
	if (LowLatency[pAC->Index] != NULL) {
		if (strcmp(LowLatency[pAC->Index], "Off") == 0) {
			pAC->LowLatency = SK_FALSE;
		}
	}
#endif
...
SK_EVPARA  Param)  /* event-parameter            */
{
	...
	case SK_DRV_NET_UP:
...
#ifdef USE_ASF_DASH_FW
		if (pAC->GIni.GP[FromPort].PLinkSpeedUsed == SK_LSPEED_STAT_10MBPS) {
			SkLocalEventQueue(pAC, SKGE_DRV, SK_DRV_RECOVER,
							  pAC->ActivePort, -1, SK_FALSE);
			pAC->SetIFFRunning = SK_TRUE;
		}
#endif
		...
		if (strstr(saved_command_line, "u4_alpha")) {
			if (DoPrintInterfaceChange)
				printk("    ultra4-alpha:    enabled\n");
			SkGmPhyWrite(pAC, IoC, FromPort, PHY_MARV_EXT_CTRL_2, 0x8040);
		}
		...
		if (pAC->MaxPorts > 0) {
			...
			if (pAC->SendLLCPacket == 1) {
				...
				if (!timer_pending(&pNet->LLCTimer)) {
					add_timer(&pNet->LLCTimer);
				}

				/* Start the Wakeup kernel timer */
				if ((pAC->SendWolPattern == SK_TRUE) &&
					(!timer_pending(&pNet->WakeupTimer))) {
					add_timer(&pNet->WakeupTimer);
				}
			}
		}
	...
	case SK_DRV_RECOVER:
		...
		netif_stop_queue(pAC->dev[FromPort]); /* stop device if running */
		pAC->dev[FromPort]->flags &= ~IFF_RUNNING;
		...
		spin_lock(&pAC->TxPort[FromPort][TX_PRIO_LOW].TxDesRingLock);
		if (CHIP_ID_YUKON_2(pAC)) {
			SkY2PortStop(pAC, IoC, FromPort, SK_STOP_ALL, SK_SOFT_RST);
		} else {
			SkGeStopPort(pAC, IoC, FromPort, SK_STOP_ALL, SK_SOFT_RST);
		}
		spin_unlock(&pAC->TxPort[FromPort][TX_PRIO_LOW].TxDesRingLock);
