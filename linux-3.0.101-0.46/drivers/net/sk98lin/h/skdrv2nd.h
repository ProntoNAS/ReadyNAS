...
extern SK_U8 AccessRamBuf(SK_AC *, int, SK_BOOL);
#endif
...
#define SkOsGetTimeCurrent(pAC, pUsec) { \
	SK_U64 currentTime; \
	__RDTSCLL(currentTime); \
	do_div(currentTime, cpu_khz); \
	do_div(currentTime,(SK_U64)((SK_U32)1000 / SK_TICKS_PER_SEC)); \
	*pUsec = currentTime; \
}
#endif

#ifndef USE_ALT_TIMING
#define SkOsGetTimeCurrent(pAC, pUsec) {\
...
#define SK_IOCTL_READASFFIFO (SK_IOCTL_BASE + 12)
#define SK_IOCTL_LINKTOFW    (SK_IOCTL_BASE + 13)
...
#define SK_ST_BUFADDR_LOW	0xc5
#define SK_ST_BUFADDR_HIGH	0x0
#define SK_ST_BUFSIZE		191
#endif
...
#ifdef USE_ASF_DASH_FW
	struct 		timer_list LLCTimer;	/* LLC timer struct  */
	struct 		timer_list WakeupTimer;	/* Wakeup packet timer struct  */
#endif
...
struct s_AC  {
	...
	unsigned int             SendLLCPacket;
	unsigned char            SuspendLinkSpeed;
	unsigned char            SuspendDuplexMode;
