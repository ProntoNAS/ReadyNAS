 ...
 *
 */
int SkGeGetStatsLen(struct net_device *dev, int stringset)
{
			...
			struct ethtool_wolinfo *ecmd)
{
	...
	if (((ecmd->wolopts & WAKE_MAGIC) == WAKE_MAGIC) || (ecmd->wolopts == 0)) {
        ...
        /* NETGEAR */
                pci_pme_active(pAC->PciDev, true);
                device_set_wakeup_enable(&pAC->PciDev->dev, pAC->WolInfo.ConfiguredWolOptions);
        /* NETGEAR */
		return 0;
	}
	return -EFAULT;
			...
			u32 data)
{
	...
	SkGmPhyWrite(pAC, IoC, port, PHY_MARV_EXT_CTRL_2, 0x8040);
