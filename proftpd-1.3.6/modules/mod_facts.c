...
static size_t facts_mlinfo_fmt(struct mlinfo *info, char *buf, size_t bufsz,
  ...
  if (!(facts_mlinfo_opts & FACTS_MLINFO_FL_NO_NAMES)) {
    if (facts_opts & FACTS_OPT_SHOW_UNIX_GROUP_NAME) {
      len = snprintf(ptr, bufsz - buflen, "UNIX.groupname=%s;",
        strpbrk(info->group, " \t\f\n\r\v") ?
        pr_gid2str(NULL, info->st.st_gid) : info->group);
      buflen += len;
      ptr = buf + buflen;
    }
  }
  ...
  if (!(facts_mlinfo_opts & FACTS_MLINFO_FL_NO_NAMES)) {
    if (facts_opts & FACTS_OPT_SHOW_UNIX_OWNER_NAME) {
      len = snprintf(ptr, bufsz - buflen, "UNIX.ownername=%s;",
        strpbrk(info->user, " \t\f\n\r\v") ?
        pr_uid2str(NULL, info->st.st_uid) : info->user);
      buflen += len;
      ptr = buf + buflen;
    }
  }
