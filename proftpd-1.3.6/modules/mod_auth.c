 ...
 * @user: login user
 * @type: AUDIT_USER_AUTH or AUDIT_USER_END
 * @success: login success or not, 1 for success, 0 for fail
 */
static int write_audit_log(const char *remote, const char *user,
  int type, int success)
{
  int audit_fd;
  int rc = 0;
  const char *op = "PROFTPD:unknown";

  if (!remote || !user)
  {
    pr_log_auth(PR_LOG_ERR,"Remote or user is NULL.");
    return -1;
  }
  ...
  if (audit_fd < 0) {
    if (errno == EINVAL || errno == EPROTONOSUPPORT ||
      errno == EAFNOSUPPORT)
      pr_log_auth(PR_LOG_NOTICE, "Audit not enabled[%m]?");
    else
      pr_log_auth(PR_LOG_ERR,"audit_open error: %m.");
    return -1;
  }

  if (type == AUDIT_USER_AUTH)
    op = "PROFTPD:authentication";
  else if (type == AUDIT_USER_END)
    op = "PROFTPD:session_close";

  PRIVS_ROOT
  rc = audit_log_acct_message(audit_fd, type, "proftpd", op,
    user, -1, remote, NULL, "proftpd", success);
  if (rc <= 0)
    pr_log_auth(PR_LOG_ERR,"audit_log_acct_message error: %s[%d]",
      strerror(errno), rc);
  PRIVS_RELINQUISH
...
static void auth_exit_ev(const void *event_data, void *user_data) {
  if (session.user &&
    (strcmp(session.user, "guest") == 0))
    write_audit_log(pr_netaddr_get_sess_remote_name(), "guest",
      AUDIT_USER_END, 1);
...
MODRET auth_log_pass(cmd_rec *cmd) {
  ...
  if (session.anon_config != NULL)
    write_audit_log(session.c->remote_name, "guest", AUDIT_USER_AUTH, 1);
...
static int setup_env(pool *p, cmd_rec *cmd, const char *user, char *pass) {
  ...
  if (pw == NULL) {
    ...
    /* RNAS-8670: log non-exist user
     * windows ftp client try to use 'anonymous' to login
     * automatically, exclude this.
     */
    if (user && strcmp(user, "anonymous"))
      write_audit_log(session.c->remote_name, user, AUDIT_USER_AUTH, 0);
