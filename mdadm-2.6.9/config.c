...
int cmp_by_node(const void *a, const void *b) 
{ 
	mddev_dev_t const * ia = a;
	mddev_dev_t const * ib = b;
	uint32_t inta = 0, intb = 0;
	char *base;

	base = basename((*ia)->devname);
	if (strlen(base) < 3 || strlen(base) > 5 || strncmp(base, "sd", 2)) {
		inta = 0;
	} else {
		base += 2;
		memcpy(&inta, base, strlen(base));
	}

	base = basename((*ib)->devname);
	if (strlen(base) < 3 || strlen(base) > 5 || strncmp(base, "sd", 2)) {
		intb = 0;
	} else {
		base += 2;
		memcpy(&intb, base, strlen(base));
	}
...
mddev_dev_t load_partitions(void)
{
	...
	/* NTGR: sort so the lower arrays get handled first */
	mddev_dev_t tmp;
	mddev_dev_t qs[num+1];
	memset(&qs, 0, sizeof(qs));
	for (tmp = rv; tmp; tmp = tmp->next) {
		qs[i++] = tmp;
	}
	qsort(qs, num, sizeof(mddev_dev_t), cmp_by_node);
	rv = qs[0];
	for (tmp = rv; tmp; tmp = tmp->next)
		tmp->next = 0;
	for (tmp = rv, i = 0; tmp; tmp = tmp->next)
		tmp->next = qs[++i];
	/* /NTGR */
	return rv;
}
