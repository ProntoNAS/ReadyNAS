			...
			if (update == NULL &&
				...
				if ((inargv && verbose >= 0) || verbose > 0)
					fprintf(stderr, Name ": %s is not built for host %s.\n",
						devname, homehost);
				/* NTGR: Add this to a list of devices to possibly try later */
				char *arr_num = strchr(info.name, ':');
				int i;
				if (arr_num)
					arr_num++;
				else
	...
	if (!old_linux) {
		...
		if (runstop == 1 ||
			...
			if (ioctl(mdfd, RUN_ARRAY, NULL)==0) {
				...
				/* NTGR: This array number is started, so remove anything matching from the list
				 * of devices to possibly try later */
				int arr = atoi(basename(mddev));
				if (arr < 32 && names[arr] > 0) {
					free(names[arr]);
					names[arr] = calloc(1, 1);
				}
				/* /NTGR */
				if (info.reshape_active &&
				    info.array.level >= 4 &&
