...
struct packet {
        uint32_t checksum;                /* Checksum of the rest of the 
                                           packet */
        uint32_t version;                 /* Version if raidard protocol/packet 
                                         * To avoid possible problems if
                                         * the packet format is later
                                         * changed */
        uint32_t flag;                    /* Packet flags */
	uint32_t cid;                     /* id of client 
                                         * Should be uniqe per client procces.
                                         * The current pid wouldn't be a bad
                                         * choice */
	uint32_t sid;                     /* id of server. Should be unique per
                                         * server. */
	uint32_t length;                  /* Total length of packet */
	uint32_t offset;                  /* Offset of packet. Incremented by
                                         * length - PACKET_HDR_LEN for
                                         * each packet */
	char buf[2048];                 /* Data */
};

#define PACKET_HDR_LEN (uint32_t)28

#define RAIDARD_LIST_SID ((uint32_t)~0)
...
void packet_header(struct packet *packet, uint32_t cid, uint32_t sid, 
                uint32_t flag, uint32_t length, uint32_t offset, 
                uint32_t zero_packet);
...
void packet_send_keepalive(struct sockaddr_in *iaddr, uint32_t cid, uint32_t sid);
...
void packet_send_ack(struct sockaddr_in *iaddr, uint32_t cid, uint32_t sid);
...
void packet_send_end(struct sockaddr_in *iaddr, uint32_t cid, uint32_t sid);
...
void packet_send_primer(struct sockaddr_in *iaddr, uint32_t cid, uint32_t sid);
...
#define packet_checksum(_packet) \
        raidard_checksum((void *)((char *)_packet + sizeof(uint32_t)), \
                        ntohl((_packet)->length) - sizeof(uint32_t))
