...
/* "ranges" feeping creaturism hacked in by Peter Samuelson. */

#include "system.h"
#include <stdio.h>
#include "getopt.h"
...
static off_t file_position PARAMS((int, off_t));
...
static off_t scan_off_t PARAMS((char **));
static int parse_range PARAMS((off_t[], char const *));
...
/* Byte ranges we need to compare.  */
static off_t range[2][2] = { {0,-1}, {0,-1} };
...
/* Output format:
   type_first_diff
     to print the byte number and line number of the first differing bytes
   type_all_diffs
     to print the (decimal) byte numbers and (octal) values of all differing bytes
   type_status
...
usage ()
{
  printf ("Usage: %s [OPTION]... FILE1 [FILE2 [RANGE1 [RANGE2]]]\n",
	  program_name);
  printf ("%s", "\
  -c  --print-chars  Output differing bytes as characters.\n\
  -i N  --ignore-initial=N  Ignore differences in the first N bytes of input.\n\
  -l  --verbose  Output byte numbers and codes of all differing bytes.\n\
  -s  --quiet  --silent  Output nothing; yield exit status only.\n\
  -v  --version  Output version info.\n\
  --help  Output this help.\n\
RANGE is a range of bytes in either file, and can be specified as follows:
  M+N    skip M bytes then compare the next N bytes\n\
  M-N    compare bytes M through N (zero-based, since it's skipping M bytes)\n\
  M      skip M bytes and read until EOF\n\
  +N, -N read the first N bytes of the file\n\
All numbers can be expressed in decimal, hex or octal.\n\
     ...
     char *argv[];
{
  ...
  while ((c = getopt_long (argc, argv, "ci:lsv", long_options, 0))
	...
	break;
      ...
      case 'i':
	range[0][0] = range[1][0] = scan_off_t(&optarg);
	break;
  ...
  if (optind < argc && parse_range(range[0], argv[optind++]))
    try_help ("invalid byte range");

  if (optind < argc && parse_range(range[1], argv[optind++]))
    try_help ("invalid byte range");
  ...
  if (0 < same_file (&stat_buf[0], &stat_buf[1])
      && file_position (0, range[0][0]) == file_position (1, range[1][0]))
    exit (0);
  ...
  if (comparison_type == type_status
    ...
    {
      off_t s0 = stat_buf[0].st_size - file_position (0, range[0][0]);
      off_t s1 = stat_buf[1].st_size - file_position (1, range[1][0]);
...
cmp ()
{
  ...
  long char_number = range[0][0] + 1;
				/* Byte number (1...) in files of 1st difference. */
  ...
  for (i = 0; i < 2; i++)
    if (range[i][0] && file_position (i, range[i][0]) == -1)
      {
  ...
  do
	...
	error (2, errno, "%s", file[1]);
	 ...
	 simultaneously.  */
      ...
      /* Reached the end of one of the ranges, so we succeed. */
      if ( (range[0][1] != -1 && char_number >= range[0][1]) ||
	   (range[1][1] != -1 && char_number >= range[1][1]) )
	return 0;
...

/* Position file I to `pos' bytes from its initial position,
     ...
     off_t pos;
{
  ...
  if (! positioned[i] || position[i] != pos)
    {
      positioned[i] = 1;
      position[i] = lseek (file_desc[i], pos, SEEK_CUR);
    }
  return position[i];
}

/* scan_off_t(): sort of a specialized sscanf() that reads "%i" (dec,
 * hex, or oct) into an off_t.  This is in case (a) `off_t' is a bigger
 * type than `long' and (b) our compiler doesn't like `long long'.
 * Returns the integer (sorry, the "off_t") and leaves the string
 * pointer just past the end of the number it read.
 */
static off_t
scan_off_t (str)
     char **str;
...
/* parse_range: given a char* and an off_t[2], return a byte range in
 * the off_t[2] in the form {start,nbytes}.  For example, given
 * "19-33", return {19,15}.  Given "22+3" return {22,3}.  Given "3" or
 * "3-", return {3,-1}.  Given "+4" return {0,4}.
 */
static int
parse_range (offsets, opt)
     off_t offsets[];
     char const *opt;
{
  char *o = (char *)opt;
  offsets[0] = scan_off_t(&o);
  offsets[1] = -1;
  ...
  case '-': case ',':
    if (*o)
      offsets[1] = scan_off_t(&o) - offsets[0] + 1;
    break;

  case '+':
    if (*o)
      offsets[1] = scan_off_t(&o);
    break;
