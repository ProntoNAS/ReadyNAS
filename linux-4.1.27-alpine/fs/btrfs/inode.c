				  ...
				  int pgoff, u64 start, size_t len)
{
	...
	__MDCS((void *)&csum_expected, (void *)&csum, 4, len,
		start, BTRFS_I(inode)->root->anon_dev, btrfs_ino(inode));
...
#endif
#ifdef CONFIG_BTRFS_FIEMAP_CHECKSUM
extern int extent_fiemap_checksum(struct inode *inode,
				struct fiemap_extent_info *fieinfo,
				__u64 start, __u64 len,
				get_extent_t *get_extent);
#endif
...
{
	int	ret;
#if defined(CONFIG_BTRFS_FIEMAP_PHYSICAL) ||	\
	defined(CONFIG_BTRFS_FIEMAP_CHECKSUM)
	unsigned int extra = fieinfo->fi_flags &
				(FIEMAP_FLAG_PHYSICAL | FIEMAP_FLAG_CHECKSUM);

	fieinfo->fi_flags &= ~(FIEMAP_FLAG_PHYSICAL | FIEMAP_FLAG_CHECKSUM);
#endif
	...
	switch (extra) {
#if defined(CONFIG_BTRFS_FIEMAP_PHYSICAL)
	case FIEMAP_FLAG_PHYSICAL:
		return extent_fiemap_physical(inode, fieinfo, start, len,
					btrfs_get_extent_fiemap);
#endif
#if defined(CONFIG_BTRFS_FIEMAP_CHECKSUM)
	case FIEMAP_FLAG_CHECKSUM:
		return extent_fiemap_checksum(inode, fieinfo, start, len,
					btrfs_get_extent_fiemap);
#endif
	case 0:
		break;
