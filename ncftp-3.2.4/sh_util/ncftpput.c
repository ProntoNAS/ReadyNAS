...
main(int argc, char **argv)
{
	...
	GetoptReset(&opt);
	while ((c = Getopt(&opt, argc, argv, "P:u:j:p:h:e:d:U:t:mar:RvVf:o:AT:S:EFcCyZzDbB:W:X:Y:I:")) > 0) {
		if (c == 'b') {
			batchmode++;
		}
	}
	...
	if (batchmode > 0) {
		GetoptReset(&opt);
		while ((c = Getopt(&opt, argc, argv, "P:u:j:p:h:e:d:U:t:mar:RvVf:AT:S:EFcCyZzDbB:W:X:Y:I:")) > 0) switch(c) {
	...
	GetoptReset(&opt);
	while ((c = Getopt(&opt, argc, argv, "P:u:j:p:h:e:d:U:t:mar:RvVf:o:AT:S:EFcCyZzDbB:W:X:Y:I:")) > 0) switch(c) {
		case 'P':
		case 'u':
		case 'j':
		case 'p':
	 ...
	 */
	GetoptReset(&opt);
	while ((c = Getopt(&opt, argc, argv, "P:u:j:p:h:e:d:t:aRTr:vVf:o:ADzZEFbcCB:W:X:Y:I:")) > 0) switch(c) {
		case 'P':
