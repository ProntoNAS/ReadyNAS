 ...
 * 20080324 - Justin Maggard <justin.maggard@netgear.com> - Ignore md/dm devices, and be quiet by default.
 */
...
int quiet = 1;		/* 1: suppress all warnings */
/* IA-64 gcc spec file currently does -DLinux... */
#undef Linux
int Linux = 1;		/* 1: suppress warnings irrelevant for Linux */
...
is_ide_cdrom_or_tape(char *device) {
	...
	if ( !strncmp("/dev/md", device, 7) ||
	     !strncmp("/dev/hda", device, 8) ||
	     !strncmp("/dev/dm-", device, 8) )
		return 1;
