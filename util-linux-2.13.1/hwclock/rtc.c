...
#endif
/* NETGEAR ReadyNAS: Alarm-related commands */
#ifndef RTC_ALM_READ
#define RTC_ALM_READ      _IOR('p', 0x08, struct linux_rtc_time)
#define RTC_ALM_SET       _IOW('p', 0x07, struct linux_rtc_time)
#define RTC_AIE_ON        _IO('p', 0x01)      /* Update alarm enable on */
#define RTC_AIE_OFF       _IO('p', 0x02)      /* Update alarm enable off */
#endif
...
static int enable_hardware_alarm_rtc(void) {
...
int ret;
  ...
  if (rtc_fd == -1) {
    ...
    if (rc == -1) {
      /* This rtc device doesn't have alarm functions. */
      printf(_("%s does not have alarm functions.\n"), rtc_dev_name);
    }
    close(rtc_fd);
  }
  return 0;
...
static int disable_hardware_alarm_rtc(void) {
int rtc_fd;  /* File descriptor of /dev/rtc */
int ret;
...
static int read_hardware_alarm_rtc(struct tm *tm) {
	int rtc_fd;
	int rc = -1;
	char *ioctlname;
	...
	/* Read the RTC alarm, return answer via tm */
	ioctlname = "RTC_ALM_READ";
	rc = ioctl(rtc_fd, RTC_ALM_READ, tm);
	if (rc == -1) {
		perror(ioctlname);
		fprintf(stderr, _("ioctl() to %s to read alarm failed.\n"),
			rtc_dev_name);
		exit(5);
	}
...
static int set_hardware_alarm_rtc(const struct tm *new_broken_time) {
	int rc = -1;
	int rtc_fd;
	char *ioctlname;
	...
	ioctlname = "RTC_ALM_SET";
	rc = ioctl(rtc_fd, RTC_ALM_SET, new_broken_time);

	if (rc == -1) {
		perror(ioctlname);
		fprintf(stderr, _("ioctl() to %s to set alarm failed.\n"),
			rtc_dev_name);
		exit(5);
	}
...
static struct clock_ops rtc = {
	...
	read_hardware_alarm_rtc,
	set_hardware_alarm_rtc,
	enable_hardware_alarm_rtc,
	disable_hardware_alarm_rtc,
