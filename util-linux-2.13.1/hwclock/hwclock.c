...
static void
read_hardware_alarm(const bool universal, bool *valid_p, time_t *systime_p){
  struct tm tm;

  ur->read_hardware_alarm(&tm);
...
static void
set_hardware_alarm(const time_t settime, 
                   const bool universal) {
  struct tm new_broken_time;  

  if (universal)
	  new_broken_time = *gmtime(&settime);
  else
	  new_broken_time = *localtime(&settime);

  ur->set_hardware_alarm(&new_broken_time);
}
...
manipulate_clock(const bool show, const bool adjust, const bool noadjfile,
                 ...
                 const bool ashow, const bool aset,
                 const bool aenable, const bool adisable,
...
----------------------------------------------------------------------------*/
    ...
    if (!noadjfile && (adjust || set || aset || systohc || (!utc && !local_opt))) {
	...
	adjtime.dirty = TRUE;
...

      {
        ...
        read_hardware_alarm(universal, &aclock_valid, &aclocktime); 

        if (show) {
          display_time(hclock_valid, hclocktime,
                       time_diff(read_time, startup_time));
        } else if (ashow) {
          display_time(aclock_valid, aclocktime, 0);
        } else if (set) {
          ...
          set_hardware_alarm(set_time, universal);
        } else if (aenable) {
          ur->enable_hardware_alarm();
        } else if (adisable) {
          ur->disable_hardware_alarm();
        } else if (adjust) {
...
usage( const char *fmt, ... ) {
  ...
  fprintf( usageto, _(
    ...
    "  --ashow       read hardware alarm and print result\n"
    "  --aset        set the rtc alarm to the time given with --date\n"
    "  --aenable     enable alarm\n"
    "  --adisable    disable alarm\n"
...
main(int argc, char **argv) {
	...
	bool ashow, aset, aenable, adisable;
	char *date_opt;
	...
	ashow = aset = aenable = adisable = FALSE;
	date_opt = NULL;

	while ((c = getopt_long (argc, argv, "?hvVDarsuwRL10AJSFf:", longopts, NULL))
	       != -1) {
		switch (c) {
		case 'D':
			debug = TRUE;
	...
	if (show + set + systohc + hctosys + adjust + getepoch + setepoch + ashow + aset + aenable + adisable > 1){
	...
	if (!(show | set | systohc | hctosys | adjust | getepoch | setepoch | ashow | aset | aenable | adisable))
	...
	if (getuid() == 0)
		...
		if (set || hctosys || systohc || adjust || aset) {
			fprintf(stderr,
	...
	rc = manipulate_clock(show, adjust, noadjfile, set, set_time,
				hctosys, systohc, startup_time,
				ashow, aset, aenable, adisable,
				utc,
				local_opt, testing);
	hwclock_exit(rc);
