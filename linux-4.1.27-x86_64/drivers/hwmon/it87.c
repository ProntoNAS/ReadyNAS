...
#define IT87_REG_SLOPE(nr)     (0x64 + (nr) * 8)
#define IT87_REG_TOLERANCE(nr) (0x66 + (nr) * 8)
...
#define FEAT_TACH_CLOSED_LOOP	(1 << 13)
...
static const struct it87_devices it87_devices[] = {
	...
	[it8732] = {
		...
		.features = FEAT_NEWER_AUTOPWM | FEAT_16BIT_FANS
		  | FEAT_TEMP_OFFSET | FEAT_TEMP_OLD_PECI | FEAT_TEMP_PECI
		  | FEAT_10_9MV_ADC | FEAT_IN7_INTERNAL | FEAT_TACH_CLOSED_LOOP,
		.peci_mask = 0x07,
		.old_peci_mask = 0x02,	/* Actually reports PCH */
	},
	[it8771] = {
		...
		char *buf)
{
	...
	struct it87_data *data = it87_update_device(dev);
	return sprintf(buf, "%d\n",
		       pwm_from_reg(data, data->pwm_duty[nr]) * 16);
}
		...
		const char *buf, size_t count)
{
	...
	it87_write_value(data, IT87_REG_PWM_DUTY(nr),
			 data->pwm_duty[nr]);
	it87_write_value(data, IT87_REG_SLOPE(nr),
			 0x8f);
	it87_write_value(data, IT87_REG_TOLERANCE(nr),
			 tolerance);
	mutex_unlock(&data->update_lock);
	return count;
}
		...
		const char *buf, size_t count)
{
	...
	superio_enter();
	superio_select(PME);
	superio_outb(IT87_EUP_REG, (int)val);
	superio_exit();
...
static const struct attribute *it87_attributes_fan_target[] = {
...
static void it87_remove_files(struct device *dev)
{
	...
	for (i = 0; i < 6; i++) {
		...
		if (i < 3 && has_tach_closed_loop(data))
			sysfs_remove_file(&dev->kobj,
					  it87_attributes_fan_target[i]);
	}
	for (i = 0; i < 3; i++) {
...
#ifdef CONFIG_GPIO_IT8732
#include <linux/gpio.h>

#define IT8732_SIMPLEIO_BASE_ADDR	0x62
#define IT8732_GPIO_POLARITY_BASE	0xb0
#define IT8732_GPIO_SIMPLEIO_BASE	0xc0
#define IT8732_GPIO_DIRECTION_BASE	0xc8
#define IT8732_GPIO_EXT_ROUTING0	0xe2
#define IT8732_GPIO_EXT_ROUTING_IRQ	0xe4

static u16 it8732_gpio_base;

static int it8732_gpio_get(struct gpio_chip *gc, unsigned int gpio_num)
{
	u8 mask = 1 << (gpio_num % 8);
	u16 reg = gpio_num / 8 + it8732_gpio_base;

	return !!(inb(reg) & mask);
}

static void it8732_gpio_set(struct gpio_chip *gc,
					unsigned int gpio_num, int val)
{
	u8 mask = 1 << (gpio_num % 8), cval;
	u16 reg = gpio_num / 8 + it8732_gpio_base;
...
static void superio_set_mask(u8 mask, u16 reg)
{
	u8 cval = superio_inb(reg);
	u8 nval = cval | mask;

	if (cval != nval);
		superio_outb(reg, nval);
}

static void superio_clr_mask(u8 mask, u16 reg)
{
	u8 cval = superio_inb(reg);
	u8 nval = cval & ~mask;
...
static int it8732_gpio_direction(struct gpio_chip *gc,
				unsigned int gpio_num, bool out)
{
	u8 mask = 1 << (gpio_num % 8);
	u8 chip_num = gpio_num / 8;
	struct it87_data *data = dev_get_drvdata(&pdev->dev);
	void (*masker)(u8 mask, u16 reg) = out	? superio_set_mask
						: superio_clr_mask;

	if (44 <= gpio_num && gpio_num < 48)
		return -EINVAL;
	...
	superio_select(GPIO);
	/* Enable simple I/O. */
	superio_set_mask(mask, chip_num + IT8732_GPIO_SIMPLEIO_BASE);
	/* Set direction. */
	masker(mask, chip_num + IT8732_GPIO_DIRECTION_BASE);
	superio_exit();
...
static int it8732_gpio_direction_out(struct gpio_chip *gc,
				unsigned int gpio_num, int val)
{
	int err = it8732_gpio_direction(gc, gpio_num, true);
	if (!err)
		it8732_gpio_set(gc, gpio_num, val);
	return err;
}

static int it8732_gpio_direction_in(struct gpio_chip *gc, unsigned int gpio_num)
{
	return it8732_gpio_direction(gc, gpio_num, false);
}

#include <linux/interrupt.h>
#include "it87-gpio.h"
static irqreturn_t (*it8732_gpio_irq_cb[16])(int, void *);

static irqreturn_t it8732_gpio_interrupt(int irq, void *dev_id)
{
	irqreturn_t ret = IRQ_HANDLED;

	if (it8732_gpio_irq_cb[irq])
		ret = it8732_gpio_irq_cb[irq](irq, dev_id);
	return ret;
}

static int it8732_gpio_control(struct gpio_chip *gc,
				unsigned int cmd, void *arg)
{
	unsigned long iarg = (unsigned long)arg;
	u8 gpio_num = iarg & 0xff;
	u8 chip_num = gpio_num / 8;
	u8 chip_off = gpio_num % 8;
	u8 mask = 1 << chip_off;
	int err = 0;
	u16 reg;
	struct it87_data *data = dev_get_drvdata(&pdev->dev);
	...
	switch (cmd) {
	case IT8732_CMD_GPIO_IRQ_EXT_ROUTING:
		/* There are invalid IDs in location mapping (Table 8-17). */
		switch (gpio_num) {
		case 3:
		case 17:
		case 18:
		...
		if (err)
			break;
		/* iarg == (ROUTE# << 16) | GPIO#) */
		reg = (iarg >> 16) + IT8732_GPIO_EXT_ROUTING0;
		/* ~0UL for invalidating. */
		mask = (iarg == ~0UL) ? 0 : (0x40 | (gpio_num + 0x8));
		superio_outb(reg, mask);
		break;
	case IT8732_CMD_GPIO_POLARITY:
		if (gpio_num >= 40) {
			err = -EINVAL;
			break;
		}
		/* iarg == (ACTIVELOW << 16) | GPIO#) */
		reg = chip_num + IT8732_GPIO_POLARITY_BASE;
		if (iarg & (1 << 16))
			superio_set_mask(mask, reg);
		else
			superio_clr_mask(mask, reg);

		break;
	case IT8732_CMD_GPIO_IRQ_GET:
		/* [reg] == (IRQ#1 << 4) | IRQ#0) */
		reg = IT8732_GPIO_EXT_ROUTING_IRQ;
		err = superio_inb(reg);
		break;
	case IT8732_CMD_GPIO_IRQ_SET:
		/* iarg == (IRQ#1 << 4) | IRQ#0) */
		/* There are invalid IDs in lvel mapping (Table 8-16). */
		switch (iarg & 0xf0) {
		case 0xf0:
		case 0xe0:
		case 0xd0:
		...
		if (err)
			break;
		reg = IT8732_GPIO_EXT_ROUTING_IRQ;
		superio_outb(reg, iarg);
		break;
	default:
		/* cmd == IRQ_REGISTER | IRQ#
		   arg == irqreturn_t(*handler)(int irq, void *dev_id)
		   Locking and superio_enter/exit is performed
		   out side of handler.
		 */
		if (IS_IT8732_CMD_GPIO_IRQ_REGISTER(cmd)) {
			int irq = IRQ_IT8732_CMD_GPIO_IRQ_REGISTER(cmd);

			if (irq >= ARRAY_SIZE(it8732_gpio_irq_cb))
				err = -EINVAL;
			else if (arg) {
				char name[20];
				sprintf(name, "gpio_it8732_%u", irq);
				if ((err = request_irq(irq,
						it8732_gpio_interrupt,
						0,
						name,
						NULL)))
					break;
				it8732_gpio_irq_cb[irq] = arg;
			} else {
				free_irq(irq, NULL);
			}
		}
...
static struct gpio_chip it8732_gpio_chip = {
	.label			= "gpio_it8732",
	.owner			= THIS_MODULE,
	.get			= it8732_gpio_get,
	.direction_input	= it8732_gpio_direction_in,
	.set			= it8732_gpio_set,
	.direction_output	= it8732_gpio_direction_out,
	.base	= -1,
	.ngpio	= 58,
	.control		= it8732_gpio_control,
};

static int __init it8732_gpio_init(void)
{
	struct it87_data *data = dev_get_drvdata(&pdev->dev);
	int err;
	...
	superio_enter();
	superio_select(GPIO);
	it8732_gpio_base = superio_inw(IT8732_SIMPLEIO_BASE_ADDR);
	superio_exit();
	mutex_unlock(&data->update_lock);

	err = gpiochip_add(&it8732_gpio_chip);
	return err;
}

static void __exit it8732_gpio_exit(void)
{
	gpiochip_remove(&it8732_gpio_chip);
}
#endif
...
#ifdef CONFIG_GPIO_IT8732
	if (sio_data.type == it8732 &&
		(err = it8732_gpio_init())) {
