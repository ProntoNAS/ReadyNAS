...
static const struct dev_pm_ops *good_pm(struct device *dev)
{
#define GOOD_PM(x) do {if (dev->x && dev->x->pm) return dev->x->pm;} while(0)
	GOOD_PM(bus);
	GOOD_PM(class);
	GOOD_PM(driver);
	GOOD_PM(type);
...
static ssize_t suspend_store(struct device *dev, struct device_attribute *attr,
			    const char *buf, size_t count)
{
	const struct dev_pm_ops *pm = good_pm(dev);
	
	if (pm)
		pm->suspend(dev);
	return count;
}
static DEVICE_ATTR_IGNORE_LOCKDEP(suspend, S_IWUSR, NULL, suspend_store);
...
static DEVICE_ATTR_IGNORE_LOCKDEP(poweroff, S_IWUSR, NULL, poweroff_store);
...
static DEVICE_ATTR_IGNORE_LOCKDEP(poweron, S_IWUSR, NULL, poweron_store);
