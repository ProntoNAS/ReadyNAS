...
static int __extent_fiemap(struct inode *inode,
		struct fiemap_extent_info *fieinfo,
		__u64 start, __u64 len, get_extent_t *get_extent, int what)
{
	int ret = 0;
	u64 off = start;
	u64 max = start + len;
...
#ifdef CONFIG_BTRFS_FIEMAP_CHECKSUM
		if (what == _EX_CSUM)
			ret = extent_fiemap_ex2csum(inode, fieinfo,
							em_start, disko,
							em_len, flags);
		else
#endif
...
{
	return __extent_fiemap(inode, fieinfo, start, len, get_extent,
				_EX_PHYS);
}
#endif
#ifdef CONFIG_BTRFS_FIEMAP_CHECKSUM
int extent_fiemap_checksum(struct inode *inode,
		struct fiemap_extent_info *fieinfo,
		__u64 start, __u64 len, get_extent_t *get_extent)
{
	return __extent_fiemap(inode, fieinfo, start, len, get_extent,
...
int extent_fiemap(struct inode *inode, struct fiemap_extent_info *fieinfo,
		__u64 start, __u64 len, get_extent_t *get_extent)
{
	return __extent_fiemap(inode, fieinfo, start, len, get_extent, 0);
}
