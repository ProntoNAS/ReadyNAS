                      ...
                      apr_pool_t *ptemp, server_rec *s)
{
...
#if 1 //JM: Shorten startup time by skipping the first init.
    void *data = NULL;
    const char *userdata_key = "modperl_hook_init";
    ...
    modperl_sys_init();
#if 0 //JM: Don't ever try to clean anything up, since we load into the process memory pool.
...
#endif
#if 1 //JM: Skip the first init to help startup time.
    if( modperl_restart_count() < 1 ) {
	return OK;
    }
#endif
