            ...
            ipp_attribute_t *uri)	/* I - URI of printer */
{
  ...
  */
  ...
  if ((attr = ippFindAttribute(con->request, "device-uri", IPP_TAG_URI)) != NULL)
  {
    ...
    */
    ...
    LogMessage(L_DEBUG2, "add_printer: method = \"%s\"", method);

    if ((strcmp(method, "http") == 0) && 
	(strncmp(attr->values[0].string.text, "http:", 5) != 0))
    {
     /*
      * No method specified in device URI; httpSeparate puts "http"
      * as the method for a default.
      */
  ...
  */
  ...
  if (printer->device_uri[0] == '\0')
    strcpy(printer->device_uri, "test:/dev/null");
