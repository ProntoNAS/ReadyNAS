/*
 * "$Id: job.c,v 1.146 2002/02/11 23:19:12 mike Exp $"
 *
         ...
         printer_t *printer)	/* I - Printer to print job */
{
  ...
  */
  ...
  for (i = 0, slot = 0; i < num_filters; i ++)
  {
    ...
    if (i < (num_filters - 1) ||
	strncmp(printer->device_uri, "test:", 5) != 0)
      pipe(filterfds[slot]);
    else
    {
      filterfds[slot][0] = -1;
      if (strncmp(printer->device_uri, "test:/dev/", 10) == 0)
  ...
  */
  ...
  if (strncmp(printer->device_uri, "test:", 5) != 0)
  {
...
/*
 * End of "$Id: job.c,v 1.146 2002/02/11 23:19:12 mike Exp $".
 */
