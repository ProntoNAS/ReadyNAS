 ...
 * 
 * $Id: state.c,v 1.23 2010-08-06 21:48:28 jmaggard Exp $
 *
...
static nfd_state_t check_io(disk_info_t di, disk_stat_t ds, int interval)
{
	...
	if (new_access) {
		spundown = 0;
		for (; di_w; di_w=di_w->next) {
			switch (di_w->state) {
			case DISK_STATE_SPINNING:
				di_w->time_left=timeout_get(di_w->timeouts);
				break;
			case DISK_STATE_STOPPED:
				di_w->time_left=timeout_get(di_w->timeouts);
				di_w->state=DISK_STATE_SPINNING;

				now = time(NULL);
				if (now > di_w->spundown_at)
					delta = now - di_w->spundown_at;
				else
					delta = di_w->spundown_at-now;
				...
				sync_disk(di_w);
				rw_stopped-=di_w->is_rw;
				rw_spinning+=di_w->is_rw;
				break;
			default:
				BUG("Illegal disk state %d on %s (%d, %d)", 
					di_w->state, di_w->name, di_w->major, di_w->minor);
			}
		}
		if (vm_setting) {
			vm_setting = 0;
		...
		now = 0;
		for (; di_w; di_w=di_w->next) {
			if (di_w->time_left > now)
				now = di_w->time_left;
		}
		if (((now <= interval) || (now < 30)) && !vm_setting) {
			read_ahead_for_cron();
			vm_setting = 1;
			adjust_vm_settings();
		}
...
//#ifdef __i386__
	...
	int chan = -1;
#endif
	system("/usr/bin/killall -USR2 monitor_enclosure");
