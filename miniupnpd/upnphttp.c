/* $Id: upnphttp.c,v 1.2 2007-01-16 03:42:47 jmaggard Exp $ */
/* Project :  miniupnp
 ...
 *  - Just removed some syslog messages to reduce clutter.
 */
#include <stdlib.h>
#include <unistd.h>
#include <stdio.h>
...
static void ProcessHTTPPOST_upnphttp(struct upnphttp * h)
{
	if((h->req_buflen - h->req_contentoff) >= h->req_contentlen)
	{
		if(h->req_soapAction)
		{
			...
			//JM syslog(LOG_INFO, "SOAPAction: %.*s",
		    	//JM    h->req_soapActionLen, h->req_soapAction);
			ExecuteSoapAction(h, 
				h->req_soapAction,
				h->req_soapActionLen);
		}
		else
		{
			...
			//JM syslog(LOG_INFO, "No SOAPAction in HTTP headers");
...
static void ProcessHttpQuery_upnphttp(struct upnphttp * h)
{
	...
	for(i = 0; i<15 && *p != '\r'; i++)
		HttpVer[i] = *(p++);
	HttpVer[i] = '\0';
	//JM syslog(LOG_INFO, "HTTP REQUEST : %s %s (%s)",
	//JM        HttpCommand, HttpUrl, HttpVer);
	ParseHttpHeaders(h);
