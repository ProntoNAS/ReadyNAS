...
static	int	minsupplies = 1, sleepval = 5, deadtime = 20;
...
static void ups_low_batt(utype *ups)
{
	...
	else if (flag_isset(ups->status, ST_ONLINE)) {  /* don't warn if it's in OL LB state */
		debug("ups_low_batt(%s) (online - don't warn)\n", ups->sys);
		return;
	}
...
void update_readynas_log(utype *ups)
{
	const char *query[4];
	const char *var[4];
	char **answer;
	...
	var[0] = "ups.mfr";
	var[1] = "ups.model";
	var[2] = "battery.charge";
	var[3] = "battery.runtime";
	query[0] = "VAR";
	query[1] = ups->upsname;

	log = fopen("/ramfs/ups.info", "w");

	for( i=0; i < 4; i++ ) {
		query[2] = var[i];

		if( upscli_get(&ups->conn, numq, query, &numa, &answer) < 0 ) {
			if( i == 1 ) {
				fprintf(log, "ERROR!!");
			}
			else if( i > 1 ) {
		...
		if( i == 0 )
			fprintf(log, "%s ", answer[3]);
		else if( i != 3 )
			fprintf(log, "%s!!", answer[3]);
		else {
			int min = (atoi(answer[3])/60);
			fprintf(log, "%d", min);
		}
	}
	fprintf(log, "\n");
