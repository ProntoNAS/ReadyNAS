 ...
 * 1.3.10: --delete-chain now takes optional arg, like -X.
 *        [ Found by Lothar Gerlach ]
 *        Maximize-throughput not Minimize-throughput for TOS!
 *        [ Found by Adam Kumiszcza ]
 *        Warn about `-i !eth0' and `-i eth0*' cases.
 *        [ Thanks to John Martinez ]
 *        ICMP numbers printed in -h icmp
 *        [ Thanks to Brett Eldridge ]
 *        Fixed wildcard interfaces getting extra + with `-A input -v'.
 *        [ Thanks to Howard Lowndes ]
 */
#include <stdio.h>
#include <stdarg.h>
#include <errno.h>
...
static char package_version[] = "ipchains 1.3.10, 1-Sep-2000";
...
static char *addr_to_host(const struct in_addr *);
static struct in_addr *network_to_addr(const char *);
static char *addr_to_network(const struct in_addr *);
static char *addr_to_anyname(const struct in_addr *);
static struct in_addr *dotted_to_addr(const char *);
static char *addr_to_dotted(const struct in_addr *);
static char *mask_to_dotted(const struct in_addr *);
...
static void print_firewall(FILE *, const struct ip_fwuser *,
			   int, __u64, __u64, int);
...
const struct icmp_names icmp_codes[] = {
	{ "0    0    ", "echo-reply", 0, 0, 0xFFFF, TRUE },
	{ "", "pong", 0, 0, 0xFFFF, TRUE },
	{ "3         ", "destination-unreachable", 3, 0, 0xFFFF, FALSE },
	{ "     0      ", "network-unreachable", 3, 0, 0, TRUE },
	{ "     1      ", "host-unreachable", 3, 1, 1, TRUE },
	{ "     2      ", "protocol-unreachable", 3, 2, 2, TRUE },
	{ "     3      ", "port-unreachable", 3, 3, 3, TRUE },
	{ "     4      ", "fragmentation-needed", 3, 4, 4, TRUE },
	{ "     5      ", "source-route-failed", 3, 5, 5, TRUE },
	{ "     6      ", "network-unknown", 3, 6, 6, TRUE },
	{ "     7      ", "host-unknown", 3, 7, 7, TRUE },
	{ "     9      ", "network-prohibited", 3, 9, 9, TRUE },
	{ "     10     ", "host-prohibited", 3, 10, 10, TRUE },
	{ "     11     ", "TOS-network-unreachable", 3, 11, 11, TRUE },
	{ "     12     ", "TOS-host-unreachable", 3, 12, 12, TRUE },
	{ "     13     ", "communication-prohibited", 3, 13, 13, TRUE },
	{ "     14     ", "host-precedence-violation", 3, 14, 14, TRUE },
	{ "     15     ", "precedence-cutoff", 3, 15, 15, TRUE },

	{ "4    0    ", "source-quench", 4, 0, 0xFFFF, TRUE },

	{ "5         ", "redirect", 5, 0, 0xFFFF, FALSE },
	{ "     0      ", "network-redirect", 5, 0, 0, TRUE },
	{ "     1      ", "host-redirect", 5, 1, 1, TRUE },
	{ "     2      ", "TOS-network-redirect", 5, 2, 2, TRUE },
	{ "     3      ", "TOS-host-redirect", 5, 3, 3, TRUE },

	{ "8    0    ", "echo-request", 8, 0, 0xFFFF, TRUE },
	{ "", "ping", 8, 0, 0xFFFF, TRUE },

	{ "9    0    ", "router-advertisement", 9, 0, 0xFFFF, TRUE },
	
	{ "10   0    ", "router-solicitation", 10, 0, 0xFFFF, TRUE }, 

	{ "11        ", "time-exceeded", 11, 0, 0xFFFF, FALSE }, 
	{ "", "ttl-exceeded", 11, 0, 0xFFFF, FALSE },
	{ "     0      ", "ttl-zero-during-transit", 11, 0, 0, TRUE },
	{ "     1      ", "ttl-zero-during-reassembly", 11, 1, 1, TRUE },

	{ "12        ", "parameter-problem", 12, 0, 0xFFFF, FALSE },
	{ "     0      ", "ip-header-bad", 12, 0, 0, TRUE },
	{ "     1      ", "required-option-missing", 12, 1, 1, TRUE },

	{ "13   0    ", "timestamp-request", 13, 0, 0xFFFF, TRUE },

	{ "14   0    ", "timestamp-reply", 14, 0, 0xFFFF, TRUE },

	{ "17   0    ", "address-mask-request", 17, 0, 0xFFFF, TRUE },

	{ "18   0    ", "address-mask-reply", 18, 0, 0xFFFF, TRUE }
};
...
static unsigned short
parse_interface(const char *ifstring, char *vianame, int warnings)
{
	unsigned int i;
	int ret;
...
addr_to_host(const struct in_addr *addr)
{
	...
	if ((host = gethostbyaddr((const char *) addr,
...
print_firewall(FILE *fp,
	       const struct ip_fwuser *fw, int num,
...
#define FMT(tab,notab) ((format & FMT_NOTABLE) ? notab : tab)
	...
	if (format & FMT_VIA) {
		...
		strcpy(buf, fw->ipfw.fw_vianame);
		if (fw->ipfw.fw_flg & IP_FW_F_WILDIF && buf[0])
			strcat(buf, "+");
		fprintf(fp, FMT("%-10.16s ","via %.16s "),
			buf[0] ? buf : ((format & FMT_NUMERIC) ? "*" : "any"));
	}
...
exit_printicmphelp()
{
	...
	for (i = 0; i < sizeof(icmp_codes)/sizeof(struct icmp_names); i++) {
		if (icmp_codes[i].columns[0])
			printf("\n%s %s",
			       icmp_codes[i].columns, icmp_codes[i].name);
		else printf(" (%s)", icmp_codes[i].name);
	}
	printf("\n");
	exit(0);
}
