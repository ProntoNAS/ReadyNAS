...
static const char * const cmd_rescue_search_leaf_usage[] = {
	"btrfs rescue search-leaf [options] <-o owner | -g generation> <device>",
	"Search for a particular leaf by scanning the entire device.",
	"",
	"-o	Owner objectid",
	"-g	Generation",
	"-n	Node size",
	"-s	Starting byte number",
	"-v	Verbose mode",
	"-h	Help",
	NULL
};

static int cmd_rescue_search_leaf(int argc, char *argv[])
{
	int ret = 1;
	char *file;
	int verbose = 0;
	...
	while (1) {
		int c = getopt(argc, argv, "o:g:n:s:vh");
		if (c < 0)
			break;
		switch (c) {
		case 'o':
			owner = strtoull(optarg, NULL, 0);
			break;
		case 'g':
			generation = strtoull(optarg, NULL, 0);
			break;
		case 'n':
			node_size = atoi(optarg);
			break;
		case 's':
			bytenr = strtoull(optarg, NULL, 0);
			break;
		case 'v':
			verbose = 1;
			break;
		case 'h':
		default:
			usage(cmd_rescue_search_leaf_usage);
		}
	}

	if (check_argc_exact(argc - optind, 1) || (!owner && !generation))
		usage(cmd_rescue_search_leaf_usage);
	...
	while (1) {
		...
		bytenr += node_size;
		if (verbose)
			fprintf(stdout, "\rChecking leaf at %llu", bytenr);
		len = pread(fd, &hdr, sizeof(hdr), bytenr);
		if (len <= 0)
			break;
		if (owner && hdr.owner != owner)
			continue;
		if (generation && hdr.generation != generation)
			continue;
		if (verbose)
			fputs("\n", stdout);
		fprintf(stdout, "Found matching leaf: owner %llu, gen %llu at %llu logical %llu\n",
			hdr.owner, hdr.generation, bytenr, hdr.bytenr);
		ret = 0;
	}
	close(fd);
	return ret;
...
static const char * const cmd_rescue_chunk_tree_scan_usage[] = {
	"btrfs rescue chunk-tree-scan [options] <device>",
	"Search for a useable chunk tree by scanning the entire device.",
	"",
	"-s	Starting byte number",
	"-v	Verbose mode",
	"-h	Help",
...
static int cmd_rescue_chunk_tree_scan(int argc, char *argv[])
{
	int ret = 1;
	char *file;
	int verbose = 0;
	...
	while (1) {
		...
		case 'h':
		default:
			usage(cmd_rescue_chunk_tree_scan_usage);
		}
	}
	...
	while (1) {
		if (verbose)
			fprintf(stdout, "\rChecking for chunk tree at %llu ...", bytenr);
		info = open_ctree_fs_info(file, 0, 0, bytenr, OPEN_CTREE_PARTIAL);
		if (info) {
			fprintf(stdout, "\nFound viable chunk tree at %llu !\n", bytenr);
			ret = 0;
			close_ctree_fs_info(info);
			break;
		}
		bytenr += 4096;
	}
	...
	rescue_cmd_group_usage, rescue_cmd_group_info, {
		...
		{ "fix-device-size", cmd_rescue_fix_device_size,
			cmd_rescue_fix_device_size_usage, NULL, 0},
                { "chunk-tree-scan", cmd_rescue_chunk_tree_scan, 
                        cmd_rescue_chunk_tree_scan_usage, NULL, 0},
                { "search-leaf", cmd_rescue_search_leaf,
                        cmd_rescue_search_leaf_usage, NULL, 0},
		NULL_CMD_STRUCT
	}
};
