...
*set-class* <path> <device>[:<class>]::
Set <device>'s "class value" for the filesystem identified by <path>.
Class value is used to determine chunk relocation strategy
in *btrfs-balance*'s "sweep" filtering.
+
<device> is either the underlying  physical device path such as /dev/sda1
or the decimal device ID.
+
<class> is a decimal class value to set.
If <class> numeral is preceded by "+" or "-,"
<device>'s class value is either incremented or decremented
by the numeral value accordingly.
+
If <class> is omitted, the current class value for <device> is printed.
