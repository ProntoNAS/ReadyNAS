...
static const char * const cmd_device_set_class_usage[] = {
	"btrfs device set-class [devid|device]:[+/-]<class> <path>",
	"Set class field on a device",
	NULL
};

static int cmd_device_set_class(int argc, char **argv)
{
	struct btrfs_ioctl_vol_args	args;
	int	fd, res, len, e;
	char	*flags, *path;
	...
	if (check_argc_exact(argc - optind, 2))
		usage(cmd_device_set_class_usage);
	...
	if (len == 0 || len >= BTRFS_VOL_NAME_MAX) {
		error("value too long (%s)", flags);
		return 1;
	}

	res = stat(path, &st);
	if (res < 0) {
		error("set-class: cannot stat %s: %s", path, strerror(errno));
		return 1;
	}
	if (!S_ISDIR(st.st_mode)) {
		error("set-class works on mounted filesystems and accepts only\n"
	...
	if (strchr(flags, ':'))
		printf("Setting class on '%s' to '%s'\n", path, flags);
	memset(&args, 0, sizeof(args));
	
	strncpy_null(args.name, flags);

	res = ioctl(fd, BTRFS_IOC_SET_DEV_TYPE, &args);
	e = errno;
	close_file_or_dir(fd, dirstream);
	if( res < 0) {
		error("unable to set-class '%s': %s", path, strerror(e));
		return 1;
	} else if (res > 0) {
		const char *err_str = btrfs_err_str(res);

		if (err_str)
			error("set-class of '%s' failed: %s", path, err_str);
		else
			error("set-class of '%s' failed: unknown error %d",
				path, res);
		return 1;
	} else if (!strchr(flags, ':'))
		printf("Class on '%s' is '%s'\n", path, args.name);
	return 0;
}
...
const struct cmd_group device_cmd_group = {
	device_cmd_group_usage, device_cmd_group_info, {
		...
		{ "set-class", cmd_device_set_class, cmd_device_set_class_usage,
			NULL, 0 },
