			...
			char **sweep)
{
	...
	for (this_char = strtok_r(filters, ",", &save_ptr);
		...
		} else if (!strcmp(this_char, "sweep")) {
			if (!value || !*value) {
				error("the sweep filter requires an argument");
				return 1;
			}
			*sweep = strdup(value);
		} else {
...
static int cmd_balance_start(int argc, char **argv)
{
	...
	int i;
	char *data_sweep = NULL, *meta_sweep = NULL, *sys_sweep = NULL;
	...
	while (1) {
		...
		case 'd':
			...
			if (parse_filters(optarg, &args.data, &data_sweep))
				return 1;
			break;
		case 's':
			start_flags |= BALANCE_START_FILTERS;
			...
			if (parse_filters(optarg, &args.sys, &sys_sweep))
				return 1;
			break;
		case 'm':
			start_flags |= BALANCE_START_FILTERS;
			...
			if (parse_filters(optarg, &args.meta, &meta_sweep))
				return 1;
			break;
		case 'f':
			force = 1;
	...
	if (data_sweep ||sys_sweep || meta_sweep) {
		struct btrfs_ioctl_vol_args sweep_args;
		char *p = sweep_args.name;
		size_t len = 0;
		int fd;
		DIR *dirstream = NULL;
		int ret;

		if ((len = (data_sweep ? strlen(data_sweep) : 0) +
			(sys_sweep ? strlen(sys_sweep) : 0) +
			(meta_sweep ? strlen(meta_sweep) : 0) + 3) >
				sizeof sweep_args.name) {
			error("sweep filter strings too long");
			return 1;
		}
		...
		*p = '\0';
		...
		if (fd < 0)
			return 1;
		ret = ioctl(fd, BTRFS_IOC_BALANCE_SWEEP, &sweep_args);
		if (ret) {
			char *dp = sweep_args.name;
			char *sp = dp + strlen(dp) +1;
			char *mp = sp + strlen(sp) +1;

			fprintf(stderr, "Sweeping %s %s %s failed: %s\n",
				dp, sp, mp, strerror(errno));
			ret = errno;
		}
		return ret;
	}
