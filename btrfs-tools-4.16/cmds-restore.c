		     ...
		     const char *file)
{
	...
	if (ignore_errors && extreme > 2)
		path.skip_check_block = 2;
...
static int overwrite_ok(const char * path)
{
	...
	if (!ret) {
		if (overwrite || (S_ISREG(st.st_mode) && st.st_size == 0))
			return 2;
...
static int search_dir(struct btrfs_root *root, struct btrfs_key *key,
		      ...
		      int key_type, const regex_t *mreg)
{
	struct btrfs_path path;
	struct extent_buffer *leaf;
	...
	while (leaf) {
		...
		if (type == BTRFS_FT_REG_FILE) {
			...
			ret = search_dir(search_root, &location,
					 output_rootdir, dir, key_type, mreg);
			free(dir);
			if (ret) {
...
static int do_list_roots(struct btrfs_root *root)
{
	...
	while (1) {
		...
		if (found_key.type == BTRFS_ROOT_ITEM_KEY) {
		...
		else if (found_key.type == BTRFS_ROOT_REF_KEY) {
			...
			if (strncmp(namebuf, "snapshot", namelen)) {
				printf(" tree ");
				btrfs_print_key(&disk_key);
				printf(" name: %.*s\n", namelen, namebuf);
				fflush(stdout);
...
int cmd_restore(int argc, char **argv)
{
	...
	while (1) {
		...
		opt = getopt_long(argc, argv, "sSxviot:u:dmf:r:lDFXc", long_options,
					NULL);
		if (opt < 0)
			break;
	...
	ret = search_dir(root, &key, dir_name, "", BTRFS_DIR_INDEX_KEY, mreg);
	if (extreme)
		ret |= search_dir(root, &key, dir_name, "", BTRFS_DIR_ITEM_KEY,
				  mreg);
