...
system call gets interrupted. Different versions of Unix have different
defaults, and different ways of achieving this. For systems for which we
don't know what to do, just use the normal signal() function. */
...
Problems:
  ...
  fixed number, defined by MAXINTERFACES. This is in the config.h file and
...
{
struct V_ifconf ifc;
struct V_ifreq ifreq, *ifr;
int vs;
ip_address_item *yield = NULL;
ip_address_item *last = NULL;
ip_address_item  *next;
char *cp;
char buf[MAXINTERFACES*sizeof(struct V_ifreq)];
...
if ((vs = socket(FAMILY, SOCK_DGRAM, 0)) < 0)
  {
  #if HAVE_IPV6
  DEBUG(9) debug_printf("Unable to create IPv6 socket to find interface "
...
for (cp = buf; cp < buf + ifc.V_ifc_len; cp +=
    #ifdef HAVE_SA_LEN
  ...
  if (ifr->V_ifr_addr.V_family != AF_INET
  #if HAVE_IPV6
    && ifr->V_ifr_addr.V_family != AF_INET6
  #endif
    ) continue;
  ...
  next->next = NULL;
  (void)host_ntoa(-1, &ifreq.V_ifr_addr, next->address, NULL);
...
interfaces. Instead, we fake a local_interfaces="127.0.0.1" without setting it.
If local_interfaces is unset, the default behaviour is to listen to all
interfaces on a single socket. The only difference is that the interfaces will
not be recognized as local. Hence, the user should add further local IPs to
host_treat_as_local. However, setting local_interfaces is probably the best
approach. */
...
{
ip_address_item *yield = NULL;
char *my_local_interfaces = "127.0.0.1"; /* local_interfaces */

/* We know that local_interfaces is NULL when we come here from
host_find_interfaces(). Also we know that if we set it to something else it
will behave differently. In particular, it won't call us again, so this is not
a loop. */

local_interfaces = my_local_interfaces;
yield = host_find_interfaces();
