...
/* Eximon calls some functions from the main exim code, and these in turn
may call log_write in error cases. That just writes to stderr, but the log
module calls smtp_closedown() on panic crashes, so we must provide a dummy
function here. One of these days this should all be made much tidier, with
less inter-dependence. */

void smtp_closedown(char *message)
{
message = message;
}
...
switch(dialog_action)
  ...
  sk = (skip_item *)store_malloc(sizeof(skip_item) + (int)strlen(s));
  sk->next = queue_skip;
  queue_skip = sk;
  strcpy(sk->text, s);
...
static char *
numlock_modifiers(Display *display, char *buf)
{
XModifierKeymap *m;
int i, j;
char *ret = NULL;
...
for (i = 3; i < 8; i++)
  ...
  for (j = 0; j < m->max_keypermod; j++)
    ...
    if (XKeycodeToKeysym(display, m->modifiermap [i*m->max_keypermod + j], 0)
        == XK_Num_Lock)
      {
      sprintf(buf+strlen(buf), " Mod%d", i-2);
      ret = buf;
      }
    }
  }
...
char modbuf[] = " Mod1 Mod2 Mod3 Mod4 Mod5";
char *numlock;
...
log_display_buffer = (char *)store_malloc(log_buffer_size);
...
end of it. Save its inode so that we can detect when the file is switched to
another one. However, allow the monitor to start up without a log file (can
happen if no messages have been sent today.) */
...
if (log_file[0] != 0)
  ...
  if (LOG == NULL)
    {
    printf("*** eximon warning: can't open log file - will try periodically\n");
    }
  else
    {
    fseek(LOG, -8192, SEEK_END);
    fgets(big_buffer, big_buffer_size, LOG);
...
if (numlock != NULL) sprintf(big_buffer + (int)strlen(big_buffer),
...
sprintf(big_buffer + (int)strlen(big_buffer),
...
if (size_stripchart != NULL)
  ...
  if (size_stripchart_name == NULL)
    {
    size_stripchart_name = size_stripchart + (int)strlen(size_stripchart) - 1;
