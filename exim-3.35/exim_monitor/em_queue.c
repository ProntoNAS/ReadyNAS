...
#ifdef ANONYMIZE
static char *anon(char *s)
{
static char anon_result[256];
char *ss = anon_result;
...
dd = (dest_item *)store_malloc(sizeof(dest_item) + (int)strlen(name));
...
static queue_item *set_up(char *name, int dir_char)
{
int i, rc, save_errno;
...
if (sender_set_untrusted)
  ...
  if (sender_address[0] == 0)
    {
    q->sender = store_malloc((int)strlen(originator_login) + 6);
    sprintf(q->sender, "<> (%s)", originator_login);
    }
  else
    {
    q->sender = store_malloc((int)strlen(sender_address) +
      (int)strlen(originator_login) + 4);
    sprintf(q->sender, "%s (%s)", sender_address, originator_login);
    }
  }
else
  {
  q->sender = store_malloc((int)strlen(sender_address) + 1);
  strcpy(q->sender, sender_address);
  }

sender_address = NULL;

sprintf(buffer, "%s/input/%s/%s-D", spool_directory, message_subdir, name);
...
queue_item *find_queue(char *name, int action, int dir_char)
{
int first = 0;
...
if (tn != NULL)
  ...
  (void)find_dest(p, tn->name, dest_remove);
  }
}
...
sprintf(buffer, "%s/input/%s/%s-J", spool_directory, message_subdir, p->name);
jread = fopen(buffer, "r");
if (jread == NULL)
  {
  sprintf(buffer, "%s/input/%s/%s-H", spool_directory, message_subdir, p->name);
...
while (p != NULL)
  ...
  if (ddd != NULL || dd == NULL)
    ...
    text_showf(queue_widget, "%c%2d%c %s %s %-8s ",
      ...
      p->name,
      (p->sender == NULL)? "       " :
        (p->sender[0] == 0)? "<>     " : anon(p->sender));

    text_showf(queue_widget, "%s%s%s",
      (dd == NULL || dd->address[0] == '*')? "" : "<",
      (dd == NULL)? "" : anon(dd->address),
      (dd == NULL || dd->address[0] == '*')? "" : ">");
