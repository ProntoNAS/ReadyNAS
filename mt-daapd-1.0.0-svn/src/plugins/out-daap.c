/*
 * $Id: out-daap.c,v 1.4 2013-10-04 01:02:43 jmaggard Exp $
...
int plugin_auth(WS_CONNINFO *pwsc, char *username, char *password) {
    ...
    if (strncasecmp(uri,"/databases/1/containers/",24) == 0) {
        if (strncasecmp(uri+24,"add",3) == 0 ||
            strncasecmp(uri+24,"del",3) == 0 ||
            strncasecmp(uri+24,"edit",4) == 0)
            return pi_ws_matchesrole(pwsc,username,password,"admin");
    }
