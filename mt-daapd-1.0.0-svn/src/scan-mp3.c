/*
 * $Id: scan-mp3.c,v 1.1.1.1 2010-09-22 17:14:54 jmaggard Exp $
 *
...
static void scan_mp3_get_average_bitrate(IOHANDLE infile, SCAN_FRAMEINFO *pfi);
static int scan_mp3_is_numeric(char *str);
static void scan_mp3_get_frame_count(IOHANDLE infile, SCAN_FRAMEINFO *pfi);
static unsigned char* get_utf8_text(const int locale_index, const id3_ucs4_t* native_text);
static int getlocale(void);
    ...
    while((pid3frame=id3_tag_findframe(pid3tag,"",index))) {
        ...
        if(have_text) {
	        ...
	        int locale_index;
                ...
                } else if ((locale_index=getlocale())>=0) {
		    // if all asian locale, conversion use iconv
		    utf8_text=get_utf8_text(locale_index, native_text);
		} else {
...
#define LANG_MAP_SIZE (sizeof(iconv_map)/sizeof(char*)/2)
#define MAX_LINE 256

static int getlocale(void) {
  static int locale_index = -2;
  FILE *fin;
  char buffer[MAX_LINE];
  char *locale_str;

  if (locale_index==-2) {
    if ((fin=fopen("/etc/default/services", "r"))) {
      while (fgets(buffer, MAX_LINE, fin)) {
	buffer[MAX_LINE-1] = '\0';
	if (!strncmp(buffer, "LANGUAGE=", 9)) {
	  while (strlen(buffer) && (strchr("\n\r ",buffer[strlen(buffer)-1])))
	    buffer[strlen(buffer)-1] = '\0';
	  locale_str = buffer+9;
	  for (locale_index=0; locale_index<LANG_MAP_SIZE; locale_index++) {
	    if (!strcmp(iconv_map[locale_index].key, locale_str)) {
	      fclose(fin);
	      return locale_index;
	    }
	  }
...
static unsigned char* get_utf8_text(const int locale_index, const id3_ucs4_t* native_text) {
  ...
  i = locale_index;
  /* (1) try utf8 -> default */
  rc = do_convert(iconv_map[i].name, "UTF-8", in8, strlen(in8), iconv_buf, MAX_ICONV_BUF);
  if (rc == ICONV_OK) {
    utf8_text = in8;
    free(iconv_buf);
  }
    ...
    /* (2) try default -> utf8 */
    rc = do_convert("UTF-8", iconv_map[i].name, in, strlen(in), iconv_buf, MAX_ICONV_BUF);
    if (rc == ICONV_OK) {
      utf8_text = iconv_buf;
    }
    else if (rc == ICONV_TRYNEXT) {
      /* (3) try other encodes */
      for (n=1; n<LANG_MAP_SIZE; n++) {
	if (++i >= LANG_MAP_SIZE)
	  i -= LANG_MAP_SIZE;
	rc = do_convert("UTF-8", iconv_map[i].name, in, strlen(in), iconv_buf, MAX_ICONV_BUF);
	if (rc == ICONV_OK) {
