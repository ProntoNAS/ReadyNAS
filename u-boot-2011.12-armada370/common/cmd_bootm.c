...
extern void LCD_show_failed(void);
...
static void finish_cmdline(int do_mtdparts)
{
	char buf[512];
	const char *cmdline = getenv("bootargs");
	const char *reason = getenv("boot_reason");
	const char *mtdparts;
	const char *bdtype;
#ifdef NETGEAR_RN104
	bdtype = "rn104";
	...
	sprintf(buf, "%s%s%s reason=%s bdtype=%s",
		cmdline, do_mtdparts ? " " : "", mtdparts, reason, bdtype);
	setenv("bootargs", buf);
}
...
static int bootm_start(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	...
	if (((images.os.type == IH_TYPE_KERNEL) ||
		...
		kname = (char *)images.legacy_hdr_os->ih_name;
		if (strncmp(kname, "Linux-3.", 8) == 0 ||
		    strncmp(kname, "Linux-RN", 8) == 0 ||
		    strncmp(kname, "Kernel-R", 8) == 0) {
			unsigned long kver = simple_strtoul(kname+8, NULL, 10);
			if (kver < 12) {
				printf("Skipping FDT on legacy kernel\n");
				images.ft_len = 0;
			}
		}
#endif
	}
	finish_cmdline(!images.ft_len);
