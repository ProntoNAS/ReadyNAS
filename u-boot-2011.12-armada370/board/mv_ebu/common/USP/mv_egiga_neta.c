...
static int mvEgigaInit(struct eth_device *dev, bd_t *p)
{
	...
	MV_BOOL switchPort = mvBoardEthPortIsSwitch(priv->port);
#endif
	...
	if (priv->devInit != MV_TRUE) {
		...
		phyAddr = mvBoardPhyAddrGet(priv->port);

		printf( "%s wait for link .", dev->name);
		/* Check if link is up (phy is ready) for 6 Sec */
		for (i = 0; i < 300 ; i++) {
#ifdef MV_INCLUDE_SWITCH
			if (switchPort)
				status = mvEthSwitchPhyCheckLink(priv->port);
			else
#endif
				status = mvEthPhyCheckLink(phyAddr);
			if (status)
				break;
			mvOsDelay(20);
			if (((i*20) % 1000) == 0)
		...
		if (status) {
			printf("Done - link up.\n");
			DB(printf( "mvNetaLinkIsUp: %s link up after %d ms.\n", dev->name, i*20 ));
		}
		else {
			printf( " no link (wait %d ms)\n", i*20 );
			return -1;
		}
