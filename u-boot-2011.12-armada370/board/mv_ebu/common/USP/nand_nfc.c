 ...
 */
#include <common.h>
#if defined(CONFIG_CMD_NAND) && defined(CONFIG_MTD_NAND_NFC)
#define UBOOT_CODE
...
#ifdef MV_NAND_READ_OOB
static struct nand_ecclayout ecc_layout_2KB_noecc = {
	.eccbytes = 0,
	.eccpos = {},
	.oobfree = { {1, 4}, {6, 57} }
};
...
#ifdef MV_NAND_READ_OOB
static struct nand_ecclayout ecc_layout_4KB_noecc = {
	.eccbytes = 0,
	.eccpos = {},
	/* Bootrom looks in bytes 0 & 5 for bad blocks */
	.oobfree = { {1, 4}, {6, 121} }
...
static struct nand_bbt_descr mv_lp_bb = {
	.options = NAND_BBT_SCANMVCUSTOM | NAND_BBT_SCAN2NDPAGE,
	.offs = 0,
	.len = 2,
	.pattern = mv_scan_pattern
};
...
#ifdef MV_NAND_READ_OOB
		&ecc_layout_2KB_noecc, &mv_lp_bb, 2048, 2048, 62, 1, 0, 0
#else
		NULL, NULL, 0, 0, 0, 0, 0, 0
#endif
	}},
...
#ifdef MV_NAND_READ_OOB
		&ecc_layout_4KB_noecc, &mv_lp_bb, 4034, 2048, 62, 2, 0, 0
#else
		NULL, NULL, 0, 0, 0, 0, 0, 0
#endif
	}},
...
int orion_nfc_wait_for_completion_timeout(struct orion_nfc_info *info, int timeout)
{
	...
	mask = ~MV_NAND_REG_READ(NFC_CONTROL_REG) & 0xFFF;
	if (mask & 0x800)
		mask |= 0x1000;
	NFC_DPRINT((PRINT_LVL ">>> wait_for_completion_timeout timeout mask is:[0x%x]\n", mask));
	info->dscr = MV_NAND_REG_READ(NFC_STATUS_REG);
	time = get_timer(0);
	...
	MV_NAND_REG_WRITE(NFC_STATUS_REG, info->dscr);
...
static irqreturn_t orion_nfc_irq_dma(int irq, void *devid)
{
	...
	status = MV_NAND_REG_READ(NFC_STATUS_REG);
	...
	MV_NAND_REG_WRITE(NFC_STATUS_REG, status);
	return IRQ_HANDLED;
}
#endif
#endif /* UBOOT_CODE */
		...
		uint32_t event)
{
	...
	MV_NAND_REG_WRITE(NFC_STATUS_REG, NFC_SR_MASK);
	...
	NFC_DPRINT((PRINT_LVL "\nAbout to issue dma cmd %d (cs %d) - 0x%x.\n",
				...
				MV_NAND_REG_READ(NFC_CONTROL_REG)));
	if ((info->cmd == MV_NFC_CMD_READ_MONOLITHIC) ||
	...
	NFC_DPRINT((PRINT_LVL "After issue command %d - 0x%x.\n",
				info->cmd, MV_NAND_REG_READ(NFC_STATUS_REG)));
...
fail_stop:
	ndcr = MV_NAND_REG_READ(NFC_CONTROL_REG);
	MV_NAND_REG_WRITE(NFC_CONTROL_REG, ndcr & ~NFC_CTRL_ND_RUN_MASK);
	udelay(10);
fail:
	return -ETIMEDOUT;
}
...
static int orion_nfc_do_cmd_pio(struct orion_nfc_info *info)
{
	...
	NFC_DPRINT((PRINT_LVL "\nStarting PIO command %d (cs %d) - NDCR=0x%08x\n",
				info->cmd, info->nfcCtrl.currCs, MV_NAND_REG_READ(NFC_CONTROL_REG)));
	...
	for (i=0; i < numCmds; i++) {
		...
		if (orion_nfc_cmd_info_lkup[descInfo[i].cmd].events_p1) {
			...
			if (!orion_nfc_wait_for_completion_timeout(info, timeout)) {
				printk(KERN_ERR "command %d execution timed out (CS %d, NDCR=0x%x, NDSR=0x%x).\n",
				       descInfo[i].cmd, info->nfcCtrl.currCs, MV_NAND_REG_READ(NFC_CONTROL_REG), MV_NAND_REG_READ(NFC_STATUS_REG));
				info->retcode = ERR_CMD_TO;
				goto fail_stop;
			}
		...
		if (orion_nfc_cmd_info_lkup[descInfo[i].cmd].events_p2) {
			...
			if (!orion_nfc_wait_for_completion_timeout(info, timeout)) {
				printk(KERN_ERR "command %d execution timed out (NDCR=0x%08x, NDSR=0x%08x, NDECCCTRL=0x%08x)\n", descInfo[i].cmd,
						MV_NAND_REG_READ(NFC_CONTROL_REG), MV_NAND_REG_READ(NFC_STATUS_REG), MV_NAND_REG_READ(NFC_ECC_CONTROL_REG));
		...
		if (ndcr & NFC_CTRL_ND_RUN_MASK) {
		    ...
		    //   	info->cmd, MV_NAND_REG_READ(NFC_CONTROL_REG), MV_NAND_REG_READ(NFC_STATUS_REG), MV_NAND_REG_READ(NFC_ECC_CONTROL_REG));
			MV_NAND_REG_WRITE(NFC_CONTROL_REG, (ndcr & ~NFC_CTRL_ND_RUN_MASK));
		}
	}

	NFC_DPRINT((PRINT_LVL "Command done (NDCR=0x%08x, NDSR=0x%08x)\n", MV_NAND_REG_READ(NFC_CONTROL_REG), MV_NAND_REG_READ(NFC_STATUS_REG)));
	info->retcode = ERR_NONE;
...
{
	return (MV_NAND_REG_READ(NFC_STATUS_REG) & (NFC_SR_RDY0_MASK | NFC_SR_RDY1_MASK)) ? 1 : 0;
}
...
static int orion_nfc_markbad(struct mtd_info *mtd, loff_t ofs)
{
	...
	if (ret == 0) {
		...
		//printf("orion_nfc_markbad: ofs=0x%llx, block=0x%x\n", ofs, block);
		ofs += mtd->erasesize;
		ofs &= ~(mtd->erasesize - 1);
		ops.datbuf = NULL;
		...
		ops.ooblen = 6;
		ops.oobbuf = (void *) buf;
#ifdef ERRATA_FE_9123155
		/* SLC mode: writing bytes 0 and 5 as zeroes in the spare area of pages 0 and 1 of the bad block */
		page_addr = ofs - mtd->erasesize; /* page_addr --> pages 0  */
		ret = mtd->write_oob(mtd, page_addr, &ops);
		if (ret) {
			printk(KERN_ERR "Error writing OOB block Page %lld\n", page_addr);
			return ret;
		}
		page_addr += mtd->writesize; /* page_addr --> pages 1  */
		ret = mtd->write_oob(mtd, page_addr, &ops);
		if (ret) {
			printk(KERN_ERR "Error writing OOB block Page %lld\n", page_addr);
			return ret;
...
#endif
		page_addr = ofs - mtd->writesize; /* page_addr --> last pages in block  */
		ret = mtd->write_oob(mtd, page_addr, &ops);
		if (ret) {
			printk(KERN_ERR "Error writing OOB block Page %lld\n", page_addr);
			return ret;
...
static int orion_nfc_probe(struct platform_device *pdev)
{
	...
	MV_NAND_REG_WRITE(NFC_STATUS_REG, MV_NAND_REG_READ(NFC_STATUS_REG));
	if (info->use_dma)
#ifdef CONFIG_MV_INCLUDE_PDMA
...
int board_nand_init(struct nand_chip *nand)
{
	...
	MV_NAND_REG_BIT_SET(MV_SOC_DEV_MUX_REG, BIT0);
...
#else
	#ifdef MV_NAND_READ_OOB
	info->ecc_type	= MV_NFC_ECC_DISABLE;
	#else
	#error ECC Mode must be defined!
#endif
#endif

	info->mmio_phys_base = (MV_NFC_REGS_BASE | INTER_REGS_BASE);
...
static int orion_nfc_resume(struct platform_device *pdev)
{
	...
	for(i = 0; i < info->pdmaDataLen; i+=2)
		MV_NAND_REG_WRITE(info->pdmaUnitData[i], info->pdmaUnitData[i+1]);
	...
	for(i = 0; i < info->nfcDataLen; i+=2)
		MV_NAND_REG_WRITE(info->nfcUnitData[i], info->nfcUnitData[i+1]);
