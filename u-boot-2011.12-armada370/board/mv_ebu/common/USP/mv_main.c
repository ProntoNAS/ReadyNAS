...
void misc_init_r_env(void){
	...
	envSetDefault("mtdparts", "mtdparts=armada-nand:0x180000@0(u-boot),0x20000@0x180000(u-boot-env),0x600000@0x200000(zImage),0x400000@0x800000(minirootfs),-(ubifs)");
	envSetDefault("nandEcc", "1bit");
#endif

#if defined(MV_NOR_BOOT)
	envSetDefault("mtdparts", "mtdparts=cfi_flash:4m(boot),-(rootfs)");
#endif
...
#if defined(MV_INCLUDE_CLK_PWR_CNTRL)
	/* Clock Gating */
	envVerifyAndSet("enaClockGating", "no", "yes",2);
#endif
...
clcd.lcd0_enable=$lcd0_enable clcd.lcd_panel=$lcd_panel; bootz 0x2000000 - $fdtaddr;";
	...
	envSetDefault("fdtfile", "armada-370-db.dtb");
#elif defined(MV88F78X60)
	envSetDefault("fdtfile", "armada-xp-db.dtb");
#endif
	envSetDefault("bootcmd_fdt", bootcmd_fdt);
#endif
...
#else
	{
		setenv("bootcmd", "run check_usb; if test -n ${file_exist} -a ${file_exist} -eq 1; then run USBboot; fi; run NANDboot;");
		env = getenv("bootargs");
		if(!env)
...
void __attribute__((weak)) hwServicesLateInit(void)
{
  /* default = return */
}
#endif
...
#ifdef CONFIG_DB_78X60_AMC
	if (mvBoardIsBC2()) {
		printf(">>> callint resetw <<<\n");
		run_command("resetw", 0);
	}
#endif
	mvBoardReset();
}
