...
/*					0x4c/[3:0]		0x4c[4],0x4d/[3:0]	Internal */
	{"533  / 266 / 533 Mhz",		0x1,			0x13		, 0},	/* 1:2:2:2 - 1:1 mode */
	{"667  / 333 / 334 Mhz",		0x2,			0x1		, 0},	/* 1:2:2:2 - 1:1 mode */
	{"400  / 400 / 400 Mhz",		0x0,			0x1B		, 1},	/* 1:1:1:1 - 1:1 mode */
	{"800  / 400 / 400 Mhz",		0x3,			0x1		, 1},	/* 1:2:2:2 - 1:1 mode */
	{"1200 / 400 / 400 Mhz",		0x6,			0x7		, 1},	/* 2:3:6:6 - 1:1 mode */
	{"800  / 266 / 533 Mhz",		0x3,			0x14		, 0},	/* 2:3:6:3 - 2:1 mode */
	{"1066 / 266 / 533 Mhz",		0x5,			0x5		, 1},	/* 1:2:4:2 - 2:1 mode */
	{"667  / 333 / 667 Mhz",		0x2,			0x13		, 1},	/* 1:1:2:1 - 2:1 mode */
	{"1000 / 333 / 667 Mhz",		0x4,			0x14		, 0},	/* 2:3:6:3 - 2:1 mode */
	{"1200 / 300 / 600 Mhz",		0x6,			0x5		, 0}	/* 1:2:4:2 - 2:1 mode */
};
char * bootSourceStr[]={
	"SPI - 32bit",
	"NAND - 8bit, 4bit ECC, 4KB page size",
	"NOR - ALE - 0, Muxed, 16bit",
	"opt' = user manual option (opt)"
};
...
static int do_sar_list(int argc, char *const argv[])
{
	...
	if (strcmp(cmd, "cpufreq") == 0) {
		...
		for (i=0; i<FREQ_MODES_NUM; i++)
			if (freq_modes[i].internalFreq == 0)	/* print only the not internal freq*/
...
static int do_sar_read(int argc, char *const argv[])
{
	...
	if (strcmp(cmd, "cpufreq") == 0) {
		...
		bootsrc = mvBoardBootDevGet();
		switch (bootsrc) {
		case 0x14:		/* SPI - 32bit */
		       printf("\t%d = %s\n", 0, bootSourceStr[0]);
			break;
		case 0x0b:		/* NAND - 8bit, 4bit ECC, 4KB page size */
		       printf("\t%d = %s\n", 1, bootSourceStr[1]);
			break;
		case 0x31:		/* NOR - ALE - 0, Muxed, 16bit */
		       printf("\t%d = %s\n", 2, bootSourceStr[2]);
			break;
		default:		/* User Option */
		       printf("\t3(%d) = %s\n", bootsrc, bootSourceStr[3]);
			break;
		}
	} else if (strcmp(cmd, "freq") == 0) {
...
int do_sar(cmd_tbl_t * cmdtp, int flag, int argc, char * const argv[])
{
	...
	if (boardId != DB_88F6710_BP_ID) {
		char tmp[80];
		mvBoardNameGet(tmp);
		printf("SatR is not valid for board %s \n", tmp);
		return 1;
	}
