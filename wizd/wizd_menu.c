...
#define	SORT_READ_MASK			( 0x0000F000 )
#define	SORT_READ_FLAG(_A_)		( ( _A_ & SORT_READ_MASK ) >> 12 )
#define	SORT_READ_UP			( 0x00001000 )
#define	SORT_READ_DOWN			( 0x00002000 )
#define	DEFAULT_SORT_RULE		SORT_NONE

static JOINT_FILE_INFO_T joint_file_info;

// #define debug_log_output printf
...
static int	 	_file_info_read_sort_order_up( const void *in_a, const void *in_b );
static int 		_file_info_read_sort_order_down( const void *in_a, const void *in_b );
...
static void * read_sort_api[] = {
	NULL,
	_file_info_read_sort_order_up,
	_file_info_read_sort_order_down
};
				 ...
				 int *accum_second, int titleid, FILE_INFO_T * file_info_p)
{
	...
	*accum_hour = *accum_minute = *accum_second = 0;
	...
	for (i = 0; i < tt_srpt->title[titleid].nr_of_ptts; i++) {
		...
		if (pgc_id > ifo_title->vts_pgcit->nr_of_pgci_srp)
			return;
		...
		if (i == 0) {
			if (cur_pgc->program_map == 0)
				return;
			if (cur_pgc->program_map[pgn-1] <= 0)
				return;
		...
		for (;;) {
			if (end_cell < 0 || cur_pgc->cell_playback == 0) {
				file_info_p->end_sector = file_info_p->start_sector;
				break;
			}
...
char *
wizd_check_file_read(char *fullpath_filename)
{
	int				 fd;
	int				 result;
	char			*wizd_filesread;
	...
	if (!global_param.highlight_read_files)
		return(0);

	wizd_filesread = 0;

	if (fullpath_filename[strlen(fullpath_filename) - 1] != '/')
		strcat(fullpath_filename, "/");
	strcat(fullpath_filename, WIZD_FILESREAD_FILE);

	result = stat(fullpath_filename, &file_stat);
	if ( result >= 0 ) {
		if ((fd = open(fullpath_filename, O_RDONLY)) < 0) {
			perror("open");
		} else if ((wizd_filesread = (char *) malloc(file_stat.st_size)) == 0) {
			close(fd);
			perror("malloc");
		} else if (read(fd, wizd_filesread, file_stat.st_size) != file_stat.st_size) {
			close(fd);
			perror("read");
		} else {
			close(fd);
		...
		if (strcasecmp(http_recv_info_p->action, "dvdplay") == 0) {
			...
			if((titleid >= 0) && (titleid < tt_srpt->nr_of_srpts)) {
				...
				for (i = 0; i < tt_srpt->title[titleid].nr_of_ptts; i++) {
					...
					file_info_p[i].title_set_nr = tt_srpt->title[titleid].title_set_nr;
...
# endif
			...
			// Create MENU with Titles and Chapters
			int shift = 0;
...
# endif
			...
			for(i=0 ; i < file_num; ++i ) {
						...
						for (j = 0;
							...
							switch (ifo_title->vtsi_mat->vts_audio_attr[j].audio_format) {
							  ...
							  case 6:
								...
								break;
							  case 0:
								// makes display to verbose if we add this
				...
				debug_log_output("workdata is %s\n", work_data);

				if (global_param.flag_show_sub_info) {
...
# endif
				...
				file_info_p[i + shift].max_chapter = tt_srpt->title[ i ].nr_of_ptts;
				file_info_p[i + shift].item_read = 0;

				if (global_param.highlight_read_files) {
					sprintf(dvdoptbuf, "%sVIDEO_TS", http_recv_info_p->send_filename);
					wizd_filesread = wizd_check_file_read(dvdoptbuf);
					sprintf(work_data2, " %d ", i);
					if (wizd_filesread && strstr(wizd_filesread, work_data2) != 0){
						// this file has already been read
						file_info_p[i + shift].item_read = 1;
						free(wizd_filesread);
					}
				}
...
# endif
	...
	if ( sort_rule != SORT_NONE )
	{
		...
		if (dvd)
			global_param.flag_sort_read = 0;

		if (global_param.flag_sort_read) {
			if (sort_rule == SORT_NAME_DOWN)
				sort_rule = SORT_NAME_UP | SORT_READ_DOWN;
			else
				sort_rule |= SORT_READ_DOWN;
		}
...
set_thumb_file(int menu_file_type, char *file_image, char *http_name, char *name, char *ext, char *icon_base, char *icon_type)
{
	...
	// printf("\nmenu_type is %d, http_name %s, name %s, no_thumbs is %d, ext %s\n", menu_file_type, http_name, name, no_thumbs);
	...
	if (!no_thumbs && ( ((menu_file_type & TYPE_MASK) == TYPE_DIRECTORY)
	   ...
	   || (menu_file_type == TYPE_SVI) ) ) {
		...
		for (i = 0; i < global_param.num_aliases; i++) {
			...
			len = strlen(global_param.alias_path[i]);
			if (strncmp(http_name, global_param.alias_path[i], len) != 0) {
				//printf("skipped, httpname %s != %s, len %d, name[len] = '%c'\n", http_name, global_param.alias_path[i], len, http_name[len]);
				continue;
			} else if (http_name[len] != '\0' && http_name[len] != '\\' && http_name[len] != '/') {
				//printf("skipped, httpname %s != %s, len %d, name[len] = '%c'\n", http_name, global_param.alias_path[i], len, http_name[len]);
				continue;
			} else {
				aname = global_param.alias_name[i];
				p = http_name + strlen(global_param.alias_path[i]);
				//printf("found alias name %s, path %s matches http_name %s\n", aname, global_param.alias_path[i], http_name);
				use_alias = 1;
				break;
			}
		}
...
all_files_are_music(HTTP_RECV_INFO *htp_recv_info_p, FILE_INFO_T *file_info_p, int file_num)
{
	...
	for (i = 0; i < file_num; i++) {
		if (file_info_p[i].type != TYPE_MUSIC) {
			...
			if (strcmp(file_info_p[i].name, "default_photo") != 0)
				return(0);
		}
	}
...
static void create_skin_filemenu(int accept_socket, HTTP_RECV_INFO *http_recv_info_p, FILE_INFO_T *file_info_p, int file_num, int search)
{
	...
	sprintf(skin_rep_data_global_p->background_image, "back.jpg");
	...
	for ( i=0, count=0 ; i < file_num ; i++) {
		...
		if ( (i>=skin_rep_data_global_p->start_file_num) && (i<(skin_rep_data_global_p->start_file_num + skin_rep_data_global_p->now_page_line)) )
		{
			...
			sprintf(skin_rep_data_line_p[count].full_pathname, "%s%s", file_info_p[i].full_pathname, file_info_p[i].name);
			if (file_info_p[i].ext[0]) {
				strcat(skin_rep_data_line_p[count].full_pathname, ".");
				strcat(skin_rep_data_line_p[count].full_pathname, file_info_p[i].ext);
			}

			skin_rep_data_line_p[count].item_read = file_info_p[i].item_read;
			...
			set_thumb_file(menu_file_type,
						   ...
						   file_info_p[i].name,
						   file_info_p[i].ext,
						   ((skin_rep_data_global_p->columns || global_param.flag_thumb_in_details) && special_view < 3 ? "thumb" : "icon"),
			...
			if (search && skin_rep_data_line_p[count].file_image[0] != '/') {
			...
			if ( (skin_rep_data_line_p[count].menu_file_type & TYPE_MASK) == STREAMED_BASE )
			{
				...
				case TYPE_CHAPTER:
					...
					if(file_info_p[i].org_name[0] == 0) {
						...
						if (global_param.flag_hide_short_titles && file_info_p[i].duration.hour == 0 && file_info_p[i].duration.minute == 0 && file_info_p[i].duration.second <= 1 && skin_rep_data_line_p[count].menu_file_type != TYPE_CHAPTER) {
							continue;
						}
						...
						if (http_recv_info_p->flag_pc) {
							...
							snprintf(skin_rep_data_line_p[count].file_uri_link
								, sizeof(skin_rep_data_line_p[count].file_uri_link)
								, "/-.-playlist.pls?http://%s%sVIDEO_TS/VTS_%02d_1.VOB?title=%d&page=0&file=chapter1.mpg&startitem=%d"
								, http_recv_info_p->recv_host
								, http_recv_info_p->recv_uri
								, file_info_p[i].title_set_nr
								, file_info_p[i].title
								, file_info_p[i].max_chapter
							);
						} else {
...
# endif
			...
			if ( skin_rep_data_line_p[count].menu_file_type == TYPE_CHAPTER) {
				if (http_recv_info_p->flag_pc) {
					...
					snprintf(skin_rep_data_line_p[count].file_uri_link
						, sizeof(skin_rep_data_line_p[count].file_uri_link)
						, "/-.-playlist.pls?http://%s%sVIDEO_TS/VTS_%02d_1.VOB?title=%d&page=%d&file=chapter%d.mpg&startitem=%d"
						, http_recv_info_p->recv_host
						, http_recv_info_p->recv_uri
						, file_info_p[i].title_set_nr
						, file_info_p[i].title
						, file_info_p[i].chap - 1
						, file_info_p[i].chap
						, file_num
					);
				} else {
...
/*		&& (skin_rep_data_global_p->stream_files == 0)
		...
		&& (skin_rep_data_global_p->photo_files == 0) */ )
	{
		...
		case TYPE_MOVIE:
			skin_rep_data_global_p->stream_files++;
			sprintf(skin_rep_data_global_p->background_image, "movie_back.jpg");
			break;
		case TYPE_MUSIC:
			skin_rep_data_global_p->music_files++;
			sprintf(skin_rep_data_global_p->background_image, "music_back.jpg");
			break;
		case TYPE_JPEG:
			skin_rep_data_global_p->photo_files++;
			sprintf(skin_rep_data_global_p->background_image, "photo_back.jpg");
			break;
		default:
			sprintf(skin_rep_data_global_p->background_image, "back.jpg");
			break;
...
void
send_menu(int accept_socket, SKIN_REPLASE_GLOBAL_DATA_T *skin_rep_data_global_p, HTTP_RECV_INFO *http_recv_info_p, int skip, int special_view)
{
...
# endif
	...
	strcpy(menu_work_p, "<tr><td>&nbsp;</td></tr>\n");
	send(accept_socket, menu_work_p, strlen(menu_work_p), 0);

	// add browsing categories
	strcpy(menu_work_p, mid_line_skin->buffer);
	strcpy(middle_rep[0].file_uri_link, "/?");
	sprintf(middle_rep[0].file_image, "/home_button.%s", global_param.menu_icon_type);
	strcpy(middle_rep[0].file_name, "Home");
	sprintf(middle_rep[0].tvid_string, "%d", 1);
	...
	strcpy(menu_work_p, mid_line_skin->buffer);
	strcpy(middle_rep[0].file_uri_link, "/?alias=movie");
	sprintf(middle_rep[0].file_image, "/video_button.%s", global_param.menu_icon_type);
	strcpy(middle_rep[0].file_name, "Movies");
	sprintf(middle_rep[0].tvid_string, "%d", 1);
	replase_skin_line_data(menu_work_p, MAX_SKIN_FILESIZE, &middle_rep[0]);
	replase_skin_grobal_data(menu_work_p, MAX_SKIN_FILESIZE, skin_rep_data_global_p);
	send(accept_socket, menu_work_p, strlen(menu_work_p), 0);
	...
	strcpy(menu_work_p, mid_line_skin->buffer);
	strcpy(middle_rep[0].file_uri_link, "/?alias=music");
	sprintf(middle_rep[0].file_image, "/music_button.%s", global_param.menu_icon_type);
	strcpy(middle_rep[0].file_name, "Music");
	sprintf(middle_rep[0].tvid_string, "%d", 1);
	replase_skin_line_data(menu_work_p, MAX_SKIN_FILESIZE, &middle_rep[0]);
	replase_skin_grobal_data(menu_work_p, MAX_SKIN_FILESIZE, skin_rep_data_global_p);
	send(accept_socket, menu_work_p, strlen(menu_work_p), 0);
	...
	strcpy(menu_work_p, mid_line_skin->buffer);
	strcpy(middle_rep[0].file_uri_link, "/?alias=photo");
	sprintf(middle_rep[0].file_image, "/photo_button.%s", global_param.menu_icon_type);
	strcpy(middle_rep[0].file_name, "Photos");
	sprintf(middle_rep[0].tvid_string, "%d", 1);
	replase_skin_line_data(menu_work_p, MAX_SKIN_FILESIZE, &middle_rep[0]);
	replase_skin_grobal_data(menu_work_p, MAX_SKIN_FILESIZE, skin_rep_data_global_p);
	send(accept_socket, menu_work_p, strlen(menu_work_p), 0);
	...
	for (i = 0; i < global_param.num_aliases + 1; i++) {
		...
		switch (http_recv_info_p->default_file_type) {
		  ...
		  case TYPE_MUSIC:
		  case TYPE_JPEG:
			if(i > 0 && global_param.alias_default_file_type[i - 1] != http_recv_info_p->default_file_type)
				continue;
			break;
		  default:
			break;
		...
		if (real_count == 6 && http_recv_info_p->menupage == 0) {
			real_count++;
			continue;
		}

		if (global_param.flag_default_thumb && special_view < 3) {
			if (real_count == 6) {
		...
		middle_rep[0].file_name[global_param.thumb_filename_length_max] = 0;
		middle_rep[0].file_image[0] = 0;
...
# endif
		...
		i = real_count;
		while (i != mid_items) {
			strcpy(menu_work_p, "<!pad> <tr><td>&nbsp;</td></tr>\n");
			send(accept_socket, menu_work_p, strlen(menu_work_p), 0);
			i++;
		}
	}
...
static void send_skin_filemenu(int accept_socket, SKIN_REPLASE_GLOBAL_DATA_T *skin_rep_data_global_p, SKIN_REPLASE_LINE_DATA_T *skin_rep_data_line_p, int lines, int special_view, HTTP_RECV_INFO *http_recv_info_p, FILE_INFO_T *file_info_p, int file_num, int search)
{
	...
	if(!skin_rep_data_global_p->delete_mode) {
		...
		if(skin_rep_data_global_p->columns && special_view < 3)
		...
		if (mid_skin != NULL) {
			...
			send_menu(accept_socket, skin_rep_data_global_p, http_recv_info_p, skip, special_view);
	...
	for (sm_ptr = skin_mapping; sm_ptr->skin_filename != NULL; sm_ptr++) {
		...
		if(!special_view && skin_rep_data_global_p->columns) {
			if (sm_ptr->filetype == TYPE_DIRECTORY) {
				if (http_recv_info_p->default_file_type == TYPE_MUSIC)
					snprintf(work_buf, sizeof(work_buf), "thumb_%sdir.html", "music");
				else if (http_recv_info_p->default_file_type == TYPE_JPEG)
					snprintf(work_buf, sizeof(work_buf), "thumb_%sdir.html", "photo");
				else if (http_recv_info_p->default_file_type == TYPE_MOVIE)
					snprintf(work_buf, sizeof(work_buf), "thumb_%sdir.html", "movie");
				else
			...
			if (sm_ptr->filetype == TYPE_DIRECTORY) {
				if (http_recv_info_p->default_file_type == TYPE_MUSIC)
					snprintf(work_buf, sizeof(work_buf), "line_%sdir.html", "music");
				else if (http_recv_info_p->default_file_type == TYPE_JPEG)
					snprintf(work_buf, sizeof(work_buf), "line_%sdir.html", "photo");
				else if (http_recv_info_p->default_file_type == TYPE_MOVIE)
					snprintf(work_buf, sizeof(work_buf), "line_%sdir.html", "movie");
				else
	...
	for (count=0; count < lines; count++) {
		...
		if (special_view == 4) {
			...
			set_html_file(TYPE_MUSIC,
						   ...
						   global_param.menu_icon_type);
			for (i = 0; i < lines; i++) {
				strcpy(skin_rep_data_line_p[i].file_image, nav_rep[0].file_image);
			}
...
# if 1
	if (!special_view && count < skin_rep_data_global_p->items_per_page) {
		for (; count < skin_rep_data_global_p->items_per_page; count++) {
			sprintf(menu_work_p, "<!pad> <td>&nbsp;</td>\n");
			send(accept_socket, menu_work_p, strlen(menu_work_p), 0);

			// pad out the rest of the page
			if(	(!special_view && skin_rep_data_global_p->columns>0)
				 && (count != skin_rep_data_global_p->items_per_page - 1)
				 && ((count % skin_rep_data_global_p->columns) == (skin_rep_data_global_p->columns-1)) ) {
				strcpy(menu_work_p, "<!pad_break> </tr>\n<tr>\n");
				send(accept_socket, menu_work_p, strlen(menu_work_p), 0);
			}
		}
	}
...
static int next_directory_stat(unsigned char *path, FILE_INFO_T *file_info_p, int file_num, regex_t *re, int g_index, int recurse, int type)
{
	int	count;
	DIR	*dir;
	struct dirent	*dent;
	...
	unsigned char	 workbuf[WIZD_FILENAME_MAX];
	struct stat		 file_stat;
	struct stat		 dir_stat;
	char			*wizd_filesread;
	...
	// check for .wizd_filesread file to determine which files
	// have been read
	strcpy(fullpath_filename, path);
	wizd_filesread = wizd_check_file_read(fullpath_filename);
	...
	while ( 1 )
	{
		...
		if (S_ISDIR(file_stat.st_mode) && recurse) {
			...
			count += next_directory_stat(fullpath_filename, &file_info_p[count], file_num - count, re, g_index, 1, type);
		...
		if (re) {
			if (type == TYPE_MOVIE && S_ISDIR(file_stat.st_mode)) {
				sprintf(workbuf, "%s/%s/VIDEO_TS", path, dent->d_name);
				memset(&dir_stat, 0, sizeof(dir_stat));
				result = stat(workbuf, &dir_stat);
				if (result < 0) {
					// printf("skipping non dvd directory %s\n", workbuf);
					continue;
				}
			}
			
			if (regexec(re, dent->d_name, 0, NULL, 0) != 0)
				continue;
		}
...
# endif
				...
				if (wizd_filesread && strstr(wizd_filesread, dent->d_name) != 0)
					// this file has already been read
					file_info_p[count].item_read = 1;
			}
		} else {
	...
	if(len > 0) {
		...
		if(i < global_param.num_aliases) {
			...
			for(j=0; j<global_param.num_aliases; j++) {
				if(strcasecmp(global_param.alias_name[i],global_param.alias_name[j])==0) {
					debug_log_output("Including alias path '%s'\n", global_param.alias_path[j]);
					count += next_directory_stat(global_param.alias_path[j], file_info_p+count, file_num-count, 0, 0, 0, TYPE_UNKNOWN);
				}
			}
		} else {
			// Not an alias
			count += next_directory_stat(path, file_info_p+count, file_num-count, 0, 0, 0, TYPE_UNKNOWN);
		}
	}
	return count;
}
...
static char* create_1line_playlist(HTTP_RECV_INFO *http_recv_info_p, char *path, FILE_INFO_T *file_info_p, off_t offset)
{
	...
	if (file_info_p != NULL) {
		// the test for file_info_p is a hack.  if NULL then it is
		// a search for a single file, in which case ignore the
		// default file type
		...
		case TYPE_JPEG:
		case TYPE_PLAYLIST:
			if ( type != TYPE_JPEG && type != TYPE_IMAGE) {
				return NULL;
			}
			break;
		}
...
# endif
	...
	if (type == TYPE_JPEG || type == TYPE_IMAGE) {
		sprintf(work_data, "%s?action=Resize.jpg&maxwidth=%d&maxheight=%d&height=%d", file_uri_link, global_param.max_jpeg_width, global_param.max_jpeg_height, global_param.max_jpeg_height);
		strcpy(file_uri_link, work_data);
	}
...
static void create_all_play_list(int accept_socket, HTTP_RECV_INFO *http_recv_info_p, FILE_INFO_T *file_info_p, int file_num, int max_items)
{
	...
	startitem = http_recv_info_p->startitem;
	http_recv_info_p->startitem = 0;
	enditem = http_recv_info_p->enditem;
	http_recv_info_p->enditem = 0;
	...
	if(dir_list != NULL) {
		...
		if (strcasecmp(http_recv_info_p->action, "shuffle") != 0) {
			for (i = 0; i < dir_count && count < max_items; i++) {
...
static void file_info_sort( FILE_INFO_T *p, int num, unsigned long type )
{
	...
	int nDir, nFile, i, row, nRead;

	// ディレクトリ数とファイル数の分離
	nRead = 0;
	nDir = 0;
	if (global_param.flag_sort_read) {
		for ( nRead = 0, i = 0; i < num; ++i )
		{
			if (p[ i ].item_read)
			{
				++nRead;
	...
	if (nRead > 0) {
		// とりあえず全部ソート
		qsort( p, num, sizeof( FILE_INFO_T ), read_sort_api[ SORT_READ_FLAG( type ) ] );

		// ディレクトリ名のソート
		qsort( &p[ ( SORT_READ_MASK & type ) == SORT_READ_DOWN ? num - nRead : 0 ], nRead, sizeof( FILE_INFO_T ), file_sort_api[ (SORT_READ_MASK & type) == SORT_READ_UP ? SORT_NAME_UP : SORT_NAME_DOWN] );

		// File position decision
		row = ( SORT_READ_MASK & type ) == SORT_READ_DOWN ? 0 : nRead;
	} else if ( SORT_DIR_FLAG( type ) && nDir > 0 )
	{
		// とりあえず全部ソート
...
static int _file_info_read_sort( const void *in_a, const void *in_b, int order )
{
	FILE_INFO_T *a, *b;
	int n1, n2;
...
static int _file_info_read_sort_order_up( const void *in_a, const void *in_b )
{
	return _file_info_read_sort( in_a, in_b, 0 );
}
static int _file_info_read_sort_order_down( const void *in_a, const void *in_b )
{
	return _file_info_read_sort( in_a, in_b, 1 );
}
...
# else
		...
		image_viewer_height = global_param.max_jpeg_height;
		image_viewer_width = ((float) image_viewer_height * (float) image_width) / (float) image_height; 

		if (image_viewer_width > global_param.max_jpeg_width) {
			image_viewer_width = global_param.max_jpeg_width;
			image_viewer_height = ((float) image_viewer_width * (float) image_height) / (float) image_width; 
		}
# endif
...
void http_music_single_play(int accept_socket, HTTP_RECV_INFO *http_recv_info_p)
{
	...
	if (ptr != NULL) {
		// Check for chapter offsets
		FILE	*fp;
		snprintf(work_buf, sizeof(work_buf), "%s.chapters", http_recv_info_p->send_filename);
		debug_log_output("Checking for chapters: '%s'", work_buf);
		fp = fopen(work_buf, "r");
		if(fp != NULL) {
			off_t offsets[WIZD_MAX_CHAPTERS];
			int numChapters = 0;
			int previousChapter = 0;

			debug_log_output("got chapter file\n");
			offsets[numChapters++] = 0; // first chapter is the beginning...
			while (fgets(work_buf, sizeof(work_buf), fp) != NULL && numChapters < WIZD_MAX_CHAPTERS) {
				offsets[numChapters] = atoll(work_buf);
				if (offset >= offsets[numChapters]) {
					previousChapter = numChapters;
				}
				numChapters++;
			}
			fclose(fp);
			...
			snprintf(work_buf, sizeof(work_buf), "Goto Chapter %d%s|%lld|0|%s?off=%lld|\r\n",
				previousChapter+1,
				((offset-offsets[previousChapter])>0) ? ">>" : "",
				offset-offsets[previousChapter],
				ptr, offsets[previousChapter]);
			send(accept_socket, work_buf, strlen(work_buf), 0);
			for(i=1; i<numChapters; i++) {
				if(++previousChapter >= numChapters)
					previousChapter = 0;
				snprintf(work_buf, sizeof(work_buf), "Goto Chapter %d|0|0|%s?off=%lld|\r\n",
					previousChapter+1, ptr, offsets[previousChapter]);
				send(accept_socket, work_buf, strlen(work_buf), 0);
			}
		}
		else if(    (mpeg2video && global_param.flag_goto_percent_video)
...
void http_listfile_to_playlist_create(int accept_socket, HTTP_RECV_INFO *http_recv_info_p)
{
	...
	while ( 1 ) {
		...
		if(ptr != NULL) {
			...
			if(list != NULL) {
				debug_log_output("adding %s\n", work_data);
				list[count++] = strdup(work_data);
			} else {
				debug_log_output("sending: %s\n", work_data);
...
int
http_dir_to_scplaylist_create(int accept_socket, char *path, char *host, int top, int aliasnum, int start)
{
	struct stat		file_stat;
	int				count;
	DIR				*dir;
	...
	if (top) {
		...
		for (i = 0; i < global_param.num_aliases; i++) {
			len = strlen(global_param.alias_name[i]);
			if (strncmp(&(path[1]), global_param.alias_name[i], len) == 0) {
				strcpy(buf, global_param.alias_path[i]);
				strcat(buf, path + len + 1);
				replase_character(buf, strlen(buf), "/", "\\");
				uri_decode(workbuf, sizeof(workbuf), buf, sizeof(buf));
				if (stat(workbuf, &dir_stat) == 0) {
					strcpy(path, workbuf);
					aliasnum = i;
					found = 1;
					break;
		...
		replase_character(path, strlen(path), "/", "\\");
	...
	while ( 1 ) {
		...
		if (S_ISDIR( file_stat.st_mode ) != 0)
			count += http_dir_to_scplaylist_create(accept_socket, workbuf, host, 0, aliasnum, 0);
		else {
			count++;
			if (count < start)
				continue;
			strcpy(buf, global_param.alias_name[aliasnum]);
			strcat(buf, &(workbuf[strlen(global_param.alias_path[aliasnum])]));
			replase_character(buf, strlen(buf), "\\", "/");
			uri_encode(work_data2, sizeof(work_data2), buf, strlen(buf));
			send_printf(accept_socket, "File%d=http://%s/%s\n", count, host, work_data2);
			send_printf(accept_socket, "Title%d=%s\n", count, dent->d_name);
			send_printf(accept_socket, "Length%d=-1\n", count);
		}
	}
	...
	if (top) {
		send_printf(accept_socket, "NumberOfEntries=%d\n", count);
		send_printf(accept_socket, "Version=2\n");
	}
...
int
http_dvd_to_scplaylist_create(int accept_socket, char *buff, HTTP_RECV_INFO *http_recv_info_p)
{
	int	i, start, end;
	int cnt;
	char *p;
	...
	start = http_recv_info_p->page;
	end = http_recv_info_p->startitem;

	for (i = start, cnt = 1; i <= end; i++, cnt++) {
		send_printf(accept_socket, "[playlist]\n");
		p = strstr(buff, "page=");
		p += (sizeof("page=") - 1);
		*p = 0;
		send_printf(accept_socket, "File%d=%s%d&file=chapter1.mpg\n", cnt, buff, i);
		send_printf(accept_socket, "Title%d=Chapter %d\n", cnt, i);
		send_printf(accept_socket, "Length%d=-1\n", cnt);
	}
	send_printf(accept_socket, "NumberOfEntries=%d\n", cnt - 1);
	send_printf(accept_socket, "Version=2\n");
...
void
http_uri_to_scplaylist_create(int accept_socket, char *uri_string)
{
	http_send_ok_header(accept_socket, 0, "audio/x-scpls");
...
void do_search(int accept_socket, HTTP_RECV_INFO *http_recv_info_p)
{
	...
	for (i = 0; i < global_param.num_aliases; i++) {
		...
		nfile_num += next_directory_stat(global_param.alias_path[i], &file_info_p[nfile_num], file_num - nfile_num, &re, i, 1, type);
	}
