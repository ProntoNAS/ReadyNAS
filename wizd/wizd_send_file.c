...
extern char *wizd_check_file_read(char *fullpath_filename);

void
wizd_filesread_update(HTTP_RECV_INFO *http_recv_info_p,
					  ...
					  int titleid)
{
	...
	// only remember files once they have been at least 90% read
	// also only remember music and movie files
	if (((.9 * content_length) > ((double) written + http_recv_info_p->range_start_pos)) ||
			(http_recv_info_p->menu_file_type != TYPE_MOVIE &&
			 http_recv_info_p->menu_file_type != TYPE_MUSIC &&
			 http_recv_info_p->menu_file_type != TYPE_SVI))
		return;

	// file the last "/" in the path
	strcpy(fullpath_filename, http_recv_info_p->send_filename);
	debug_log_output("searching for %s\n", fullpath_filename);
	if ((p = strrchr(fullpath_filename, '/')) == 0)
		return;

	// extract the name from the path and replace with WIZD_FILESREAD_FILE
	p++;
	if (titleid != -1) {
		sprintf(name, " %d ", titleid);
	} else
		strcpy(name, p);
	debug_log_output("looking for name %s\n", name);
	*p = 0;
	strcat(fullpath_filename, WIZD_FILESREAD_FILE);

	wizd_filesread = 0;

	debug_log_output("stating for %s\n", fullpath_filename);

	// see if file exists,if not create it, if it
	// does then search for "name"
	// if name does not exist then add it
	result = stat(fullpath_filename, &file_stat);
	if ( result < 0 ) {
		// not found, try and create the file
		debug_log_output("%s not found\n", fullpath_filename);
		if ((fp = fopen(fullpath_filename, "w")) == NULL)
			perror("fopen");
	} else {
		// found the file, open for read/write
		debug_log_output("%s found\n", fullpath_filename);
		if ((fp = fopen(fullpath_filename, "r+")) == NULL)
			...
			// read in the file and search for "name"
			debug_log_output("%s being read\n", fullpath_filename);
			fread(wizd_filesread, file_stat.st_size, 1, fp);

			if (strstr(wizd_filesread, name) != 0) {
				debug_log_output("%s already in file\n", name);
				fclose(fp);
				fp = 0;
				free(wizd_filesread);
				wizd_filesread = 0;
	...
	if (fp) {
		...
		debug_log_output("adding name %s\n", name);
		fprintf(fp, "%s\n", name);
		fclose(fp);
	}
...
*/
	...
	if (http_recv_info_p->off == 0) {
		offset = (file_stat.st_size/10) * http_recv_info_p->page;
	} else {
		offset = http_recv_info_p->off;
	}
	...
	if(    global_param.bookmark_threshold
		...
		} else if(previous_size > 0 && http_recv_info_p->off == 0) {
	...
	debug_log_output("%lld written\n", written);
	debug_log_output("type is %d\n", http_recv_info_p->menu_file_type);

	// see if we can mark this file read
	wizd_filesread_update(http_recv_info_p, content_length, written, -1);
