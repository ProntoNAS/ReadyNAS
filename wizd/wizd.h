...
#define DEFAULT_FLAG_SHOW_SUB_INFO		FALSE
...
// filename for file that keeps track of what has been read
#define WIZD_FILESREAD_FILE	".wizd_filesread"
...
typedef struct {
	...
	int				startitem;		// playlist item to start with
	int				enditem;		// playlist item to end with

	unsigned char 	passwd[64];		// 受信したURI(decoded)
} HTTP_RECV_INFO;

/* resize algorithme - used in target_jpeg_mode */
