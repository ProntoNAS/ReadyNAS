...
U_BOOT_CMD(
	...
	"crc32 chip_address offset count - compute CRC32 checksum\n"
#if defined(CONFIG_I2C_MULTI_BUS)
	"i2c dev [dev] - show or set current I2C bus\n"
#endif  /* CONFIG_I2C_MULTI_BUS */
	"i2c loop chip_address offset [count of objects] - looping read of device\n"
	"i2c md chip_address offset{.0, .1, .2}  [count] - read from I2C device\n"
	"i2c mm{.b, .w, .l} chip_address offset{.0, .1, .2} - write to I2C device (auto-incrementing)\n"
	"i2c mw chip_address offset value [count] - write to I2C device (fill)\n"
	"i2c nm{.b, .w, .l} chip_address offset{.0, .1, .2} - write to I2C device (constant address)\n"
	"i2c probe - show devices(chip addresses) on the I2C bus\n"
	"i2c read chip_address offset length memaddress - read to memory \n"
