...
/*******************************************************************************
* hwReadPagedPhyRegField
*
* DESCRIPTION:
*       This function reads a specified field from a switch's port phy register
*        in page mode. it is another way of hwGetPagedPhyRegField
*
* INPUTS:
*       portNum     - Port number to read the register for.
*       pageNum     - Page number of the register to be read.
...
*
*******************************************************************************/
GT_STATUS hwReadPagedPhyRegField(IN GT_QD_DEV * dev,
				 IN GT_U8 portNum,
				 IN GT_U8 pageNum,
				 IN GT_U8 regAddr,
				 IN GT_U8 fieldOffset,
...
{
	GT_U16 mask;            /* Bits mask to be read */
	GT_U16 tmpData, original_pge_num, page_addr;
	GT_STATUS retVal;

	page_addr = GT_GET_PAGE_ADDR(regAddr);
	retVal = gprtGetPhyReg(dev, portNum, page_addr, &original_pge_num);
	if (retVal != GT_OK)
		return retVal;
	retVal = gprtSetPhyReg(dev, portNum, page_addr, (GT_U16)pageNum);
	if (retVal != GT_OK)
		return retVal;
	retVal = gprtGetPhyReg(dev, portNum, page_addr, &tmpData);
	if (retVal != GT_OK)
		return retVal;
	CALC_MASK(fieldOffset, fieldLength, mask);
	...
	*data = tmpData;
	...
	retVal = gprtSetPhyReg(dev, portNum, page_addr, original_pge_num);
	if (retVal != GT_OK)
		return retVal;

	DBG_INFO(("Read from phy(%d) register: regAddr 0x%x, ", portNum, regAddr));
	DBG_INFO(("fOff %d, fLen %d, data 0x%x.\n", fieldOffset, fieldLength, *data));
...
/*******************************************************************************
* hwWritePagedPhyRegField
*
* DESCRIPTION:
*       This function writes to specified field in a switch's port phy register
*        in page mode; another implementation of hwSetPagedPhyRegField.
*
* INPUTS:
*       portNum     - Port number to write the register for.
*       pageNum     - Page number of the register to be read.
...
*
*******************************************************************************/
GT_STATUS hwWritePagedPhyRegField(IN GT_QD_DEV * dev,
				  IN GT_U8 portNum,
				  IN GT_U8 pageNum,
				  IN GT_U8 regAddr,
				  IN GT_U8 fieldOffset,
				  ...
				  IN GT_U16 data)
{
	...
	/* Set the desired bits to 0 */
	tmpData &= ~mask;
	/* Set the given data to above reset bits */
	tmpData |= ((data << fieldOffset) & mask);
	retVal = gprtSetPhyReg(dev, portNum, regAddr, tmpData);
	if (retVal != GT_OK)
		return retVal;
	retVal = gprtSetPhyReg(dev, portNum, page_addr, original_pge_num);
	if (retVal != GT_OK)
	...
	DBG_INFO(("Write to phy(%d) register: regAddr 0x%x, ", portNum, regAddr));
	DBG_INFO(("fieldOff %d, fieldLen %d, data 0x%x.\n", fieldOffset, fieldLength, data));
