#include <common.h>
#if defined(CONFIG_CMD_NAND) && defined(CONFIG_MTD_NAND_LNC)
#define UBOOT_CODE
...
static struct mtd_partition parts_info[] = {
	...
	{ .name = "root",
	  ...
	  /*
	   * Leave 1MB at the end of the last partition to
	   * avoid overwriting of the BBT created by u-boot.
	   */
	  .size = SZ_1M },
};
...
int __init mv_nand_init(void)
{
	...
	size = addr_win.addrWin.size;
	/* Set default size in case of no addrWin.size info. */
	if (!size)
		size = 1024;
	...
	baseaddr = (unsigned long)ioremap(physaddr, size);
	if(!baseaddr) {
	...
	printk("Using Software BCH ECC for NAND device\n");
	this->ecc.mode = NAND_ECC_SOFT_BCH;
	this->ecc.layout = &ecc_layout_4KB_bch4bit;
	/*
	 * Encode each 512B with 4 bit correction, 8 ECC steps.
	 */
	this->ecc.size		= 512;
	this->ecc.bytes		= 7;
#else
...
int board_nand_init(struct nand_chip *nand)
{
	...
	nand_dev_num = boardGetDevCSNum(0, BOARD_DEV_NAND_FLASH);
	if(-1 == nand_dev_num) {
		printf("NAND init: NAND device not found on board\n");
		err = -ENODEV;
		goto out;
	}

	if( MV_OK != mvCpuIfTargetWinGet((nand_dev_num), &addr_win) ) {
                printf("Failed to init NAND MTD (boot-CS window %d err).\n", nand_dev_num);
		err = -ENODEV;
		goto out;
	}

	if(!addr_win.enable) {
                printf("Failed to init NAND MTD (boot-CS window disabled).\n" );
		err = -ENODEV;
		goto out;
	}

	baseaddr = addr_win.addrWin.baseLow;

	MV_REG_BIT_RESET(MV_SOC_DEV_MUX_REG, BIT0);

	nand->IO_ADDR_R = nand->IO_ADDR_W = (void __iomem *)baseaddr;
	nand->cmd_ctrl = board_hwcontrol;
#ifdef	CONFIG_MTD_NAND_LNC_8BYTE_READ
	nand->read_buf = mv_nand_read_buf;
#endif
#ifdef CONFIG_NAND_ECC_BCH
	printk("Using Software BCH ECC for NAND device\n");
	nand->ecc.mode = NAND_ECC_SOFT_BCH;
	nand->ecc.layout = &ecc_layout_4KB_bch4bit;
	/*
	 * Encode each 512B with 4 bit correction, 8 ECC steps.
	 */
	nand->ecc.size		= 512;
...
#else
#ifdef CONFIG_MTD_NAND_LNC_RS_ECC
	printf("Using Reed-Solomon 4-bit ECC for NAND device\n");
	nand->ecc.hwctl		= mv_nand_enable_hwecc;
	nand->ecc.calculate	= mv_nand_calculate_ecc_rs;
	nand->ecc.correct	= mv_nand_correct_data_rs;
	nand->ecc.size		= 512;
	nand->ecc.bytes		= 10;
	nand->ecc.layout	= &mv_nand_rs_oobinfo;
	nand->ecc.mode		= NAND_ECC_HW;
#else /* !CONFIG_MTD_NAND_LNC_RS_ECC */
	printf("Using Hamming 1-bit ECC for NAND device\n");
...
#endif /* defined(CONFIG_CMD_NAND) && defined(CONFIG_MTD_NAND_LNC) */
