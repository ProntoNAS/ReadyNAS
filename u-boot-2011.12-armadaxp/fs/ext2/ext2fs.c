...
static int ext2fs_iterate_dir (ext2fs_node_t dir, char *name, ext2fs_node_t * fnode,
	int *ftype, char *outbuff, int outsize)
{
	unsigned int fpos = 0;
	int status;
...
int ext2fs_find_file1
	...
	for (;;) {
		...
		found = ext2fs_iterate_dir (currnode, name, &currnode, &type, NULL, 0);
		if (found == 0) {
			return (0);
		}
		if (found == -1) {
...
int ext2fs_ls (const char *dirname, char *outbuff, int outsize) {
	...
	ext2fs_iterate_dir (dirnode, NULL, NULL, NULL, outbuff, outsize);
...
int ext2fs_mount (lbaint_t part_length) {
	struct ext2_data *data;
	int status;
