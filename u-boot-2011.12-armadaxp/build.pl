...
getopt('f:b:o:i:v:n:s:a:k:m:j:D:');
...
#	print "\n**** [Configuring Make] version $opt_v\t to board $targetBoard *****\n\n";
	...
	if(defined $opt_i)
	{
       ...
       #Variable support
      print " *** the Type of Main Board ****************";
       if(defined $opt_m)
       {
                @varialbes = split(':', $opt_m);
                foreach $if (@varialbes)
                {
                        if($if eq "MOMIJI_4BAY")
			{
				system("echo \"#define MOMIJI_4BAY\" >> include/config.h");
			}
                        elsif($if eq "MOMIJI_2BAY")
				{
					system("echo \"#define MOMIJI_2BAY\" >> include/config.h");
				}
                       		else {
                                	print " *** Warning: Ignoring unrecognized variable - $if\n";
                       		 }
                }
        }
