...
int nand_load_fdt(int start, size_t size, u_char *addr)
{
	size_t offset;
	int i = 0;
	size_t len;
	...
	/* Find FDT start */
	for (i = start / blocksize; i * blocksize < CONFIG_UBOOT_SIZE + CONFIG_SPARE_AREA; i++) {
		if (nand_block_isbad(&nand_info[0], (i * blocksize)))
			continue;
		len = size;
		if (nand_read_skip_bad(&nand_info[0], (i * blocksize + offset), &len, addr))
			continue;
		if (fdt_check_header(addr) == 0)
			return 0;
	}
