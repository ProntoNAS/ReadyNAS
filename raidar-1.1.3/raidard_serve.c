...
process_status_event(int fd, uint32_t *flags)
{
	...
	if (sscanf(buf, "%63[^=]=%63s", key, val) == 2)
	{
...
get_disk_gb(const char *disk)
{
	...
	** So we should calculate capacity using the reverse of that formula */
	ret = ((strtoll(buf, NULL, 0) - 97696368) / 1953504) + 50;
...
static bool systemd_is_active(const char *unit)
{
	FILE *fp;
	char cmd[128];
	char buf[16];
	...
	snprintf(cmd, sizeof(cmd), "/bin/systemctl is-active %s", unit);
	fp = popen(cmd, "r");
	if (!fp)
		return false;
	if (fscanf(fp, "%15s", buf) == 1)
	{
...
static const char *build_raidar_response(struct sys_info *info, char *sysInfo, size_t len)
{
	...
	{
		if (systemd_is_active("default.target"))
		{
	...
	snprintf(sysInfo, len,  "%s"
				"model!!0!!sn=%s::descr=%s::arch=%s::flags=%X::%s",
				disk_buf, info->serial, info->model, ARCH, flags, info->os_version);
	return boot;
}
	...
	while (!quitting)
	{
		...
		while ((fd = interface_set_fd_next(&read_template)) >= 0)
		{
			...
			if (ntohl(packet.sid) == RAIDARD_LIST_SID)
			{
				...
				boot = build_raidar_response(&info, sysInfo, sizeof(sysInfo));
