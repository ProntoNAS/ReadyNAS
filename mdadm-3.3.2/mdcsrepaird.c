...
static bool dopt, lopt, nopt, ropt, vopt, xopt;
 ...
 * found recently and thus it is to be processed.
...
#if 0
/*
 * Converts path's intra-file (logical) position into disk position.
 * Returns zero for success or errno otherwise.
 * Caller must provide a good buffer for the returned string.
 */
static int fpos2dpos(const char *path, const char *fpos, char *dpos)
{
	int fd, rv = 0;
	__u64 f, d;
	struct fiemap *fiemap = alloca(sizeof(struct fiemap) +
					sizeof(struct fiemap_extent));

	errno = 0;
	f = strtoull(fpos, NULL, 10);
	if (errno)
		return errno;
	...
	if ((fiemap->fm_extents[0].fe_flags & FIEMAP_EXTENT_NOT_ALIGNED) ||
		!fiemap->fm_extents[0].fe_physical)
		return -EIO;

	d = fiemap->fm_extents[0].fe_physical +
			(fiemap->fm_extents[0].fe_logical - f);
	sprintf(dpos, "%llu", d);
	return 0;
}
#endif
static char *find_inum(const char *point, const char *inum, const char *fs)
{
	FILE *fp;
	...
	if (!strcmp(fs, "btrfs"))
		ret = asprintf(&cmd, "find %s -xdev -inum %s", point, inum);
	else
		ret = asprintf(&cmd, "btrfs inspect inode %s %s", inum, point);
	if (ret <= 0) {
					...
					dev_t adev)
{
	...
	while (fgets(line, sizeof line, pp)) {
		...
		if (!stat(subvol, &st) &&
			st.st_dev == adev &&
			(path = find_inum(subvol, inum, "btrfs")))
			break;
		free(subvol);
	}
	pclose(pp);
...
static void parseXML(const char *content)
{
	...
	if (_strcmp(doc->children->name, "mdcsrepair") &&
		_strcmp(doc->children->name, "mdcsrepair_netlink"))
		goto exit;
...
static int nl_cb(struct nl_msg *msg, void *arg)
{
	...
	struct nlattr *attrs[MDCSREPAIR_NL_A_MAX + 1];
	int ret;
	static struct nla_policy mdcs_nl_xml_policy[MDCSREPAIR_NL_A_MAX + 1] = {
		[MDCSREPAIR_NL_A_XML] = { .type = NLA_STRING },
	};
	...
	if (ghdr->cmd != MDCSREPAIR_NL_C_REQUEST)
		return 0;

	if ((ret = genlmsg_parse(nlh, 0, attrs, MDCSREPAIR_NL_A_MAX,
				mdcs_nl_xml_policy)) < 0) {
		__warnx("error parsing netlink message.");
		return ret;
	}
	if (!attrs[MDCSREPAIR_NL_A_XML]) {
		__warnx("unknown format of kernel netlink message.");
		return -EINVAL;
	}
	__warnx("received NETLINK message.");
	parseXML(nla_get_string(attrs[MDCSREPAIR_NL_A_XML]));
	return 0;
}

static struct nl_handle *init_nl(void)
{
	int family;
	struct nl_handle *nh = nl_handle_alloc();
	
	if (!nh)
		__err(2, "failed to allocate netlink handle");

	nl_disable_sequence_check(nh);

	if (genl_connect(nh) < 0)
		__err(2, "failed to connect to netlink socket");
	if ((family = genl_ctrl_resolve(nh, "VFS_MDCSREPAIR")) < 0)
		__err(2, "failed to resolve VFS_MDCSREPAIR netlink name");
	if (nl_socket_add_membership(nh, family) < 0)
		__err(2, "failed to join VFS_MDCSREPAIR multicast group");

	if (nl_socket_modify_cb(nh, NL_CB_VALID, NL_CB_CUSTOM, nl_cb, NULL) < 0)
		__err(2, "failed to register callback for netlink messages");

	return nh;
}

static void help(FILE *fp, const char *argv0, int ec)
{
	fprintf(fp,
...
int main(int argc, char *argv[])
{
	...
	while ((opt = getopt (argc, argv, "?D:dhlno:p:rvx")) != -1) {
		switch (opt) {
		case '?':
		case 'h':
	...
	struct nl_handle *nh = init_nl();

	pthread_attr_init(&thattr);
	if (!ropt && pthread_create(&thid, &thattr, garbage_collector, NULL))
		err(1, "failed to create a garbage collector thread.");

	/* Loop to listen on a NELINK socket. */
	while (true) {
		if ((nl_recvmsgs_default(nh)) < 0)
			__warn("failed to read or parse netlink message");
	}
