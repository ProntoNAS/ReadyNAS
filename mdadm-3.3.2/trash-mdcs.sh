...
#	Demo to repair corrupted RAID component
#	Copyright (c) 2014 NETGEAR
#	Copyright (c) 2014 Hiro Sugawara
#
#	Usage: [SBIN=/usr/sbin] demo-mdcs.sh <file> [<len>@<pos> ...]'
#		level: RAID levels
#
	...
	do
		...
		case "$d" in
			...
			-)
				...
				;;
			*)
				echo "cannot examine filesystem for $f" >&2
				exit 1
			;;
		esac
	done
...
trash() {
	...
	then
		echo "Bad <len>@<pos> '$2'" >&2
		exit 1
	fi

	local debug=`cat /proc/fs/btrfs/mdcsrepair`
	echo :$ino >/proc/fs/btrfs/mdcsrepair
	local blklen=4096
