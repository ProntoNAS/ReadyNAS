 ...
 * 
 * Version: $Id: dquot.c,v 1.1.1.1 2010-09-22 00:23:47 jmaggard Exp $
 * 
...
/*
 * QUOTA hotplugging invokes what /proc/sys/kernel/hotplug says
 * (normally /sbin/hotplug) when QUOTA limits are exceeded.
 */
static inline void quota_hotplug(char *verb, char *reason, struct dquot *dquot)
{
	...
	if (in_interrupt ()) {
        ...
        envp [i++] = scratch;
        scratch += sprintf(scratch, "TYPE=%s", quotatypes[dquot->dq_type]) + 1;
        envp [i++] = scratch;
        scratch += sprintf(scratch, "ID=%d", dquot->dq_id) + 1;
        envp [i++] = scratch;
        scratch += sprintf(scratch, "DEVICE=%s", (char *)dquot->dq_sb->s_id) + 1;
        envp [i++] = scratch;
        scratch += sprintf(scratch, "QUOTA=%d:%d:%llu:%d:%d:%d:%lu:%lu",
...
#ifdef CONFIG_PADRE
#ifdef CONFIG_HOTPLUG
        quota_hotplug("warn", msg + 1, dquot);
#endif /* CONFIG_HOTPLUG */
#endif /* CONFIG_PADRE */
