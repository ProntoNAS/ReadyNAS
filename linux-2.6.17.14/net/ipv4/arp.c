 ...
 *
 * Version:	$Id: arp.c,v 1.1.1.1 2010-09-22 00:23:45 jmaggard Exp $
 *
...
static int is_local_netif(char *ha)
{
        int rc = 0;
...
static void arp_hotplug(char *verb, char *info, char *data);
static void arp_event_handler(void * unused)
{
        //printk("arp_event_handler\n");
        arp_hotplug((char *)&g_verb, (char *)&g_info, (char *)&g_data);
	g_pending = 0;
}

static DECLARE_WORK(arp_event_work, arp_event_handler, 0);

void arp_event_schedule(char *verb, char *info, char *data)
{
        if(!g_pending)
        {
                g_pending = 1;
...
static void arp_hotplug(char *verb, char *info, char *data)
{
	...
	scratch = buf;
        ...
        envp [i++] = scratch;
        scratch += sprintf(scratch, "ARP_INFO=%s", info) + 1;
        envp [i++] = scratch;
        scratch += sprintf(scratch, "ARP_DATA=%s", data) + 1;

    	envp [i++] = 0;

    	/* NOTE: user mode daemons can call the agents too */
    	//printk("run hotplug %s %s\n", argv[0],argv[1]);
...
* someone -- Marc <marcsoft@merlins.org> */
		...
		if (ip_route_input(skb, sip, tip, 0, dev) == 0) {
			...
			//if (addr_type == RTN_LOCAL) {
			if (addr_type == RTN_LOCAL && !is_local_netif(sha)) {
				if (net_ratelimit()) {
#ifdef CONFIG_PADRE
					printk (KERN_WARNING "Uh Oh, MAC address %s claims to have our IP address (%s) (duplicate IP conflict likely)\n", mac2asc(hbuffer,sha,dev->addr_len), in_ntoa(sip));
#ifdef CONFIG_HOTPLUG
					arp_event_schedule("IPCONFLICT", mac2asc(hbuffer,sha,dev->addr_len), in_ntoa(sip));
#endif
#endif
				}
			}
