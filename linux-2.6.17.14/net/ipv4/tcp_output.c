 ...
 *
 * Version:	$Id: tcp_output.c,v 1.1.1.1 2010-09-22 00:23:45 jmaggard Exp $
 *
 * Authors:	Ross Biro
...
#ifdef CONFIG_PADRE
unsigned int tso_buffer_ratio = 4;
unsigned long ec_tcp_retrans = 0;
unsigned long ec_tcp_retrans_unrecover = 0;
int (* check_sock_func1)(struct sock *sk) = NULL;
#endif
...
#ifdef CONFIG_PADRE
#ifdef CONFIG_PADRE_OLDTSO
                if (tp->max_window &&
                    (xmit_size_goal > (tp->max_window * 2 / 3)))
                        xmit_size_goal = max((tp->max_window * 2 / 3),
                                             68U - tp->tcp_header_len);
		if(tp->rcv_wnd < (tp->max_window >> 2))
			xmit_size_goal = tp->rcv_wnd >> 1;
//			xmit_size_goal = max((tp->rcv_wnd * 2 / 3), mss_now);
#else
                if(tp->max_window &&
                    (xmit_size_goal > (tp->max_window * tso_buffer_ratio / 6)))
                        xmit_size_goal = max((tp->max_window * tso_buffer_ratio / 6),
                                             68U - tp->tcp_header_len);
                if(tp->rcv_wnd < (tp->max_window >> 2) && (xmit_size_goal > tp->rcv_wnd >> 1))
                        xmit_size_goal = tp->rcv_wnd >> 1;
#endif
#else
		if (tp->max_window &&
		...
		if(check_sock_func1)
		{
			mss_now = check_sock_func1(sk) ? mss_now : xmit_size_goal;
		}
		else
...
#ifdef CONFIG_PADRE
		if(check_sock_func1 && check_sock_func1(sk))
		{
	                if (skb->len > limit &&
                	...
                	tcp_set_skb_tso_segs(sk, skb, tp->mss_cache);
#else
		if (skb->len > limit &&
		    unlikely(tso_fragment(sk, skb, limit, mss_now)))
			break;
...
#ifdef CONFIG_PADRE
        tso_segs = tcp_init_tso_segs(sk, skb, tp->mss_cache);
#else
	tso_segs = tcp_init_tso_segs(sk, skb, mss_now);
#endif
...
void tcp_simple_retransmit(struct sock *sk)
{
	...
	__u32 daddr = (inet != NULL) ? inet->daddr : 0;
...
#ifdef CONFIG_PADRE
        ec_tcp_retrans++;
        if(icsk->icsk_retransmits > 0 && (daddr != 0x7F000001 && daddr != 0x0100007F))
                ec_tcp_retrans_unrecover++;
#endif
