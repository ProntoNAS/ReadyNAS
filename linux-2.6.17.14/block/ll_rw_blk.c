...
void blk_queue_make_request1(request_queue_t * q, make_request_fn * mfn)
{
	/*
	 * set defaults
	 */
	...
	q->make_request_fn = mfn;
#ifdef CONFIG_PADRE
	q->backing_dev_info.ra_pages = 2*(VM_MAX_READAHEAD * 1024) / PAGE_CACHE_SIZE;
	q->backing_dev_info.state = 0;
	q->backing_dev_info.capabilities = BDI_CAP_MAP_COPY;
	blk_queue_max_sectors(q, (SAFE_MAX_SECTORS+1)*2);
	blk_queue_hardsect_size(q, 1024);
	blk_queue_dma_alignment(q, 1024);
	blk_queue_congestion_threshold(q);
	q->nr_batching = BLK_BATCH_REQ;
	if(padre_NSPIO.load_level)
		q->unplug_thresh = 1;		/* hmm */
...
EXPORT_SYMBOL(blk_queue_make_request1);
...
#ifdef CONFIG_PADRE_DEBUG
{
  ...
  struct bio* bi=bio;
  int i, j, len_size = 0;
  struct bio_vec *theVec;
  if (first_time) {
    j = bi->bi_vcnt;
    for (i = 0; i < j; i++) {
      theVec = bio_iovec_idx(bi,i);
      len_size += (theVec->bv_len >> 9);
    }
    if (len_size != bio_sectors(bi)) {
      printk("%%%%%%%%%%%% bio sector miss-match: %d, %d\n", len_size, bio_sectors(bi));
...
int __init blk_dev_init(void)
{
	...
	requestq_cachep = kmem_cache_create("blkdev_queue",
			sizeof(request_queue_t), CONFIG_PADRE_64BIT, SLAB_PANIC, NULL, NULL);
