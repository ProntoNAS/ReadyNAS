...
static unsigned long fb_mask[] = 
					{ 0, F_L | F_G, F_L, F_G, 0x10, F_U | F_E, F_U | F_G | F_E, F_U | F_L | F_E,
					  F_G | F_L | F_U, F_U | F_L, F_U | F_G, F_U, F_E, F_G | F_E,F_L | F_E,
				   	  F_E | F_L | F_G };	   
...
static int do_one_bmathemu(u32 insn, unsigned long fsr, struct pt_regs *regs);
static int do_one_ldstmathemu(u32 insn, struct pt_regs *regs, unsigned long *fregs,
		   unsigned long *fsr,struct task_struct *fpt);
	...
	if (fpt->thread.fpqdepth == 0) {                   /* no queue, guilty insn is at regs->pc */
		...
		if (!get_user(insn, (u32 *)regs->pc)) {
			if( (insn & 0xc0000000) == 0xc0000000) 
				retcode = do_one_ldstmathemu(insn,regs,fpt->thread.float_regs,
						&fpt->thread.fsr,fpt);
			else if((insn & 0xc0000000) == 0x0)
				retcode = do_one_bmathemu(insn,fpt->thread.fsr,regs);
			else {
...
static inline unsigned long r(unsigned long rs, struct pt_regs *regs,struct task_struct *fpt)
{
	if( rs == 0)
		return 0;
	else if( rs < 16)
		return regs->u_regs[rs];
	else if( rs < 24)
		return ((struct reg_window *)fpt->thread.fpqueue)->locals[rs-16];
	else 
		return ((struct reg_window *)fpt->thread.fpqueue)->ins[rs-24];
}
		                              ...
		                              unsigned int insn)
{
    ...
    if(insn & 0x2000) {
		maybe_flush_windows(rs1, 0);
		return (fetch_reg(rs1, regs) + sign_extend_imm13(insn));
	} else {
		maybe_flush_windows(rs1, rs2);
...
static int do_one_ldstmathemu(u32 insn, struct pt_regs *regs, unsigned long *fregs,
		unsigned long *fsr,struct task_struct *fpt)
{
	unsigned long freg = (insn >> 25) & 0x1f;
	unsigned long addr = compute_effective_address(regs,insn);

#if 0
	if( addr >= 0xfffff000UL) {
		printk("do_one_ldstmathemu w_saved=%d\n",fpt->thread.w_saved);
		printk("insn=%x, regs=%p fregs=%p, fsr=%p, fpt=%p\n",
				insn,regs,fregs,fsr,fpt);
	...
	//printk("|");
#ifdef DEBUG_MATHEMU
	printk("In do_one_ldstmathemu(), emulating %08lx\n", insn);
	printk("In do_one_ldstmathemu(), address %08lx\n", addr);
#endif
	switch((insn >> 19) & 0x3f) {
	case LDDF:	
	 	fregs[freg+1] = *(unsigned long *)(addr+4);
	case LDF:
		fregs[freg] = *(unsigned long *)addr;
		break;
	case LDFSR:
		*fsr = *(unsigned long *)addr;
		break;
	case STDF:
	 	*(unsigned long *)(addr+4) = fregs[freg+1];
	case STF:
		*(unsigned long *)addr = fregs[freg];
		break;
	case STFSR:
		*(unsigned long *)addr = *fsr;
		break;
	default:
#ifdef DEBUG_MATHEMU
...
static int do_one_bmathemu(u32 insn, unsigned long fsr, struct pt_regs *regs)
{
	int  disp22 = ((insn & 0x1fffff) - (insn & 0x200000)) << 2;
	unsigned long annul = insn & 0x20000000;
	unsigned long fcc = (fsr >> 10) & 0x3,eval_fcc;
	unsigned long temp;
	unsigned long op = (insn >> 22) & 0x7f;
	

#ifdef DEBUG_MATHEMU
	printk("In do_one_bmathemu(), emulating %08lx\n", insn);
#endif
	fcc = (1 << fcc) | 0x10;
	temp = ((op & 0xf) == 0x6) ? 0 : 8;
	temp += ((op >> 4) & 0xf);
	
	eval_fcc = fcc & fb_mask[temp];
	if(eval_fcc) {
		temp  = regs->pc+disp22;
		regs->pc = regs->npc;
		if((op == FBA) && annul) {
		...
		regs->pc = annul ? regs->npc + 4 : regs->npc;
		regs->npc = regs->pc + 4;
	}
	return 1;
}
...
#define TYPE(dummy, r, ru, b, bu, a, au) type = (au << 2) | (a << 0) | (bu << 5) | (b << 3) | (ru << 8) | (r << 6)
	...
	*pfsr &= ~0x1c000;				/* clear the traptype bits */
	...
	/* mov */
	case FMOVS: IR = rs2->s;	break;
	case FABSS: IR = rs2->s & 0x7fffffff; break;
	case FNEGS: IR = rs2->s ^ 0x80000000; break;
	/* float to int */
