/*  $Id: process.c,v 1.1.1.1 2010-09-22 00:23:49 jmaggard Exp $
...
#define PADRE_PASEO_GPIO_PSON         5
void machine_halt(void)
{
	...
	local_irq_disable();
        ...
        {
	        writel(1 << 2, PADRE_GPIO_DIR);
       		writel(1 << 2, PADRE_GPIO_DATA);
	}
	else
	{
		writel(1 << PADRE_PASEO_GPIO_PSON, PADRE_GPIO_DIR);
    		writel(1 << PADRE_PASEO_GPIO_PSON, PADRE_GPIO_DATA);
    		udelay(10);
    		writel(0 << PADRE_PASEO_GPIO_PSON, PADRE_GPIO_DIR);
	}
}
...
void machine_restart(char * cmd)
{
	...
	local_irq_disable();
        ...
        {
	        writel(0, 0x70000000);
	        writel(0x8, PADRE_LP_CH_ENABLE);
	        writel(0x8, PADRE_LP_CH_TRIG); //start LP chn
	        while(readl(0x70000000) != 0x55aa)
        	        printk(".");
	}

    	writel(1, PADRE_SYS_CTRL);
	while(1);
}
		...
		struct task_struct *p, struct pt_regs *regs)
{
	...
	if(last_task_used_math == current) {
       	memcpy(&p->thread.float_regs[0], &current->thread.float_regs[0], 32*4);
        p->thread.fsr = current->thread.fsr;
        p->thread.fpqdepth = current->thread.fpqdepth;
        if( current->thread.fpqdepth > 0)
	    	memcpy(&p->thread.fpqueue[0],&current->thread.fpqueue[0],
			       current->thread.fpqdepth * 8);
	}
...
static int padre_hotplug_thread(void *arg)
{
	padre_hotplug_thread_t *thread = (padre_hotplug_thread_t *)arg;
	...
	while(thread->run) {
		...
		run = thread->run;
		if(run)  (*run)(thread->data);
	}
	return 0;
}

void padre_wakeup_hotplug_thread(padre_hotplug_thread_t *thread)
{
...
{
	padre_hotplug_thread_t *thread;
	int ret;
	struct completion event;
	thread =  kmalloc(sizeof(padre_hotplug_thread_t), GFP_KERNEL);
        if (!thread)
		return NULL;
	memset(thread, sizeof(padre_hotplug_thread_t), 0);
	...
	thread->data = data;
	thread->name = name;
	ret = kernel_thread(padre_hotplug_thread, thread, 0);
	if (ret < 0) {
		kfree(thread);
		return NULL;
	}
