...
#ifdef CONFIG_PADRE
//#define CONFIG_PADRE_DEBUG
	if (!uptodate) {
#ifdef CONFIG_PADRE_DEBUG
		char bb[BDEVNAME_SIZE];
		printk("11raid1: %s: rescheduling sector %llu\n",
	       	bdevname(conf->mirrors[mirror].rdev->bdev,bb), (unsigned long long)r1_bio->sector);
#endif
...
#ifdef CONFIG_PADRE_DEBUG
		printk(KERN_ERR "raid1: %s: end rescheduleing sector %llu\n",
       				bdevname(conf->mirrors[mirror].rdev->bdev,b), (unsigned long long)r1_bio->sector);
#endif
	}
...
static int raid1_end_write_request(struct bio *bio, unsigned int bytes_done, int error)
{
	...
	if (!uptodate) {
		char bb[BDEVNAME_SIZE];
		printk("raid1: %s: failing write sector %llu\n",
       		bdevname(conf->mirrors[mirror].rdev->bdev,bb), (unsigned long long)r1_bio->sector);
	}
#endif
		...
		while(sectors) {
			...
			if (success) {
				...
				while (d != r1_bio->read_disk) {
					...
					rdev = conf->mirrors[d].rdev;
#ifdef CONFIG_PADRE
					if (rdev && &rdev->corrected_errors)
					atomic_add(s, &rdev->corrected_errors);
					else
					printk("rdev->corrected_error is NULL, skip record number of errors\n");
#else
					atomic_add(s, &rdev->corrected_errors);
#endif //CONFIG_PADRE
					if (sync_page_io(rdev->bdev,
			...
			if (mddev->ro == 0) while(sectors) {
				...
				if (success) {
					...
					while (d != r1_bio->read_disk) {
						...
						printk("rdev->corrected_error is NULL, skip record the number, %s\n",
						((rdev && rdev->bdev)? bdevname(rdev->bdev,b) : "unknown"));
#else
						atomic_add(s, &rdev->corrected_errors);
#endif //CONFIG_PADRE
						if (rdev &&
