...
extern raid_failed_drives_t *raid_failed_drives;

extern int sysctl_speed_limit_opt;
extern int sysctl_speed_limit_dma;
#ifdef NOT_YET
extern int padre_RXAENGINE(void);
#else
int padre_RXAENGINE(void)
{
#ifdef RXA_SPEEDUP
	int i = 0;
	unsigned int status ;
	while (i++ < 200000) {
		status = readl(PADRE_MC_DMA_FFCTRL(PADRE_DMA_CH_RXAOUT));
		if( !(status & 0x4000) ) {
			return 1;
		}
		if (0 && i > 10)
	...
	printk("RXA engine dead loop, DATA ERROR!\n");
	return 0;
#else
	return 1;
#endif
...
extern void dump_buf(void *buf, int length, unsigned long base);
extern void init_dj_buf(raid5_conf_t *conf);
#endif //CONFIG_PADRE
...
#ifdef CONFIG_PADRE
int grow_stripes(raid5_conf_t *conf, int num)
#else
...
#ifdef CONFIG_PADRE
void shrink_stripes(raid5_conf_t *conf)
#else
...
static void error(mddev_t *mddev, mdk_rdev_t *rdev)
{
	...
	raid5_conf_t *conf = (raid5_conf_t *) mddev->private;
        ...
        mdp_super_t *sb = sb = (mdp_super_t*)page_address(rdev->sb_page);
...
#ifdef CONFIG_HOTPLUG
                //This is needed in 2.4, come back to fix it.
                md_hotplug("failure", mddev, sb->disks+disk->number);
#endif /* CONFIG_HOTPLUG */
#endif /* CONFIG_PADRE */
 ...
 */
#ifdef CONFIG_PADRE
sector_t raid5_compute_sector(sector_t r_sector, unsigned int raid_disks,
#else
...
#ifndef CONFIG_PADRE
                        if (frombio) {
                                if (sysctl_speed_limit_dma && padre_NSPIO.mport_proc)
                                        padre_NSPIO.mport_proc(padre_3_md,
                                        (void*)(pa+page_offset),
                                        (void*)(ba+b_offset), (void *)clen);
                                else
...
int make_request_save(request_queue_t *q, struct bio * bi, int show);

static int make_request(request_queue_t *q, struct bio * bi)
{
	mddev_t *mddev = q->queuedata;
	raid5_conf_t *conf = mddev_to_conf(mddev);
	int s_s = STRIPE_SECTORS;
...
#ifdef OLD_CODE
	if ( (*padre_NSPIO.ssl_opt) /* && rw !=WRITE  &&  
		!conf->resync_parity && !conf->failed_disks && 
		conf->buffer_size == PAGE_SIZE */) {

		if (0) printk("RAID5: %p %p %p %p %p %p.\n",
                __builtin_return_address(0),__builtin_return_address(1),
                __builtin_return_address(2),__builtin_return_address(3),
                __builtin_return_address(4),__builtin_return_address(5));

		if (padre_NSPIO.mdjnm_proc && !(padre_NSPIO.mdjnm_proc(padre_5_rc,
		(void *)conf,(void *)mddev,(void *)q,(void *)bi,(void *)&s_s))) {
			if (!(*padre_NSPIO.ssl_dbg & 0x8000000))
          		return 0;
        	}
	}
#else
...
#endif //OLD_CODE
	return make_request_save(q, bi, 0);
}
...
#ifdef CONFIG_PADRE
	if (*padre_NSPIO.ssl_opt) {
		if (!(*padre_NSPIO.ssl_dbg & 0x8000000 /*DBG_TURBO_MERG*/)) {
			if (padre_NSPIO.mdjnm_proc && !(padre_NSPIO.mdjnm_proc(padre_5_ts,
			(void *)0,(void *)mddev,(void *)0,(void *)0,(void *)0))) {
				//printk("Failed to send raid5 req to TURBO\n");
        		}
		}
	}
	if (mddev->degraded > 1) 
		printk("raid5d: on lifesupport.\n");
#endif //CONFIG_PADRE
...
#ifdef CONFIG_PADRE
	padre_NSPIO.init(padre_1_RTE, conf);
#endif //CONFIG_PADRE
	return 0;
abort:
	if (conf) {
...
#ifdef CONFIG_PADRE
EXPORT_SYMBOL(shrink_stripes);
EXPORT_SYMBOL(grow_stripes);
EXPORT_SYMBOL(raid5_compute_sector);
EXPORT_SYMBOL(make_request_save);
#endif //CONFIG_PADRE
