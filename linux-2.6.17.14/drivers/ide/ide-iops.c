...
{
        printk("PROBLEM!!!!!!, need 32bits, return 16 bytes.\n");
        return (u32) IN_WORD(port);
}
#else
static u32 ide_inl (unsigned long port)
{
...
int drive_is_ready (ide_drive_t *drive)
{
...
#else
	{
		printk("This drive %p is waiting LP DMA, ",drive);
		printk("will let LP handle intrs.\n");
...
#ifdef CONFIG_PADRE
	{
		int counter = 0;
		while (counter++ < 50) {
			if (!padre_NSPIO.init(padre_1_sir,
				(void *)HWIF(drive))) return 0;
			if (stat & BUSY_STAT) {
				mdelay(100);
				stat = hwif->INB(IDE_STATUS_REG);
			}
...
int ide_wait_stat (ide_startstop_t *startstop, ide_drive_t *drive, u8 good, u8 bad, unsigned long timeout)
{
	...
	if (!padre_NSPIO.init(padre_1_sir, (void *)HWIF(drive))) {
		printk("wait_stat: Drive %s is missing.\n",drive->name);
		*startstop = ide_error(drive, "interface error", stat);
		return 1;
	}
#endif
