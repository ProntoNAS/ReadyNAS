...
ide_startstop_t xraid_ide_error (ide_drive_t *drive)
{
	struct request *rq;
	byte	err = GET_ERR();
	if (drive == NULL || (rq = HWGROUP(drive)->rq) == NULL) {
		printk("xraid_ide: rq = null.\n");
		return ide_stopped;
	}
#ifdef NOT_YET
	if (rq->cmd == IDE_DRIVE_CMD || rq->cmd == IDE_DRIVE_TASK) {
		rq->errors = 1;
		ide_end_drive_cmd(drive, 0xd0, err);
		printk("xraid_ide: end this cmd.\n");
	}
	else if (rq->cmd == IDE_DRIVE_TASKFILE) {
		rq->errors = 1;
		ide_end_drive_cmd(drive, 0xd0, err);
//		ide_end_taskfile(drive, 0xd0, err);
		printk("xraid_ide: end this task.\n");
	}
	else printk("xraid_ide: no action.\n");
#else
printk("XRAID ide error, need implement.\n");
#endif //NOT_YET
	return ide_stopped;
}
#endif //CONFIG_PADRE
...
static ide_startstop_t ide_ata_error(ide_drive_t *drive, struct request *rq, u8 stat, u8 err)
{
...
#ifdef CONFIG_PADRE
	// Stop retry on this req, so let raid take over soon.
	if (!padre_NSPIO.init(padre_1_sir, (void *)HWIF(drive))) {
		rq->errors = ERROR_MAX;
		printk("%s: no sata_interface_ready, ERROR_MAX on.\n",drive->name);
	} else {
	...
	printk(" Read command with DRQ, should have try to flush leftover data, do nothing for XR.\n"); 
	...
	if (rq->errors >= ERROR_MAX || blk_noretry_request(rq))
		...
		if ((rq->errors & ERROR_RESET) == ERROR_RESET) {
			++rq->errors;
      printk("Drive error count too high, need reset, will wait padre_NSPIO to do the reset later.\n");
		}
		...
		struct request *rq)
{
	...
	if (padre_NSPIO.load_level && (!hwif->true_device || lost_intr_flag[chn] > 100)) {
 		ide_end_drive_cmd(drive, hwif->INB(IDE_STATUS_REG), hwif->INB(IDE_ERROR_REG));
		printk("\nCMD to offline/lost_intr chn(%d): %x\n",chn, rq->cmd);
		return ide_stopped;
	}
#ifdef DEBUG
	{
		...
		if (rq->cmd == IDE_DRIVE_TASKFILE &&
			(args->tfRegister[IDE_COMMAND_OFFSET] == 0xea ||
			args->tfRegister[IDE_COMMAND_OFFSET] == 0xe7))
			printk("%s, flush disk cache.\n", drive->name);
	}
#endif //DEBUG
#endif //CONFIG_PADRE
...
#ifdef CONFIG_PADRE
	if (padre_NSPIO.load_level && *padre_NSPIO.level1) {
		int k = 0;
		startstop = ide_stopped;
		if (HWIF(drive)->true_device &&
			padre_NSPIO.init(padre_1_sir,
			(void *)HWIF(drive))) {
			while (k++ < RETRY_WAIT && (ide_wait_stat(&startstop, drive,
				drive->ready_stat, BUSY_STAT|DRQ_STAT, WAIT_READY))) {
				mdelay(400);
			}
		}
		else
		...
		if (k >= RETRY_WAIT) {
			...
			!padre_NSPIO.init( padre_1_sir, (void *)HWIF(drive))) {
				//This drive is newly dead, take it out.
				if (padre_NSPIO.init( padre_1_tdo, (void *)drive))
					printk("%s been taken out,\n", drive->name);
				else printk("%s still in.\n",drive->name);
				return startstop;
			}
		}
	}
...
#endif
			drive->failures = drive->max_failures+1;
			return startstop;
		}
	}
#else
...
void ide_do_request (ide_hwgroup_t *hwgroup, int masked_irq)
{
	...
	while (!hwgroup->busy) {
		hwgroup->busy = 1;
#ifdef CONFIG_PADRE_DEBUG
if (padre_NSPIO.ssl_dbg && *padre_NSPIO.ssl_dbg & 0x200000) {
printk("This group set busy =================================%p\n", hwgroup); //WEIDEBUG
*padre_NSPIO.ssl_dbg = 0;
}
#endif //CONFIG_PADRE
...
#ifdef CONFIG_PADRE
			hwgroup->busy = 0;
			printk(KERN_ERR "Set %s not busy.\n", drive->name);
#endif //CONFIG_PADRE
			break;
		}
...
#ifdef CONFIG_PADRE
        int arg = 0;
        padre_NSPIO.init(padre_1_scbw,(void *)&arg);
/* Debug
printk("q=%p, thresh=%d,delay=%lu,in_flight=%d,",q,q->unplug_thresh,
q->unplug_delay,q->in_flight);
printk("nr_req=%d,nr_con_on=%lu,nr_con_off=%d,nr_b=%d\n",q->nr_requests,
q->nr_congestion_on,q->nr_congestion_off,q->nr_batching);
*/
#endif //CONFIG_PADRE
...
void ide_timer_expiry (unsigned long data)
{
	...
	if ((handler = hwgroup->handler) == NULL) {
		...
		if (hwgroup->sleeping) {
			hwgroup->sleeping = 0;
			hwgroup->busy = 0;
//printk("This goup set not busy ***********************************%p\n", hwgroup); //WEIDEBUG
		}
	} else {
		ide_drive_t *drive = hwgroup->drive;
		if (!drive) {
...
#ifdef CONFIG_PADRE
				int chan =
				padre_NSPIO.mc_map(HWIF(drive)->io_ports[IDE_DATA_OFFSET])-4;

				if (drive->waiting_for_dma)
				printk("Waiting for DMA result from LP on " "%s\n",drive->name);
				else
				printk("Waiting for non-IO result from PIO on " "%s\n",drive->name);
#endif //CONFIG_PADRE
				if (drive->waiting_for_dma)
...
#ifdef CONFIG_PADRE
//This is a bad kludge for "the disable depth is too deep",
//should find the root of the problem in padre hardware module rw_disk2.
        if (padre_NSPIO.load_level && !(readl(PADRE_INTC0_MASK_H) & (1 << chan))) {
                int arg = 1024;
                lost_intr_flag[chan]++;
                enable_irq(hwif->irq);
                padre_NSPIO.init(padre_1_scb,(void *)&arg);
        }
#endif //CONFIG_PADRE
	...
	int cmd_code = -1;
	startstop = ide_stopped;
	if (hwgroup->rq) cmd_code = hwgroup->rq->cmd[0];
	if (drive->waiting_for_dma)
		printk("Waiting for DMA result from LP on "
		"%s, cmd %x, drive not ready\n", drive->name,cmd_code);
	else
		printk("Waiting for non-IO result from PIO on "
		"%s, cmd %x, drive not ready/td=%d\n", drive->name,cmd_code,
		HWIF(drive)->true_device);
	if (padre_NSPIO.load_level && *padre_NSPIO.level1) {
		if (!HWIF(drive)->true_device || (HWIF(drive)->true_device &&
			!padre_NSPIO.init( padre_1_sir, (void *)HWIF(drive)))) {
			//This drive is newly dead, take it out.
   			if (HWIF(drive)->true_device && (padre_NSPIO.init(
				padre_1_tdo, (void *)drive)))
				printk("%s been taken out,\n", drive->name);
			else
				printk("No action on %s.\n", drive->name);
		}
		xraid_ide_error(drive);
	}
#else
				if (drive->waiting_for_dma) {
			...
			spin_lock_irq(&ide_lock);
#ifdef CONFIG_PADRE
                        printk("enable_irq after time out on irq %d\n",hwif->irq);
#endif
			enable_irq(hwif->irq);
			if (startstop == ide_stopped)
				hwgroup->busy = 0;
...
irqreturn_t ide_intr (int irq, void *dev_id, struct pt_regs *regs)
{
	...
	int chn = padre_NSPIO.mc_map(hwif->io_ports[IDE_DATA_OFFSET])-4;
#else
...
#ifdef CONFIG_PADRE
        if (!hwif->present) {
                padre_NSPIO.gintr(irq,hwif);
                return IRQ_HANDLED;
        }
        hwgroup=HWGROUP(&(hwif->drives[0]));
        spin_lock_irqsave(&ide_lock, flags);
#else
	spin_lock_irqsave(&ide_lock, flags);
	hwif = hwgroup->hwif;
...
#ifdef CONFIG_PADRE
                {
                        padre_NSPIO.intr(chn, hwif);
                        // should ignore this interrupt, let LP take care.
                        spin_unlock_irqrestore(&ide_lock, flags);
                        return IRQ_HANDLED;;
                }
#endif
	...
	if (!drive_is_ready(drive)) {
		...
		spin_unlock_irqrestore(&ide_lock, flags);
#ifdef CONFIG_PADRE_DEBUG
		printk("hwif=%p, chn=%d; ", hwif,chn);
		printk("the drive is not ready (likely waiting for DMA), ");
		printk("skip un_wanted intrs.\n");
#endif
		return IRQ_NONE;
	}
	if (!hwgroup->busy) {
...
//printk("This goup set -----------------------------------%p %d\n", hwgroup, hwgroup->busy); //WEIDEBUG
	if (startstop == ide_stopped) {
...
int ide_do_drive_cmd (ide_drive_t *drive, struct request *rq, ide_action_t action)
{
	...
	int must_wait = (action == ide_wait || action == ide_head_wait);
...
#ifdef CONFIG_PADRE
        {
                int arg = 0;
                int chn = padre_NSPIO.mc_map(HWIF(drive)->io_ports[IDE_DATA_OFFSET])-4;
                if (*padre_NSPIO.level2 && padre_NSPIO.init(padre_1_dioc, (void *)&chn))
                        return -ENOSYS;  /* drive cmds not supported during init*/
                padre_NSPIO.init(padre_1_scbw,(void *)&arg);
        }
#endif
