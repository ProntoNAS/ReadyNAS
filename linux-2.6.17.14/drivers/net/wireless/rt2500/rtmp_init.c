...
//    {PCICSR,    0x000003b8},        // 0x8c, alexsu : PCI control register
    {PCICSR,    0x000002f8},        // Infrnat 9=MWRI 8=MRDM 7=CLKRUN 6:5=BL 4:3=TxTH 2:1=RxTH 0=BE
    ...
    IN  PRTMP_ADAPTER   pAd)
{
    ...
    // Allocate Ring data DMA blocks
    //Infrant:bug#1394:: Add +0x40
    ring_data = pci_alloc_consistent(pAd->pPci_Dev, (RX_RING_SIZE * RX_BUFFER_SIZE + 0x40), &ring_data_dma);
    ...
    // Rx ring & DMA buffer
    ring_data += 4;                                             // Infrant:bug#1394
    ring_data_dma += 4;                                         // Infrant:bug#1394
...
err_out_allocate_beaconring:
    ...
    // Infrant:bug#1394
    pci_free_consistent(pAd->pPci_Dev, (RX_RING_SIZE * RX_BUFFER_SIZE + 0x40),
        pAd->RxRing[0].va_data_addr-4, pAd->RxRing[0].pa_data_addr-4);
    ...
    IN  PRTMP_ADAPTER   pAdapter)
{
    ...
    // Value from 0 - 31. Default value is 24.
    // Krellan: Log values read from EEPROM, to aid troubleshooting
    printk(KERN_DEBUG DRV_NAME " EEPROM:");
    for (i = 0; i < NUM_EEPROM_TX_PARMS; i++)
    {
        printk("%3d%3d", (i * 2) + 1, (i * 2) + 2);
    }
    printk("  Channel\n");
    printk(KERN_DEBUG DRV_NAME " EEPROM:");
    for (i = 0; i < NUM_EEPROM_TX_PARMS; i++)
    {
        ...
        by0 = Power.field.Byte0;
        by1 = Power.field.Byte1;
        pAdapter->PortCfg.ChannelTxPower[i * 2]     = ((by0 > 32) ? 24 : by0);
        pAdapter->PortCfg.ChannelTxPower[i * 2 + 1] = ((by1 > 32) ? 24 : by1);
		  // Krellan: This is the theoretical max dBm value possible (raw
		  // power 31), which will most likely be a small positive number
		  // of dBm, since we scale a user setting of 0 dBm to correspond
		  // to exactly the recommended value from the EEPROM, a raw
		  // power value usually just a little bit under the max of 31.
        printk("%3d%3d", 31 - ((int)by0), 31 - ((int)by1));
    }
    printk("  dBm Maximum\n");
    ...
    IN  PCHAR   buffer)
{
    ...
    temp_buf1 = kmalloc(600, GFP_KERNEL);
    temp_buf2 = kmalloc(600, GFP_KERNEL);
        ...
        else
        {
            ...
            if (srcf->f_op && srcf->f_op->read) 
            {
				...
				DBGPRINT(RT_DEBUG_INFO, "FAST_PSP power mode not available - defaulting to CAM\n");
                    ...
                    //WPANONE_KEY
                    if (RTMPGetKeyParameter("Default", "WPANONE", tmpbuf, 255, buffer))
                    {
					    Set_WPANONE_Proc(pAd, tmpbuf);
                    }
                    //WPAPSK_KEY
                    ...
                    //Key1Str
                    if (RTMPGetKeyParameter("Default", "Key1", tmpbuf, 26, buffer) ||
                        RTMPGetKeyParameter("Default", "Key1Str", tmpbuf, 26, buffer))
                    {
                    ...
                    //Key2Str
                    if (RTMPGetKeyParameter("Default", "Key2", tmpbuf, 26, buffer) ||
                        RTMPGetKeyParameter("Default", "Key2Str", tmpbuf, 26, buffer))
                    {
                        KeyLen = strlen(tmpbuf);
                        switch (KeyLen)
                        {
                    ...
                    //Key3Str
                    if (RTMPGetKeyParameter("Default", "Key3", tmpbuf, 26, buffer) ||
                        RTMPGetKeyParameter("Default", "Key3Str", tmpbuf, 26, buffer))
                    {
                        KeyLen = strlen(tmpbuf);
                        switch (KeyLen)
                        {
                    ...
                    //Key4Str
                    if (RTMPGetKeyParameter("Default", "Key4", tmpbuf, 26, buffer) ||
                        RTMPGetKeyParameter("Default", "Key4Str", tmpbuf, 26, buffer))
                    {
                        KeyLen = strlen(tmpbuf);
                        switch (KeyLen)
                        {
...
//  pAdapter->PortCfg.NumOfTxPowerLevel = 1;
    ...
    // Krellan: Init to automatic settings, starting at 0 dBm
    pAdapter->PortCfg.TxPowerDriver = 0;
    pAdapter->PortCfg.TxPowerUser = 0;
    pAdapter->PortCfg.TxPowerAuto = TRUE;
