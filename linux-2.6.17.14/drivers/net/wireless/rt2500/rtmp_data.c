	...
	IN  UCHAR           AccessCategory)
{
    ...
    RTMP_IO_WRITE32(pAdapter, SECCSR0, 0x1);                // Infrant But#1407:: Dummy to flash pci-buffer
	...
	{
		DBGPRINT(RT_DEBUG_ERROR,"RTMPHardEncrypt --> Ndis Packet buffer error !!!\n");
		// Make sure to release Tx ring resource
		spin_unlock_irqrestore(&pAdapter->TxRingLock, irqflag);
		return (NDIS_STATUS_FAILURE);
	}
	...
	do
	{
		...
		else if ((pAdapter->PortCfg.WepStatus == Ndis802_11Encryption2Enabled) && (pWpaKey != NULL))
		{
            ...
            // Prepare 8 bytes TKIP encapsulation for MPDU
            {
				...
				tkipIv.IV32 = 0;			// (ensure reserved fields clear)
