		   ...
		   struct iw_point *data, char *extra)
{
	...
	range->avg_qual.level = pAdapter->PortCfg.LastRssi;
        range->avg_qual.level -= pAdapter->PortCfg.RssiToDbm;   // Infrant: convert to dBm
...
//#endif
	    ...
	    // Krellan: Get TxPower in dBm now, not percentage
	    {
			 ...
			 wrq->u.txpower.disabled = FALSE;
	    }
	    break;
#endif
        case SIOCSIWTXPOW:  //set transmit power (dBm)
#if WIRELESS_EXT >= 17
	    // Krellan: Set TxPower in dBm now, not percentage
	    {
          int Value;

			 if (wrq->u.txpower.disabled)
			 {
			 	 // Krellan: I don't know how to turn off the radio entirely
			 	 Status = -EOPNOTSUPP;
			 	 printk(KERN_ERR DRV_NAME " SIOCSIWTXPOW: Setting txpower off is not supported\n");
			 }
			 else
			 {
				 if (wrq->u.txpower.fixed)
				 {
					 ...
					 {
						  Status = -EINVAL;
						  printk(KERN_ERR DRV_NAME " SIOCSIWTXPOW: Value not in IW_TXPOW_DBM units\n");
					 }
					 else
					 {
						 ...
						 {
						    Status = -EINVAL;
						    printk(KERN_ERR DRV_NAME " SIOCSIWTXPOW: Value out of range [%d,%d]\n",
						        MIN_TXPOWER_DBM, MAX_TXPOWER_DBM);
						 }
						 else
						 {
							 pAdapter->PortCfg.TxPowerUser = Value;
							 pAdapter->PortCfg.TxPowerAuto = FALSE;
						 }
					 }
				 }
				 else
    ...
    IN OUT struct iwreq    *wrq)
{
    ...
    if (wrq->u.data.length) /* Argument : set the state. */
    {
