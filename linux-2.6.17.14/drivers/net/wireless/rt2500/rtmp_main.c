...
#define DEVNAME "wlan%d"
char uevent_helper[] = "/sbin/hotplug";
 ...
 * Calls /etc/hotplug
 */
static void wireless_hotplug(char *verb, char *state, char *wlan)
{
	...
	if (in_interrupt ()) {
        	printk (KERN_DEBUG __FILE__ "In_interrupt");
        	return;
    	}

	if(!(envp = (char **) kmalloc (20 * sizeof (char *), GFP_KERNEL))) {
		printk (KERN_DEBUG __FILE__ ":enomem");
		return;
	}

	if(!(buf = kmalloc (256, GFP_KERNEL))) {
		kfree (envp);
		printk (KERN_DEBUG __FILE__ ":enomem2");
		return;
	}
	...
	envp [i++] = scratch;
	scratch += sprintf(scratch, "STATE=%s", state) + 1;
	envp [i++] = scratch;
	scratch += sprintf(scratch, "DEVICE=%s", wlan) + 1;
	envp [i++] = scratch;
	...
	kfree (envp);
	if (value != 0)
		printk (KERN_INFO __FILE__ ":hotplug wireless returned 0x%x", value);
}
    ...
    IN  const struct pci_device_id  *ent)
{
    ...
    // Allocate DMA descriptors & buffers
    ...
    // Infrant
#ifdef CONFIG_PADRE
    pci_bus_write_config_byte(pPci_Dev->bus, pPci_Dev->devfn, PCI_CACHE_LINE_SIZE, 0);
    *(volatile ULONG*)0x6000010c = 0x302c;                      // 3=NOPF 2=PCION 1=B64 0=M66EN
#endif // CONFIG_PADRE
...
#ifdef CONFIG_PADRE
#ifdef CONFIG_HOTPLUG
    //wireless_hotplug("link", "up", net_dev->name);
#endif // CONFIG_HOTPLUG
#endif // CONFIG_PADRE
    ...
    IN  struct net_device *net_dev)
{
    ...
    //Infrant
    pAd->iw_stats.qual.level = pAd->PortCfg.LastRssi - pAd->PortCfg.RssiToDbm;
...
#ifdef CONFIG_PADRE
#ifdef CONFIG_HOTPLUG
    //wireless_hotplug("link", "down", net_dev->name);
#endif // CONFIG_HOTPLUG
#endif // CONFIG_PADRE
