...
/*
 * USB printer hotplugging invokes what /proc/sys/kernel/hotplug says
 * (normally /sbin/hotplug) when a printer device is registered.
 */
static inline void printer_hotplug(char *verb, struct usblp *usblp)
{
	...
	printk("USB: %s %s printer %s.\n", usblp->dev->manufacturer,
		usblp->dev->product, verb);
	...
	envp [i++] = scratch;
	scratch += sprintf(scratch, "DEVICE=%d", usblp->minor) + 1;
	envp [i++] = scratch;
	scratch += sprintf(scratch, "ID=%s", &usblp->device_id_string[2]) + 1;
	envp [i++] = scratch;
	scratch += sprintf(scratch, "PORT=%d", (usblp->dev->devpath[0] - '0' -1) & 1) + 1;
	envp [i++] = scratch;
	scratch += sprintf(scratch, "SERIAL=%s", (usblp->dev->serial)) + 1;
	envp [i++] = scratch;
	...
	kfree (envp);
	if (value != 0)
		dbg("hotplug printer returned 0x%x", value);
}
...
#ifdef CONFIG_PADRE
	if (g_sys_type==SYSTEM_TYPE_PZERO && padre_boot_param.vpd_extend[0]==3) {
		int port = (usblp->dev->devpath[0] - '0' - 1) & 1;
		padre_led_handler.do_mount_led_ctrl(port+4, 2);
	}
#endif //CONFIG_PADRE
...
#ifdef CONFIG_PADRE
#ifdef CONFIG_HOTPLUG
	printer_hotplug("disconnect", usblp);
#endif // CONFIG_HOTPLUG
#endif // CONFIG_PADRE
...
static int usblp_release(struct inode *inode, struct file *file)
{
	struct usblp *usblp = file->private_data;
        ...
        if (g_sys_type==SYSTEM_TYPE_PZERO && padre_boot_param.vpd_extend[0]==3) {
		int port = (usblp->dev->devpath[0] - '0' - 1) & 1;
		padre_led_handler.do_mount_led_ctrl(port+4, 1);
	}
#endif //CONFIG_PADRE
