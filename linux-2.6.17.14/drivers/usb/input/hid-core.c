...
/*
 * USB UPS hotplugging invokes what /proc/sys/kernel/hotplug says
 * (normally /sbin/hotplug) when a USB UPS device is registered.
 */
static inline void ups_hotplug(char *verb, int device, unsigned char *id, int bus, int devno)
{
	...
	if (!uevent_helper [0])
		return;
	if (in_interrupt ()) {
		printk("Printer hotplug failed - In interrupt\n");
		return;
	}
	if(!(envp = (char **) kmalloc (2048, GFP_KERNEL))) {
		dbg("enomem");
		return;
	}
	if(!(buf = kmalloc (2048, GFP_KERNEL))) {
		kfree (envp);
		dbg("enomem2");
		return;
	}
	...
	envp [i++] = scratch;
	scratch += sprintf(scratch, "DEVICE=%d", device) + 1;
	envp [i++] = scratch;
	scratch += sprintf(scratch, "ID=%s", id) + 1;
	envp [i++] = scratch;
	scratch += sprintf(scratch, "PORT=%03d/%03d", bus, devno) + 1;
	envp [i++] = scratch;
	envp [i++] = 0;
	...
	kfree (envp);
	if (value != 0)
		dbg("hotplug ups returned 0x%x", value);
}
...
#ifdef CONFIG_PADRE
#define USB_VENDOR_ID_CPS               0x0764
#define USB_DEVICE_ID_CPS_UPS           0x0005

#define USB_VENDOR_ID_BELKIN            0x050d
#define USB_DEVICE_ID_BELKIN_UPS        0x0551
#define USB_DEVICE_ID_BELKIN_UPS1       0x0375
#define USB_DEVICE_ID_BELKIN_UPS2       0x0751
#define USB_DEVICE_ID_BELKIN_UPS3       0x0900
#define USB_DEVICE_ID_BELKIN_UPS4       0x0910
#define USB_DEVICE_ID_BELKIN_UPS5       0x0912
#define USB_DEVICE_ID_BELKIN_UPS6       0x0930
#define USB_DEVICE_ID_BELKIN_UPS7       0x0950
#define USB_DEVICE_ID_BELKIN_UPS8       0x0980
#define USB_DEVICE_ID_BELKIN_UPS9       0x0f51
#endif // CONFIG_PADRE
...
static const struct hid_blacklist {
	...
	{ USB_VENDOR_ID_CPS, USB_DEVICE_ID_CPS_UPS, HID_QUIRK_HIDDEV },
	{ USB_VENDOR_ID_BELKIN, USB_DEVICE_ID_BELKIN_UPS, HID_QUIRK_UPS },
	{ USB_VENDOR_ID_BELKIN, USB_DEVICE_ID_BELKIN_UPS1, HID_QUIRK_UPS },
	{ USB_VENDOR_ID_BELKIN, USB_DEVICE_ID_BELKIN_UPS2, HID_QUIRK_UPS },
	{ USB_VENDOR_ID_BELKIN, USB_DEVICE_ID_BELKIN_UPS3, HID_QUIRK_UPS },
	{ USB_VENDOR_ID_BELKIN, USB_DEVICE_ID_BELKIN_UPS4, HID_QUIRK_UPS },
	{ USB_VENDOR_ID_BELKIN, USB_DEVICE_ID_BELKIN_UPS5, HID_QUIRK_UPS },
	{ USB_VENDOR_ID_BELKIN, USB_DEVICE_ID_BELKIN_UPS6, HID_QUIRK_UPS },
	{ USB_VENDOR_ID_BELKIN, USB_DEVICE_ID_BELKIN_UPS7, HID_QUIRK_UPS },
	{ USB_VENDOR_ID_BELKIN, USB_DEVICE_ID_BELKIN_UPS8, HID_QUIRK_UPS },
	{ USB_VENDOR_ID_BELKIN, USB_DEVICE_ID_BELKIN_UPS9, HID_QUIRK_UPS },
#endif // CONFIG_PADRE
...
#ifdef CONFIG_PADRE
	if (quirks & HID_QUIRK_UPS) {
		printk("USB: Using newhidups driver for %s %s.\n", dev->manufacturer, dev->product);
		ups_hotplug("connect", n, "NEWHIDUPS_UPS", dev->bus->busnum, dev->devnum);
		return NULL;
	}
#endif // CONFIG_PADRE
...
#ifdef CONFIG_HOTPLUG
	for (i = 0; i < hid->maxcollection; i++) {
		if(hid->collection[i].usage == UPS_USAGE ||
		   hid->collection[i].usage == POWER_USAGE) {
			ups_hotplug("connect", hid->minor, hid->name, 0, 0);
			break;
		}
	}
#endif // CONFIG_HOTPLUG
