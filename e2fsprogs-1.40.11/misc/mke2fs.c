...
//JM: Define to enable printing percentage to ReadyNAS LCD.
#define INFRANT
...
#ifdef INFRANT
int infrant_print_lcd (unsigned int fixed_percent) {
	int lcd_cmd;
	FILE *lcd;
	int cmd=0xC0; // Set LCD position to beginning of line 2
	struct stat lcd_present;

	/* Return if the percentage hasn't changed */
	if( last_pct == fixed_percent )
		return(0);
	last_pct = fixed_percent;

        /* Just return if there is no LCD proc */
        if( stat("/proc/sys/dev/lcd/", &lcd_present) == -1 )
                return(10);
	/*
	 * Turn on backlight and print percentage on LCD panel.
	 */
	lcd_cmd=open("/dev/lcd", O_RDONLY|O_NONBLOCK);
	if( lcd_cmd < 0 ) {
		return(1);
	}
	if( ioctl(lcd_cmd, 1, &cmd) != 0 ) {
		close(lcd_cmd);
		return(1);
	}
	cmd = 1;
	ioctl(lcd_cmd, 16, &cmd);
	close(lcd_cmd);
	lcd=fopen("/dev/lcd", "w");
	if(!lcd) {
		return(1);
	}
	fprintf(lcd, "Create %c: %3d.%d%%", toupper(device_name[5]), (fixed_percent / 10), (fixed_percent % 10));
	fclose(lcd);
...
static void progress_update(struct progress_struct *progress, __u32 val)
{
	if ((progress->format[0] == 0) || progress->skip_progress)
		return;
#ifdef INFRANT
	infrant_print_lcd((val * 1000)/progress->max);
#endif // INFRANT
