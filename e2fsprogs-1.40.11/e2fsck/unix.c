...
static void check_if_skip(e2fsck_t ctx)
{
	...
	if ((fs->super->s_state & EXT2_ERROR_FS) ||
		...
		printf(_("\007\007\007\007WARNING!!  "
			 "Primary superblock features different from backup.\007\007\007\n"
			 "This should be okay, just copy primary to backup.\n"));
...
#ifdef INFRANT
int infrant_print_lcd (float percent) {
	int lcd_cmd;
	FILE *lcd;
	...
	fprintf(lcd, "FS check: %5.1f%%", percent);
	fclose(lcd);
...
#ifdef INFRANT
	infrant_print_lcd(percent);
	if (ctx->progress_fd == -1)
		return 0;
#endif // INFRANT
	/*
...
#ifdef INFRANT
			if (!ctx->progress_fd || ctx->progress_fd == -1)
#else //INFRANT
			if (ctx->progress_fd < 0) {
				ctx->progress = 0;
...
no_journal:
	...
	if (check_backup_super_block(ctx)) {
		fs->flags &= ~EXT2_FLAG_MASTER_SB_ONLY;
		unlink("/var/log/frontview/.newly_expanded");
		printf(_("\007\007\007\007WARNING!!  "
			 "Primary superblock features different from backup.\007\007\007\n"
			 "This should be okay, just copy primary to backup.\n"));
