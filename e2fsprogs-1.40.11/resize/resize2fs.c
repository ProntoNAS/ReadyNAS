...
static unsigned int reduce_fs = 0;
#endif // INFRANT
...
#ifdef INFRANT
	if (rfs->new_fs->super->s_blocks_count < rfs->old_fs->super->s_blocks_count)
		reduce_fs = 1;
#endif // INFRANT
...
int infrant_print_lcd (unsigned int fixed_percent) {
	...
	fprintf(lcd, "%s:%3d.%d%%", 
		reduce_fs ? "Reduce FS" : "Expansion",
		(fixed_percent / 10), (fixed_percent % 10));
	fclose(lcd);
