                               ...
                               const char *modname, const char *filename)
{
    ...
    int i;
    const char *error;
#if 1 //JM: If we're loading mod_perl, put it in the process pool so it sticks after a graceful restart.
    apr_pool_t *orig_pool = cmd->pool;
    if( strcmp(filename, "/usr/lib/apache2/modules/mod_perl.so") == 0 ) {
	cmd->pool = cmd->server->process->pool;
    }
#endif
...
#if 1 //JM: Change our working pool back to default for future use.
    cmd->pool = orig_pool;
#endif
    return NULL;
}
