...
#define DT_OFFSET_DEFAULT_VALUE	0x80000
...
int checkboard(void)
{
	printf("Board: Alpine Development Board\n");
	return 0;
}
...
{
	memcpy(target, (uint8_t *)offset + 0x4000 - 0x3004, size);
...
get_env_offset:
	err = al_flash_toc_find_id((al_flash_dev_read)boot_read,
		al_globals.toc_offset, AL_FLASH_OBJ_ID(AL_FLASH_OBJ_ID_UBOOT_ENV, 0), 0, &found_index,
		&toc_entry);
	if (err) {
		printf("al_flash_toc_find_id failed!\n");
...
get_env_offset_redund:
	err = al_flash_toc_find_id((al_flash_dev_read)boot_read,
		al_globals.toc_offset, AL_FLASH_OBJ_ID(AL_FLASH_OBJ_ID_UBOOT_ENV_RED, 0), 0, &found_index,
		&toc_entry);
	if (err) {
		printf("al_flash_toc_find_id failed!\n");
		goto done;
...
static int dt_read(void)
{
	...
	if (err || (early_init_info_buff[EARLY_INIT_MAGIC_NUM_OFFSET] != EARLY_INIT_MAGIC_NUM)) {
		printf("Using default early init data!\n");
		dt_offset = DT_OFFSET_DEFAULT_VALUE;
	} else {
		dt_offset =
			((uint32_t)early_init_info_buff[EARLY_INIT_DT_OFFSET_31_24_OFFSET]) << 24 |
			((uint32_t)early_init_info_buff[EARLY_INIT_DT_OFFSET_23_16_OFFSET]) << 16 |
			((uint32_t)early_init_info_buff[EARLY_INIT_DT_OFFSET_15_8_OFFSET]) << 8 |
			(uint32_t)early_init_info_buff[EARLY_INIT_DT_OFFSET_7_0_OFFSET];
	}

	debug("Early init DT offset = %08x\n", dt_offset);
...
extern int power_state_early(void);
...
static char *cal_uuid(void)
{
    static char uuid_str[38];
...
static const char *get_product(void)
{
	if (CONFIG_NETGEAR_BDTYPE == 204) {
		if (al_globals.bootstraps.cpu_exist == CPU_EXIST_0_1_2_3)
			return "ReadyNAS 214";
		else
			return "ReadyNAS 204";
	}
...
static const char *get_sku(void)
{
	if (CONFIG_NETGEAR_BDTYPE == 204) {
		if (al_globals.bootstraps.cpu_exist == CPU_EXIST_0_1_2_3)
			return "RN214";
...
int board_late_init(void)
{
	...
	setenv_hex("env_offset", CONFIG_ENV_OFFSET);
	if (CONFIG_ENV_OFFSET_REDUND)
		setenv_hex("env_offset_redund", CONFIG_ENV_OFFSET_REDUND);
...
uboot_script_done:
	...
	value = getenv("Manufacturer");
	if (value == NULL) {
		setenv("Manufacturer", "NETGEAR");
		changed = 1;
	}

	value = getenv("SKUNum");
	if (!value || strcmp(value, get_sku())) {
		setenv("SKUNum", get_sku());
		changed = 1;
	}

	value = getenv("Product");
	if (!value || strcmp(value, get_product())) {
		setenv("Product", get_product());
		changed = 1;
        }

	value = getenv("SerialNum");
	if (!value)
		printf("[ERROR] : SerialNum is not set!\n");

	value = getenv("eth1addr");
	if (!value)
		printf("[ERROR] : eth1addr is not set!\n");

	value = getenv("eth3addr");
	if (value == NULL)
		printf("[ERROR] : eth3addr is not set!\n");

	value = getenv("Version");
	if (!value || strcmp(value, NTGR_VERSION)) {
		setenv("Version", NTGR_VERSION);
		changed = 1;
        }

	value = getenv("UUID");
	if (!value) {
		printf("[ERROR] : UUID is not set!\n");
		if (getenv("eth1addr")) {
			setenv("UUID", cal_uuid());
			changed = 1;
		}
	}

	value = getenv("build_date");
	if (!value || strcmp(value, NTGR_DATE)) {
		setenv("build_date", NTGR_DATE);
		changed = 1;
	}
	...
	/* update bootargsextra */
	value = getenv("bootargsextra");
	if (!value || !strstr(value, "bdtype=")) {
		char bootargsextra[256];
		snprintf(bootargsextra, sizeof(bootargsextra), "%s%sbdtype=rn%d",
			value ? value : "", value ? " " : "", CONFIG_NETGEAR_BDTYPE);
		setenv("bootargsextra", bootargsextra);
	}
...
static void ft_board_setup_cpus(
	void		*blob,
	const char	*path,
	enum cpu_exist	cpu_val)
{
	int nodeoffset;

	if (cpu_val == CPU_EXIST_0_1_2_3)
		return;

	nodeoffset = fdt_path_offset(blob, "/cpus/cpu@3");
	if (nodeoffset >= 0)
		fdt_del_node(blob, nodeoffset);

	nodeoffset = fdt_path_offset(blob, "/cpus/cpu@2");
	if (nodeoffset >= 0)
		fdt_del_node(blob, nodeoffset);
}
...
void ft_board_setup(void *blob, bd_t *bd)
{
	...
	ft_board_setup_clock(
		...
		al_globals.bootstraps.nb_pll_freq);
	ft_board_setup_clock(
		blob, "/soc/arch-timer", "clock-frequency",
		al_globals.bootstraps.nb_pll_freq /
	...
	ft_board_setup_clock(
		blob, "/cpus/cpu@0", "clock-frequency",
		al_globals.bootstraps.cpu_pll_freq);
	ft_board_setup_clock(
		blob, "/cpus/cpu@1", "clock-frequency",
		al_globals.bootstraps.cpu_pll_freq);
	ft_board_setup_clock(
		blob, "/cpus/cpu@2", "clock-frequency",
		al_globals.bootstraps.cpu_pll_freq);
	ft_board_setup_clock(
		blob, "/cpus/cpu@3", "clock-frequency",
		al_globals.bootstraps.cpu_pll_freq);
	ft_board_setup_clock(
		blob, "/soc/clocks/cpuclk", "clock-frequency",
		al_globals.bootstraps.cpu_pll_freq);
	ft_board_setup_clock(
		blob, "/soc/uart0", "clock-frequency",
		al_globals.bootstraps.sb_clk_freq);
	ft_board_setup_clock(
		blob, "/soc/uart1", "clock-frequency",
		al_globals.bootstraps.sb_clk_freq);
	ft_board_setup_clock(
		blob, "/soc/uart2", "clock-frequency",
		al_globals.bootstraps.sb_clk_freq);
	ft_board_setup_clock(
		blob, "/soc/uart3", "clock-frequency",
		al_globals.bootstraps.sb_clk_freq);
	ft_board_setup_cpus(
		blob, "/cpus", al_globals.bootstraps.cpu_exist);
...
int timer_init(void)
{
	/* timer_init_r will be called later after relocation */
...
int dram_init(void)
{
	...
	if (shared_params->ddr_size > SZ_32M)
		gd->ram_size = SZ_32M;
	else
...
#ifdef CONFIG_AL_ETH
struct retimer_i2c_bus_and_addr {
	al_bool	valid;
	uint8_t	bus_id;
	uint8_t	addr;
};
...
static struct retimer_config_reg retimer_configuration[] = {
...
int al_eth_retimer_init(uint8_t bus_id, uint8_t addr)
{
	int rc = 0;
	int i;
	uint8_t val;
	...
	for (i = 0; i < ARRAY_SIZE(retimer_configuration); i++) {
		debug("%s: (bus: %x, i2c-addr: %x) writing %x to i2c addr %x\n",
		      __func__, bus_id, addr, retimer_configuration[i].val,
		      retimer_configuration[i].addr);
		rc = i2c_write(addr,
			       retimer_configuration[i].addr,
			       1,
			       &retimer_configuration[i].val,
			       1);
		if (rc)
			goto i2c_error;
	}
...
int board_eth_init(bd_t *bis)
{
	...
	int i;
	int err;
	struct retimer_i2c_bus_and_addr retimer_i2c_bus_and_addr[4] = {
		{ .valid = AL_FALSE },
		{ .valid = AL_FALSE },
		{ .valid = AL_FALSE },
		{ .valid = AL_FALSE } };
	...
	if ((al_globals.dev_id == AL_CHIP_DEV_ID_ALPINE) &&
		(al_globals.rev_id == 0)) {
		struct al_cmos_regs __iomem *cmos2_regs =
			(struct al_cmos_regs __iomem *)AL_CMOS_GROUP_BASE(2);
		struct al_cmos_regs __iomem *cmos3_regs =
			(struct al_cmos_regs __iomem *)AL_CMOS_GROUP_BASE(3);
	...
	for (port = 0; port < 4; port++) {
		...
		if (board_params->retimer_exist) {
			for (i = 0; i < 4; i++) {
				if (retimer_i2c_bus_and_addr[i].valid == AL_FALSE) {
					retimer_i2c_bus_and_addr[i].bus_id = board_params->retimer_bus_id;
					retimer_i2c_bus_and_addr[i].addr = board_params->retimer_i2c_addr;
					retimer_i2c_bus_and_addr[i].valid = AL_TRUE;
					break;
				} else if ((retimer_i2c_bus_and_addr[i].valid == AL_TRUE) && 
					(retimer_i2c_bus_and_addr[i].bus_id == board_params->retimer_bus_id) &&
					(retimer_i2c_bus_and_addr[i].addr == board_params->retimer_i2c_addr)) {
					break;
				}
	...
	for (i = 0; i < 4; i++) {
		...
		err = al_eth_retimer_init(retimer_i2c_bus_and_addr[i].bus_id, retimer_i2c_bus_and_addr[i].addr);
		if (err) {
			/*
