...
static unsigned int obj_id_from_args(const char *arg_id, const char *arg_instance)
{
	...
	instance = simple_strtoul(arg_instance, NULL, 16);
	if (id < 0)
		id = simple_strtoul(arg_id, NULL, 16);
	/* For the command "flash_contents_obj_read DT 0 $loadaddr_dt", use DT
	 * instance 1 instead so it can boot a modern kernel. */
	else if (id == AL_FLASH_OBJ_ID_DT && instance == 0)
		instance = 1;

	return AL_FLASH_OBJ_ID(id, instance);
}

static int flash_contents_mem_read(unsigned int offset, void *buff, unsigned int size)
{
	memcpy(buff, (const void *)offset, size);
...
static int do_flash_contents_obj_read(cmd_tbl_t *cmdtp, int flag, int argc, char *const argv[])
{
	...
	err = al_flash_obj_header_read_and_validate(dev_read, toc_entry.offset,
		&obj_hdr);
	if (err) {
	...
	err = al_flash_obj_data_load(dev_read, toc_entry.offset, (void *)load_addr);
	if (err) {
...
static int do_flash_contents_obj_read_mem(cmd_tbl_t *cmdtp, int flag, int argc, char *const argv[])
{
	...
	err = al_flash_obj_data_load(flash_contents_mem_read, read_addr, (void *)load_addr);
	if (err) {
		printf("al_flash_obj_data_load failed!\n");
		return 1;
	}
...
static int do_flash_contents_obj_validate(cmd_tbl_t *cmdtp, int flag, int argc, char *const argv[])
{
	...
	if (AL_FLASH_OBJ_ID_ID(obj_id) == AL_FLASH_OBJ_ID_STG2) {
		err = al_flash_stage2_validate(dev_read, toc_entry.offset, (void *)load_addr, size, &obj_hdr);
		if (err) {
			printf("al_flash_stage2_validate failed!\n");
			return 1;
		}
	} else {
		err = al_flash_obj_validate(dev_read, toc_entry.offset, (void *)load_addr, size, &obj_hdr);
		if (err) {
...
static int do_flash_contents_obj_update(cmd_tbl_t *cmdtp, int flag, int argc, char *const argv[])
{
	...
	if (first_block_num_bytes) {
		...
		debug("memcpy(%p, %p, %08x)\n", first_block_buff + first_block_num_bytes, (void *)load_addr, copy_size);
		memcpy(first_block_buff + first_block_num_bytes, (void *)load_addr, copy_size);
		load_addr += copy_size;
		size -= copy_size;
	...
	if (size > dev_erase_size) {
		...
		err = dev_write(toc_entry.offset, (void *)load_addr, size_current);
		if (err) {
			printf("dev_write failed!\n");
			err = 1;
			goto done_free_last;
...
static int do_flash_contents_stage2_trash(cmd_tbl_t *cmdtp, int flag, int argc, char *const argv[])
{
	int err;
	int found_index;
	struct al_flash_toc_entry toc_entry;
...
static int do_flash_contents_stage2_untrash(cmd_tbl_t *cmdtp, int flag, int argc, char *const argv[])
{
	int err;
	int found_index;
	struct al_flash_toc_entry toc_entry;
...
U_BOOT_CMD(
	flash_contents_stage2_trash, 2, 0, do_flash_contents_stage2_trash,
	"Trash a stage 2 instance",
	"<Instance number>");

U_BOOT_CMD(
	flash_contents_stage2_untrash, 2, 0, do_flash_contents_stage2_untrash,
	"Untrash a stage 2 instance",
	"<Instance number>");
