...
int do_ddr_ecc_stats(
	...
	char *const argv[])
{
	void __iomem *nb_regs_base = (void __iomem *)AL_NB_SERVICE_BASE;
	void __iomem *ddr_ctrl_regs_base = (void __iomem *)AL_NB_DDR_CTL_BASE;
	...
	al_ddr_ecc_cfg_get(ddr_ctrl_regs_base, &ddr_ecc_cfg);
	...
	err = al_ddr_ecc_status_get(
		ddr_ctrl_regs_base, &corr_status, &uncorr_status);
	if (err) {
	...
	if (clear) {
		al_ddr_ecc_corr_count_clear(ddr_ctrl_regs_base);
		al_ddr_ecc_corr_int_clear(nb_regs_base, ddr_ctrl_regs_base);
		al_ddr_ecc_uncorr_count_clear(ddr_ctrl_regs_base);
		al_ddr_ecc_uncorr_int_clear(nb_regs_base, ddr_ctrl_regs_base);
	}
