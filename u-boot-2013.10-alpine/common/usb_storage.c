...
static int usb_stor_BBB_clear_endpt_stall(struct us_data *us, int pipe)
{
	int result;
	int endpt = usb_pipeendpoint(pipe);
	...
	if (result >= 0)
		usb_reset_endpoint(us->pusb_dev, pipe);
...
static int usb_stor_BBB_transport(ccb *srb, struct us_data *us)
{
	...
	if ((result < 0) && (us->pusb_dev->status & USB_ST_STALLED)) {
		...
		result = usb_stor_BBB_clear_endpt_stall(us, pipe);
		if (result >= 0)
...
again:
	...
	if ((result < 0) && (retry < 1) &&
		...
		debug("STATUS:stall\n");
		/* clear the STALL on the endpoint */
		result = usb_stor_BBB_clear_endpt_stall(us, pipein);
		if (result >= 0 && (retry++ < 1))
			/* do a retry */
			goto again;
	}
