...
static void get_system_time(uuid_time_t *uuid_time)
{
  ...
  *uuid_time = ((uint64_t)rtc.tm_sec * 10000000)
    + ((uint64_t)0 * 10)
...
static void get_current_time(uuid_time_t *timestamp)
{
  ...
  static uint64_t uuids_this_tick;
  uuid_time_t time_now;
...
static void get_ieee_node_identifier(uuid_node_t *node)
{
  ...
  if (!inited) {
    ...
    if (getenv("eth1addr")) {
      eth_getenv_enetaddr ("eth1addr", (uchar *)&saved_node);
    }
  }
  inited = 1;
...
static int do_uuid(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
  char uuid_str[38];
