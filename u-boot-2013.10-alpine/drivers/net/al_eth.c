	...
	void __iomem	*mac_regs_base)
{
	...
	priv->retimer_bus_id = al_globals.eth_board_params[index].retimer_bus_id;
	...
	priv->bus_index    = al_globals.eth_board_params[index].i2c_adapter_id;
...
static int al_eth_dev_init(struct eth_device *dev)
{
	...
	pci_internal_snoop_enable(priv->pci_devno);
#endif
	...
	struct eth_device *dev)
{
	...
	if (al_eth_flr_rmn(
		...
		(void *)((struct al_eth_priv *)dev->priv)->pci_devno,
		((struct al_eth_priv *)dev->priv)->mac_regs_base))
...
static int al_eth_flr_read_config_u32(
	...
	uint32_t *val)
{
	pci_dev_t devno = (pci_dev_t)handle;
