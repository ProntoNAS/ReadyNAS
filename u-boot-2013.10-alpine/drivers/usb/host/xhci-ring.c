...
void xhci_find_new_dequeue_state(struct xhci_ctrl *ctrl,
		int slot_id, int ep_index, struct xhci_dequeue_state *state)
{
	struct xhci_virt_device *virt_dev;
	struct xhci_ring *ring;
	...
	/*
	 * XXX: Calling routine prepare_ring() called in place of
	 * xhci_find_new_dequeue_state() as there in 'Linux' since we are not
	 * maintaining multiple TDs/transfer at the same time.
	 */
	BUG_ON(prepare_ring(ctrl, ring, EP_STATE_RUNNING));
	state->new_deq_seg = ring->enq_seg;
	state->new_deq_ptr = ring->enqueue;
	state->new_cycle_state = ring->cycle_state;	
}
