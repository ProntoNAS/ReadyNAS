 ...
 * @return none
 */
void usb_reset_endpoint(struct usb_device *udev, int pipe)
{
	struct xhci_ctrl *ctrl = udev->controller;
	...
	xhci_queue_command(ctrl, NULL, slot_id, ep_index, TRB_RESET_EP);
	event = xhci_wait_for_event(ctrl, TRB_COMPLETION);
	BUG_ON(GET_COMP_CODE(le32_to_cpu(event->event_cmd.status))
		!= COMP_SUCCESS);
	xhci_acknowledge_event(ctrl);

	xhci_find_new_dequeue_state(ctrl, slot_id, ep_index, &deq_state);
	xhci_queue_command(ctrl, (void *)((uintptr_t)deq_state.new_deq_ptr |
		deq_state.new_cycle_state), slot_id, ep_index, TRB_SET_DEQ);
	event = xhci_wait_for_event(ctrl, TRB_COMPLETION);
	BUG_ON(GET_COMP_CODE(le32_to_cpu(event->event_cmd.status))
		!= COMP_SUCCESS);
	xhci_acknowledge_event(ctrl);
