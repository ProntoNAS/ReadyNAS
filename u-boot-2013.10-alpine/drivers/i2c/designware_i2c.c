...
#if !defined(CONFIG_SYS_I2C_DW_SPEED)
#define CONFIG_SYS_I2C_DW_SPEED CONFIG_SYS_I2C_SPEED
#endif
#if !defined(CONFIG_SYS_I2C_DW_SLAVE)
#define CONFIG_SYS_I2C_DW_SLAVE CONFIG_SYS_I2C_SLAVE
#endif
...
/*
 * dw_set_speed - Set the i2c speed mode (standard, high, fast)
 * @i2c_spd:	required i2c speed mode
 *
 * Set the i2c speed mode (standard, high, fast)
 */
static void dw_set_speed(int i2c_spd)
{
	unsigned int cntl;
			...
			unsigned int speed)
{
	if (speed >= I2C_MAX_SPEED)
		dw_set_speed(IC_SPEED_MODE_MAX);
	else if (speed >= I2C_FAST_SPEED)
		dw_set_speed(IC_SPEED_MODE_FAST);
	else
		dw_set_speed(IC_SPEED_MODE_STANDARD);

	return 0;
}

/*
 * dw_i2c_get_bus_speed - Gets the i2c speed
 *
 * Gets the i2c speed.
 */
int dw_i2c_get_bus_speed(void)
{
	u32 cntl;
 ...
 */
static void dw_i2c_init(struct i2c_adapter *adap, int speed, int slaveadd)
{
	unsigned int enbl;
	...
	writel(0, &i2c_regs_p->ic_intr_mask);
...
/* check parameters for dw_i2c_read and dw_i2c_write */
...
static int i2c_xfer_init(uchar chip, uint addr, int alen)
{
	...
	/* write i2c slave internal offset address, MSB to LSB */
	while (alen > 0) {
		alen = alen - 1;
		addr_byte = addr >> (8 * alen);
		addr_byte = addr_byte & 0xFF;
		writel(addr_byte, &i2c_regs_p->ic_cmd_data);
	}
 ...
 */
static int dw_i2c_read(struct i2c_adapter *adap, uchar chip, uint addr,
		int alen, uchar *buffer, int rx_bytes_cnt)
{
	unsigned long start_time_rx;
	int tx_bytes_cnt = rx_bytes_cnt;

	if (check_params(addr, alen, buffer, rx_bytes_cnt))
		return 1;
	...
	while (rx_bytes_cnt) {
		int status = readl(&i2c_regs_p->ic_status);
		/* For new read xfer: issue read command */
		if ((tx_bytes_cnt > 0) &&
			(status & IC_STATUS_TFNF) && !(status & IC_STATUS_RFF)) {
			/* On last byte, issue a STOP intr as well */
			if (--tx_bytes_cnt == 0)
				writel(IC_CMD | IC_STOP,
 ...
 * Write to i2c memory.
 */
static int dw_i2c_write(struct i2c_adapter *adap, uchar chip, uint addr,
 ...
 */
static int dw_i2c_probe(struct i2c_adapter *adap, uchar chip)
{
	u32 tmp;
	int ret;
	 ...
	 */
	ret = dw_i2c_read(adap, chip, 0, 1, (uchar *)&tmp, 1);
	if (ret)
		dw_i2c_init(adap, CONFIG_SYS_I2C_SPEED, CONFIG_SYS_I2C_SLAVE);

	return ret;
}

#ifdef CONFIG_I2C_MULTI_BUS
int dw_i2c_set_bus_num(struct i2c_adapter *adap, unsigned int bus)
{
	switch (bus) {
	case 0:
...
int dw_i2c_get_bus_num(void)
{
	return current_bus;
}
#endif

U_BOOT_I2C_ADAP_COMPLETE(dw, dw_i2c_init, dw_i2c_probe,
			 dw_i2c_read, dw_i2c_write, dw_i2c_set_bus_speed,
			 CONFIG_SYS_I2C_DW_SPEED, CONFIG_SYS_I2C_DW_SLAVE,
			 0)
