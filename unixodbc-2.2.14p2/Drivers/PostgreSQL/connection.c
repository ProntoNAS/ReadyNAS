...
*/
	...
	result = PG__SQLExecDirect(hstmt, (unsigned char*) "set DateStyle to 'ISO'", SQL_NTS);
	if((result != SQL_SUCCESS) && (result != SQL_SUCCESS_WITH_INFO))
		status = FALSE;
	...
	if (globals.disable_optimizer) {
		result = PG__SQLExecDirect(hstmt, (unsigned char*) "set geqo to 'OFF'", SQL_NTS);
		if((result != SQL_SUCCESS) && (result != SQL_SUCCESS_WITH_INFO))
			status = FALSE;
	...
	/*	KSQO */
	if (globals.ksqo) {
		result = PG__SQLExecDirect(hstmt, (unsigned char*) "set ksqo to 'ON'", SQL_NTS);
		if((result != SQL_SUCCESS) && (result != SQL_SUCCESS_WITH_INFO))
			status = FALSE;
	...
	if (globals.conn_settings[0] != '\0') {
		...
		ptr = strtok(cs, ";");
		while (ptr) {
			result = PG__SQLExecDirect(hstmt, (unsigned char*) ptr, SQL_NTS);
			if((result != SQL_SUCCESS) && (result != SQL_SUCCESS_WITH_INFO))
				status = FALSE;
...
*/
	...
	result = PG__SQLExecDirect(hstmt, (unsigned char*) "select oid from pg_type where typname='" PG_TYPE_LO_NAME "'", SQL_NTS);
	if((result != SQL_SUCCESS) && (result != SQL_SUCCESS_WITH_INFO)) {
		PG__SQLFreeStmt(hstmt, SQL_DROP);
		return;
	}
