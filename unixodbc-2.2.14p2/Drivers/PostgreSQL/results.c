...
char *msg, *ptr;
	...
	/* BEGIN - Why not return rowcount? Anyway; its need by StarOffice so...
	...
	*/	
	if ( stmt->manual_result )
	{
		*pcrow = stmt->result->manual_tuples->num_tuples;
		return SQL_SUCCESS;
	}
	/* END - Peter Harvey */
...
mylog("SQLGetData: enter, stmt=%u\n", stmt);
	...
	if ( stmt->manual_result || ! globals.use_declarefetch) {
		...
		/* BEGIN - This gets rid of dot prefix on, table names, in StarOffice (see also OWNER_USAGE) */
		if ( field_type == PG_TYPE_TEXT && value != NULL )
		{
			if ( *(char*)value == '\0' )
			{
				value = NULL;
		...
		/* END - Peter Harvey, Christian Kirsch */
	}
...
mylog("SQLSetCursorName: hstmt=%u, szCursor=%u, cbCursorMax=%d\n", hstmt, szCursor, cbCursor);
	...
	strncpy_null(stmt->cursor_name, (char*)szCursor, len+1);
	return SQL_SUCCESS;
}
...
mylog("SQLGetCursorName: hstmt=%u, szCursor=%u, cbCursorMax=%d, pcbCursor=%u\n", hstmt, szCursor, cbCursorMax, pcbCursor);
	...
	if (szCursor) {
		strncpy_null((char*)szCursor, stmt->cursor_name, cbCursorMax);
