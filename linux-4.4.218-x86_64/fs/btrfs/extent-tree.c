...
out:
	...
	if (root->root_key.objectid == BTRFS_DATA_RELOC_TREE_OBJECTID
		...
		btrfs_debug(root->fs_info,
			"cross ref:objectid=%llu,offset=%llu,bytenr=%llu",
			objectid, offset, bytenr);
	return ret;
}
					    ...
					    u64 len)
{
	...
	spin_lock(&data_sinfo->lock);
	if (data_sinfo->bytes_may_use < len)
...
static void update_global_block_rsv(struct btrfs_fs_info *fs_info)
{
	...
	if (num_bytes >> 1 > sinfo->bytes_used)
		num_bytes = ALIGN(sinfo->bytes_used >> 1,
				  fs_info->extent_root->nodesize << 10);
...
/*
 * Check whether block group is located in sweep destination devices.
 * Avoid data migrated back and forth in sweep destination device.
 * This is only for balance sweep.
 */
static bool
is_block_group_in_sweep_device(struct btrfs_root *root, u64 flags,
			      struct btrfs_block_group_cache *block_group)
{
	struct btrfs_fs_info *fs_info = root->fs_info;
	...
	/*only for data sweeping*/
	sweep_devid = fs_info->sweep_ctl->data.device->devid;

	read_lock(&map_tree->map_tree.lock);
	em = lookup_extent_mapping(&map_tree->map_tree,
			block_group->key.objectid, 1);
	if (!em) {
		read_unlock(&map_tree->map_tree.lock);
		return found;
	}
	read_unlock(&map_tree->map_tree.lock);
	if (em && em->start == block_group->key.objectid) {
		map = em->map_lookup;
		for (i = 0; i < map->num_stripes; ++i) {
			if (map->stripes[i].dev->devid == sweep_devid) {
				btrfs_debug(fs_info, "Found sweep devid=%llu\n",
...
/*
 * Check whether block group is located in tier devices.
 * Avoid data written to non-tier devices even there are free space in
 * tier devices. This should exclude normal balance and data sweeping.
 */
static bool
is_block_group_in_tier_device(struct btrfs_root *root, u64 flags,
			      struct btrfs_block_group_cache *block_group)
{
	struct btrfs_fs_info *fs_info = root->fs_info;
	struct btrfs_mapping_tree *map_tree = &fs_info->mapping_tree;
	...
	if (em && em->start == block_group->key.objectid) {
		...
		for (i = 0; i < map->num_stripes; ++i) {
			...
			if (BTRFS_DEV_CLASS_IS_TIER(device)) {
				btrfs_debug(fs_info, "Found devid=%llu\n",
					    device->devid);
				ret = true;
				break;
			}
...
/*
 * Check whether there is enough space to allocate chunk
 * from tier devices.
 */
static bool
can_alloc_chunk_in_tier_devices(struct btrfs_root *root)
{
	struct btrfs_fs_devices *fs_devices = root->fs_info->fs_devices;
	struct list_head *cur;
	/* 1024 * 1024 * 1024 */
	u64 min_chunk_size = 1073741824;
	bool ret = false;
...
static noinline int find_free_extent(struct btrfs_root *orig_root,
				...
				u64 flags, int delalloc, int tier)
{
	int ret = 0;
...
search:
	...
	list_for_each_entry(block_group, &space_info->block_groups[index],
		...
		if (is_block_group_in_sweep_device(root, flags, block_group))
			continue;

		if (tier &&
		    !is_block_group_in_tier_device(root, flags, block_group))
			continue;
...
loop:
	...
	if (!ins->objectid && loop < LOOP_NO_EMPTY_SIZE) {
		...
		/*
		 * Need to check whether tier devices have enough space
		 * to allocate a chunk. If not,just return and
		 * try second-round find.
		 */
		if (tier && loop == LOOP_ALLOC_CHUNK &&
		    !can_alloc_chunk_in_tier_devices(root)) {
			ret = -EAGAIN;
			goto out;
		}
			 ...
			 struct btrfs_key *ins, int is_data, int delalloc)
{
	...
	WARN_ON(num_bytes < root->sectorsize);
	if (is_data && BTRFS_TIER_DATA_ENABLED(super_flags) &&
	    !sweep_in_progress(root) && !balance_in_progress(root)) {
		ret = find_free_extent(root, ram_bytes, num_bytes, empty_size,
			       hint_byte, ins, flags, delalloc, 1);
		if (!ret)
			goto out;
	}
again:
	ret = find_free_extent(root, ram_bytes, num_bytes, empty_size,
			       hint_byte, ins, flags, delalloc, 0);
	if (!ret && !is_data) {
