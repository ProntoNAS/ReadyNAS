...
{
	unsigned long long index = start >> PAGE_CACHE_SHIFT;
	unsigned long long end_index = end >> PAGE_CACHE_SHIFT;
	struct page *page;
					   ...
					   u64 start, u64 end)
{
	...
	unsigned long long index = start >> PAGE_CACHE_SHIFT;
	unsigned long long end_index = end >> PAGE_CACHE_SHIFT;
	unsigned long long nr_pages = end_index - index + 1;
	int i;
...
static noinline int lock_delalloc_pages(struct inode *inode,
					...
					u64 delalloc_end)
{
	unsigned long long index = delalloc_start >> PAGE_CACHE_SHIFT;
	unsigned long long start_index = index;
	unsigned long long end_index = delalloc_end >> PAGE_CACHE_SHIFT;
			 ...
			 u64 *prev_em_start)
{
	...
	while (cur <= end) {
		unsigned long long pnr = (last_byte >> PAGE_CACHE_SHIFT) + 1;
	...
	while (cur <= end) {
		...
		if (!PageWriteback(page)) {
			btrfs_err(BTRFS_I(inode)->root->fs_info,
			       ...
			       (long long unsigned)page->index, cur, end);
		}
			      ...
			      void *data)
{
	...
	size_t pg_offset = 0;
	loff_t i_size = i_size_read(inode);
	unsigned long long end_index = i_size >> PAGE_CACHE_SHIFT;
	int write_flags;
			...
			unsigned long start, unsigned long len)
{
	...
	unsigned long long i = (start_offset + start) >> PAGE_CACHE_SHIFT;
			      ...
			      unsigned long *map_len)
{
	...
	size_t start_offset = eb->start & ((u64)PAGE_CACHE_SIZE - 1);
	unsigned long long i = (start_offset + start) >> PAGE_CACHE_SHIFT;
	unsigned long long end_i = (start_offset + start + min_len - 1) >>
		PAGE_CACHE_SHIFT;
		...
		const void *name, unsigned long start, unsigned long len)
{
	...
	if (CASEINSENSITIVE_CHECK(root)) {
		return memcmp_extent_buffer(eb, name, start, len);
	}
	...
	return CASEINSENSITIVE_OPS(root)->strcasecmp_vec(&namevec, 1,
							vec, vecpos);
}
#endif
			...
			unsigned long len)
{
	...
	unsigned long long i = (start_offset + dst_offset) >> PAGE_CACHE_SHIFT;
			   ...
			   unsigned long src_offset, unsigned long len)
{
	...
	size_t start_offset = dst->start & ((u64)PAGE_CACHE_SIZE - 1);
	unsigned long long dst_i;
	unsigned long long src_i;
