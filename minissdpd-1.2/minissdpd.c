...
int get_ifaces(const char *ifaces[], int max_ifaces)
{
        DIR *dir;
        struct dirent _de;
        struct dirent *de;
        ...
        if( (dir = opendir("/sys/class/net")) == NULL )
                return -1;
        while( ! readdir_r(dir, &_de, &de) && de &&
                        i < max_ifaces ) {
                if( strncmp(de->d_name, "eth", 3) == 0 ||
                        ( (strncmp(de->d_name, "bond", 4) == 0) && (strlen(de->d_name) <= 6)) ) {
                        ifaces[i] = strdup(de->d_name);
                        ++i;
                }
        }
        closedir(dir);
...
#define s_ssdp6 (-1)
	...
	for(i=1; i<argc; i++)
	{
        ...
        n_if_addr = get_ifaces(if_addr, MAX_IF_ADDR);
	if(n_if_addr < 1)
	{
		fprintf(stderr,
	...
	while(!quitting)
	{
		...
		if((s_ssdp6 >= 0) && FD_ISSET(s_ssdp6, &readfds))
		{
			...
			else
			{
				...
				{
					if(deltadev > 0)
						syslog(LOG_INFO, "%d new devices added", deltadev);
					else if(deltadev < 0)
						syslog(LOG_INFO, "%d devices removed (good-bye!)", -deltadev);
					deltadev = i;
				}
