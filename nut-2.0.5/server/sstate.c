...
int sstate_connect(upstype *ups)
{
	...
	/* Retry with just the UPS name */
	if (ret < 0) {
		snprintf(sa.sun_path, sizeof(sa.sun_path), "%s", ups->name);
		ret = connect(fd, (struct sockaddr *) &sa, sizeof(sa));
	}
	/* Retry with just the end portion of the default name */
	if (ret < 0) {
		tmp = strrchr(ups->fn, '-');
		if( tmp ) {
	...
	/* Retry replacing the port 'auto' with the UPS name */
	if (ret < 0) {
		if(strstr(ups->fn, "auto")) {
			tmp = strrchr(ups->fn, '-');
			if( tmp )
				*tmp = '\0';
			snprintf(sa.sun_path, sizeof(sa.sun_path), "%s-%s", ups->fn, ups->name);
			ret = connect(fd, (struct sockaddr *) &sa, sizeof(sa));
		}
	}
