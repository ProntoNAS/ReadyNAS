...
#include "rtc/ext_rtc/usiRS5C372.h" //libo 2011-02-18
...
#if 0 //libo 2011-02-18
	        mvRtcTimeGet(&time);
#else
	       mvRS5C372TimeGet(&time);
       //printk("libo ...... %s, %d\n", __func__, time.year);
#endif
...
static int mv_rtc_read_alarm(struct device *dev, struct rtc_wkalrm *t)
{
       MV_RTC_TIME time;
       unsigned long temp_t;

       usiRS5C372AlarmRead(&time, &(t->enabled), &(t->pending));

       /* same as in the U-Boot we use the year for century 20 only */
       temp_t = mktime ( time.year + 2000, time.month,
                               time.date, time.hours,
                               time.minutes, time.seconds);
       rtc_time_to_tm(temp_t, &(t->time));

       return 0;

}
static int mv_rtc_set_alarm(struct device *dev, struct rtc_wkalrm *t)
{
       MV_RTC_TIME time;
       unsigned long temp_t;

       rtc_tm_to_time(&(t->time), &temp_t);
       to_tm(temp_t, &time);
       /* same as in the U-Boot we use the year for century 20 only */
       time.year -= 2000;

       usiRS5C372AlarmSet(&time, &(t->enabled), &(t->pending));
       return 0;
}
#endif
...
#if 1 //libo 2011-02-18
       usiNasTwsiInit();  //robinson add
        usiRS5C272Init();
       mvRS5C372TimeGet(&time);
#else
	mvRtcTimeGet(&time);
