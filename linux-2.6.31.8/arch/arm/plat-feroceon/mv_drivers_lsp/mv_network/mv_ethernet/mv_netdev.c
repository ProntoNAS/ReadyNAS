...
//andrew start
static struct proc_dir_entry *eth_link_status;
...
int eth_link_status_read (char *buffer, char **buffer_location, off_t offset,
       ...
       u32         port_status;
#if 0
       port_status = MV_REG_READ( ETH_PORT_STATUS_REG( 0 ) );
       if(port_status & ETH_LINK_UP_MASK)
       {
               printk( KERN_NOTICE "%s: link up", dev->name );
       ...
       {
               if( port_status & ETH_GMII_SPEED_1000_MASK )
                       sprintf(name_buff, "%s", "link up, speed 1 Gbps");
               else
                       sprintf(name_buff, "%s", "link up, speed 100 Mbps or 10 Mbps");
       }
...
#endif /* CONFIG_MV_CPU_PERF_CNTRS */
//andrew start
       eth_link_status = create_proc_entry ("eth_link_status" , 0666 , &proc_root);
       eth_link_status->read_proc = eth_link_status_read;
       eth_link_status->write_proc = NULL;
       eth_link_status->nlink = 1;
//andrew end
