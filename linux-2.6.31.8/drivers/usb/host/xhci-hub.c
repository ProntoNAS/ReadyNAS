...
int xhci_hub_status_data(struct usb_hcd *hcd, char *buf)
{
	...
	for (i = 0; i < ports; i++) {
		...
		/* Adding for Kingston DT Ultimate 16GB USB3 thumb */
                t2 = xhci_port_state_to_neutral(temp);
                if (temp & PORT_POWER) {
                        if ((temp & PORT_PLS_MASK) == XDEV_INACTIVE) {
                                t2 |= PORT_WR;
                        } else if ((temp & PORT_PLS_MASK) == XDEV_DISABLED) {
                                t2 = (t2 & ~(PORT_PLS_MASK | PORT_PE)) | XDEV_RXDETECT;
                                t2 |= PORT_LINK_STROBE;
                        }
                }
                ...
                temp = xhci_port_state_to_neutral(temp);
                if (temp != t2)
                        xhci_writel(xhci, t2, port_array[i]);
		/* end */
