...
static int
is_snapdir(const char *name, const char *path, struct stat *sb)
{
  struct statfs f;

  if (!sb && strcmp(name, ".snapshots") == 0)
    /* Inotify context */
    return 1;
...
process_directory(char *path, int parent_id, int flags)
{
  ...
  for (;;)
      ...
      else if (S_ISFIFO(sb.st_mode))
	{
      ...
      else if (is_snapdir(buf.d_name, entry, &sb))
	DPRINTF(E_LOG, L_SCAN, "Skipping %s, visible snapshot directory\n", entry);
...
process_inotify_dir(struct watch_info *wi, char *path, struct inotify_event *ie)
{
  ...
  if (is_snapdir(ie->name, path, NULL))
    {
      DPRINTF(E_LOG, L_SCAN, "Skipping %s, snapshot directory\n", path);
      return;
    }
