...
db_pragma_integrity_check()
{
  ...
  ret = strcmp(status, "ok") ? SQLITE_ERROR : SQLITE_OK;
...
db_integrity_check(void)
{
  ...
  ret = db_pragma_integrity_check();
  sqlite3_close(hdl);
  if (ret != SQLITE_OK)
    {
      DPRINTF(E_LOG, L_DB, "Failed database integrity check; regenerating\n");
      unlink(db_path);
    }
