...
mv_xor_prep_dma_crc32c(struct dma_chan *chan, dma_addr_t src,
		size_t len, u32 *crc, unsigned long flags)
{
	struct mv_xor_chan *mv_chan = to_mv_xor_chan(chan);
	struct mv_xor_desc_slot *sw_desc;

	dev_dbg(mv_chan->device->common.dev,
		"%s src %x len: %u flags: %lx\n",
		__func__, src, len, flags);

	/* This hardware assumes seed is always ~0.
	 * Despite documentation, data seems to need aligned.
	 * Data size limitation: 8..16M-1.
	 */
	if (*crc != ~0 || src & 3 || len & 3 || len < 8 || (1 << 14) <= len)
		return (struct dma_async_tx_descriptor *)1;

	BUG_ON(len > MV_CRC32C_MAX_BYTE_COUNT);
