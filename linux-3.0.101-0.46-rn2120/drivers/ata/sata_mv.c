				  ...
				  void __iomem *mmio, unsigned int port)
{
	...
	/* USI 78230 SATA controller eye diagram adjustment*/
	printk("RN2120 adjust signal for port %d\n",port);
	reg = readl(port_mmio + 0x83C);
	writel(reg, port_mmio + 0x83C);
	//reg = 0x8A2F;
	reg = 0x8A2B;
	writel(reg, port_mmio + 0x83C);

	reg = readl(port_mmio + 0x834);
	reg = 0xC924 ;
	writel(reg, port_mmio + 0x834);
