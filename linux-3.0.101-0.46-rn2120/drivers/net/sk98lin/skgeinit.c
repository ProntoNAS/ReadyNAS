...
SK_IOC	IoC)		/* I/O Context */
{
	...
	/* the RAM Interface Arbiter is enabled */
...
SK_IOC	IoC)		/* I/O Context */
{
	...
	if (ChipId < CHIP_ID_YUKON || ChipId > CHIP_ID_YUKON_UL_2) {
		/* unexpected Chip Id */
		return(5);
	}
	...
	case CHIP_ID_YUKON_SUPR:
		...
		if (ChipId == CHIP_ID_YUKON_SUPR && ChipRev >= CHIP_REV_YU_SU_B1) {
			/* enable MACSec clock gating */
			SK_OUT32(IoC, PCI_C(pAC, PCI_OUR_REG_3), BIT_17);
		}
		break;
		...
		if (ChipId == CHIP_ID_YUKON_EC_U ||
			...
			if (HW_FEATURE(pAC, HWF_ENA_POW_SAV_W_WOL)) {
				...
				if ((ChipId == CHIP_ID_YUKON_EC_U &&
					...
					SK_OUT32(IoC, PCI_C(pAC, PCI_CFG_REG_1),
...
int		Port)		/* Port Index 0 or 1 */
{
	...
	if (pAC->GIni.GIAsfEnabled) {
		/*
		 * ASF firmware is checking start and end pointer of its ASF queue
		 * periodically.
