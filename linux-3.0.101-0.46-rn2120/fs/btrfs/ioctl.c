...
static int btrfs_ioctl_get_root_item(struct file *file, void __user *arg)
{
	struct btrfs_root_item *root_item =
		&BTRFS_I(fdentry(file)->d_inode)->root->root_item;

	if (copy_to_user(arg, root_item, sizeof(*root_item)))
		return -EFAULT;
	return 0;
}

static int btrfs_ioctl_get_root_item_otime(struct file *file, void __user *arg)
{
	struct btrfs_root_item *root_item =
		&BTRFS_I(fdentry(file)->d_inode)->root->root_item;
	struct timespec tv;

	tv.tv_sec = le64_to_cpu(root_item->otime.sec);
	tv.tv_nsec = le64_to_cpu(root_item->otime.nsec);
	if (copy_to_user(arg, &tv, sizeof(tv)))
		return -EFAULT;
	return 0;
}
		...
		cmd, unsigned long arg)
{
	...
	case BTRFS_IOC_GET_ROOT_ITEM:
		return btrfs_ioctl_get_root_item(file, argp);
	case BTRFS_IOC_GET_ROOT_ITEM_OTIME:
		return btrfs_ioctl_get_root_item_otime(file, argp);
	}
