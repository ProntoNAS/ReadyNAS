...
int get_key() {
    ...
    if ((controllers[ctrl.index].device == 0x5020) || 
		(controllers[ctrl.index].device == 0xa000)||
		(controllers[ctrl.index].device == 0x0BF3)) 
		return(0);
	if ((c & 1) == 0) {
		if (serial_cons) {
			int comstat;
...
void serial_echo_print(const char *p)
{
 ...
 void serial2_echo_print(const char *p)
 {
 	/* Now, do each character */
 	while (*p) {
 		WAIT_FOR_2XMITR;
 		...
 		/* Send the character out. */
 		serial2_echo_outb(*p, UART_TX);
 		if(*p==10) {
 			WAIT_FOR_2XMITR;
 			serial2_echo_outb(13, UART_TX);
             return;
 		}
 		p++;
 	}
...
diff -EbwBp -x 'cscope.*' -x changelog -x '*.o' memtest86+-2.10/Makefile ../mycvs/memtest86+-2.10/Makefile
*** memtest86+-2.10/Makefile	2008-10-24 19:43:43.000000000 -0700
--- ../mycvs/memtest86+-2.10/Makefile	2008-12-09 17:27:26.000000000 -0800
/* Except for multi-character key sequences this mapping
 * table is complete.  So it should not need to be updated
 * when new keys are searched for.  However the key handling
