...
static char     memhead[16]="Pss Err  Time  "; 
static char     memdisp[16]=" 0   0 00:00:00";
...
static void display_init(void)
{
	...
	cprint(0, 0, "      Memtest86  v4.20   NG0");
...
#ifdef G2_BOOT
#  warning "Compiling for G2 x86 boot menu."	
#else
#  warning "Compiling for G1 x86 platform."	
#endif
  ...
  /* Check fail safe */	
    ...
    if ((controllers[ctrl.index].device == 0x5020) || 
                ((controllers[ctrl.index].device == 0xa000) && (ctrl.sernum == 0))) {
        lcd_main(controllers[ctrl.index].device);
    } else if ((controllers[ctrl.index].device == 0x2990) || 
                ((controllers[ctrl.index].device == 0xa000) && (ctrl.sernum != 0))) {
        serial2_echo_print("p0\r");
        serial2_echo_print("e\r");
        serial2_echo_print("f2\r");
        for (i=0; i<10000; i++) __delay(50000);
        serial2_echo_print("o8100\r");
        for (i=0; i<10000; i++) __delay(50000);
        serial2_echo_print("c0 0\n");
        serial2_echo_print("l     Memory Test     \n");
        for (i=0; i<10000; i++) __delay(50000);
        serial2_echo_print("c0 11\n");
        serial2_echo_print("l-------- ---- --- ---\n");
        for (i=0; i<10000; i++) __delay(50000);
        serial2_echo_print("c0 a\n");
        serial2_echo_print("l  Time   Prog Pss Err\n");
        for (i=0; i<10000; i++) __delay(50000);
        serial2_echo_print("c0 18\n");
        serial2_echo_print("l00:00:00   0%   0   0\n");
    }else if((controllers[ctrl.index].device == 0x0BF3)){    		
    		G2_4bay_lcd_main(controllers[ctrl.index].device);
    }else if((controllers[ctrl.index].device == 0x04)&&
    		 (controllers[ctrl.index].vendor == 0xFFFF)){
    		  oled_init();
    }

	show_lcd_line(1, memhead);
    show_lcd_line(2, memdisp);
